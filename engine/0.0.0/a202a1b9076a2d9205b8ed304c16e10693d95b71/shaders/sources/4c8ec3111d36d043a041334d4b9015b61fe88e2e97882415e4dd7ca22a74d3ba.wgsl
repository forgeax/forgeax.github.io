...
fn evaluateStandardSurface(in_10: VsOut, frontFacing_2: bool) -> SurfaceData {
    ...
    let _e26 = materialVertexColor(in_10);
    let input_4 = SurfaceInput(positionOS, in_10.worldPos, in_10.worldNormal, in_10.worldTangent, viewDirectionWS, in_10.uv, in_10.uv1_, in_10.uv2_, in_10.uv3_, in_10.uv4_, in_10.uv5_, in_10.uv6And7_.xy, in_10.uv6And7_.zw, _e26, frontFacing_2);
    let _e31 = selectedMaterial(in_10.materialAddress.x);
    let _e32 = evaluate_standard_surface(input_4, _e31);
    return _e32;
}
...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let _e75 = probeBlendRecords[(probeBase + 1u)];
    let _e80 = probeBlendRecords[(probeBase + 2u)];
    let _e85 = probeBlendRecords[(probeBase + 3u)];
    let _e90 = probeBlendRecords[(probeBase + 4u)];
    let _e95 = probeBlendRecords[(probeBase + 5u)];
    let _e100 = probeBlendRecords[(probeBase + 6u)];
    let _e105 = probeBlendRecords[(probeBase + 7u)];
    let _e110 = probeBlendRecords[(probeBase + 8u)];
    let _e115 = probeBlendRecords[(probeBase + 9u)];
    probeShPreblend = array<vec4<f32>, 9>(_e75, _e80, _e85, _e90, _e95, _e100, _e105, _e110, _e115);
    let _e121 = probeBlendRecords[probeBase].z;
    probeLocalBlendFraction = select(0f, _e121, probeIdentityValid);
    let _e126 = physicalNormal;
    let _e127 = diffuseAlbedo;
    let _e128 = f0_;
    let _e130 = skylight;
    let _e131 = probeShPreblend;
    let _e132 = probeLocalBlendFraction;
    let _e139 = evaluateStandardEnvironmentX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_2.worldPos, _e126, v_5, _e127, metallic_17, iblRoughness, _e128, _e130, irradianceMap_2, irradianceSampler_2, prefilterMap_2, prefilterSampler_2, brdfLut_4, skylightPrefilterMap_1, _e131, _e132);
    let ao = _e8.occlusion;
    let _e143 = skylight.colorR;
    let _e146 = skylight.colorG;
    let _e149 = skylight.colorB;
    let _e153 = skylight.intensity;
    let _e154 = decodeSpecularEnvironmentScaleX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(vec3<f32>(_e143, _e146, _e149), _e153);
    let _e161 = coatF;
    ambient = (((_e139.diffuse + _e139.specular) * (vec3(1f) - _e161)) * ao);
    let _e169 = coatF;
    reflectionFallback = ((_e139.specular * (vec3(1f) - _e169)) * ao);
    let _e177 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.worldViewProj;
    let _e180 = sampleStandardAmbientOcclusionX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_2.worldPos, _e177, ssaoBlurredTexture, ssaoBlurredSampler);
    let _e181 = ambient;
    ambient = (_e181 * _e180);
    let _e183 = reflectionFallback;
    reflectionFallback = (_e183 * _e180);
    let _e185 = ambient;
    color = _e185;
    ...
    let _e191 = color;
    let _e195 = standardViewZ(in_2);
    let _e196 = physicalNormal;
    let _e197 = diffuseAlbedo;
    let _e198 = f0_;
    let _e199 = evaluateStandardDirectX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_2.worldPos, in_2.ndc.xyz, _e195, _e196, v_5, _e197, metallic_17, a_4, _e198, _e190);
    color = (_e191 + _e199);
    let _e201 = color;
    color = (_e201 + _e8.emissive);
    let _e206 = color;
    output.color = vec4<f32>(_e206, alpha_3);
    let _e209 = reflectionFallback;
    let _e210 = standardSsrCoverage();
    output.reflectionFallback = vec4<f32>(_e209, _e210);
    let _e212 = output;
    return _e212;
}
...
fn fs_gbuffer(in_3: VsOut, @builtin(front_facing) frontFacing_1: bool) -> GBufferOutputX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX {
    ...
    if ((u32(header.x) + 1u) == in_3.materialAddress.y) {
        local_1 = (u32(header.y) == in_3.materialAddress.z);
    } else {
        local_1 = false;
    }
    let _e26 = local_1;
