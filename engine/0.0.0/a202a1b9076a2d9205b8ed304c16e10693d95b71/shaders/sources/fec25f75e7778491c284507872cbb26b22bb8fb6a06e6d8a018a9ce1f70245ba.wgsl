...
fn evaluate_cluster_lightX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(light: DirectLightSlotX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX, world_pos_1: vec3<f32>, normal_12: vec3<f32>, view_dir: vec3<f32>, base_color: vec3<f32>, metallic_9: f32, alpha_sq: f32, f0_1: vec3<f32>, flat_2d: bool, receive_shadows: bool) -> vec3<f32> {
    ...
    if (kind == KIND_POINTX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX) {
        ...
        if _e25 {
            ...
            let _e44 = evalPointShadowedX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(light.position.xyz, light.colorTimesIntensity.xyz, light.position.w, world_pos_1, normal_12, view_dir, base_color, metallic_9, alpha_sq, f0_1, layer_8, shadow_3.x, shadow_3.y, shadow_3.z, shadow_3.w);
            return _e44;
        }
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_13: vec3<f32>, layer_7: u32, count_5: u32, normal_19: vec3<f32>, l_2: vec3<f32>) -> f32 {
    ...
    if (kernel == 3u) {
        if all((uv_4 >= (tileLo + texel_5))) {
            local_36 = all((uv_4 <= (tileHi - texel_5)));
        } else {
            local_36 = false;
        }
...
fn resolveStandardDeferred(in_2: FullscreenOutputX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX) -> DeferredOutput {
    ...
    if (probeRow > 0u) {
        local_38 = ((base_1 + 9u) < arrayLength((&probeBlendRecords)));
    } else {
        local_38 = false;
    }
    let _e82 = local_38;
