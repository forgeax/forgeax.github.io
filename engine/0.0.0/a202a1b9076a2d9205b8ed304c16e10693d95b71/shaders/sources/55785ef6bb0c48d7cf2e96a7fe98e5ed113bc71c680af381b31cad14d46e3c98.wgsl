...
@group(0) @binding(1) 
var sceneNormal: texture_2d<u32>;
@group(0) @binding(2) 
...
fn sampleSsrHiZ(uv_3: vec2<f32>, mip: u32) -> f32 {
    ...
    if (mip == 0u) {
        ...
        let pixel_10 = clamp(vec2<i32>((uv_3 * vec2<f32>(size))), vec2(0i), (vec2<i32>(size) - vec2(1i)));
        let depth_4 = textureLoad(sceneDepth, pixel_10, 0i);
        if !((depth_4 <= 0f)) {
            local_21 = (depth_4 >= 1f);
        } else {
            local_21 = true;
        ...
        let centerUv = ((vec2<f32>(pixel_10) + vec2(0.5f)) / vec2<f32>(size));
        let _e36 = reconstructWorldPosition(centerUv, depth_4);
    ...
    let _e59 = local_22;
    return select(3.402823e38f, depth_5, _e59);
}
...
fn ssrRescueCandidateInRay(origin: vec3<f32>, direction: vec3<f32>, rayLength: f32, surface: vec3<f32>, normal_1: vec3<f32>, fullSize_1: vec2<u32>, pixel_1: vec2<u32>) -> bool {
    var local_25: bool;
    var local_26: bool;
    var local_27: bool;
    var local_28: bool;
    ...
    let denominator = dot(direction, normal_1);
    let planeDistance = (dot((surface - origin), normal_1) / denominator);
    ...
    return all((projectedPixel == pixel_1));
}
                ...
                }
                {
                    ...
                    loop {
                        ...
                        }
                        {
                            let _e92 = childX;
                            let _e93 = childY;
                            let candidate_3 = min(vec2<u32>(_e92, _e93), (nextSize - vec2(1u)));
                            let _e102 = textureLoad(hizPyramid, vec2<i32>(candidate_3), i32(nextPhysicalMip));
...
fn locateSsrLocalCandidate(uv_6: vec2<f32>, pixel_2: vec2<u32>, fullSize_3: vec2<u32>, startUv_2: vec2<f32>, deltaUv_2: vec2<f32>, fallbackFraction_1: f32, endFraction_2: f32) -> SsrDepthCandidate {
    var local_39: bool;
...
fn ssrSourceReactivity(pixel_3: vec2<i32>) -> f32 {
    var local_45: bool;
    var local_46: bool;
    var local_47: bool;

    let temporal_1 = textureLoad(sceneTemporal, pixel_3, 0i);
...
fn ssrShadingNormalVaries(pixel_4: vec2<u32>, normal_2: vec3<f32>) -> bool {
    var axis: u32 = 0u;
    var sign_: i32;
        ...
        }
        {
            ...
            loop {
                ...
                }
                {
                    ...
                    let _e52 = loadStandardNormalRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(sceneNormal, tap);
                    let neighbor = _e52.xyz;
                    let delta = (neighbor - normal_2);
                    let agreement = (dot(neighbor, normal_2) * inverseSqrt(max((dot(neighbor, neighbor) * dot(normal_2, normal_2)), 0.00000001f)));
                    if (agreement > 0.9f) {
...
fn ssrGeometricNormal(pixel_5: vec2<u32>, normal_3: vec3<f32>) -> vec3<f32> {
    ...
    let _e14 = textureLoad(sceneDepth, vec2<i32>(pixel_5), 0i);
    let _e15 = reconstructWorldPosition(((vec2<f32>(pixel_5) + vec2(0.5f)) / vec2<f32>(size_4)), _e14);
    loop {
        let _e17 = axis_1;
        if (_e17 < 2u) {
        } else {
        ...
        }
        {
            ...
            loop {
                ...
                }
                {
                    ...
                    offset_2[_e30] = _e32;
                    let _e34 = offset_2;
                    let tap_1 = (vec2<i32>(pixel_5) + _e34);
                    ...
                    let depth_7 = textureLoad(sceneDepth, tap_1, 0i);
                    let _e52 = loadStandardNormalRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(sceneNormal, tap_1);
                    let tapNormal = _e52.xyz;
                    if !((depth_7 >= 1f)) {
                        local_52 = (dot(normal_3, tapNormal) < 0.9f);
                    } else {
                        local_52 = true;
                    }
                    let _e64 = local_52;
                    ...
                    let _e71 = reconstructWorldPosition(((vec2<f32>(tap_1) + vec2(0.5f)) / vec2<f32>(size_4)), depth_7);
                    let _e73 = sign_1;
                    let delta_1 = ((_e71 - _e15) * f32(_e73));
                    let distance_ = dot(delta_1, delta_1);
                    let _e77 = shortest;
                    if (distance_ < _e77) {
                        shortest = distance_;
                        let _e80 = axis_1;
                        derivatives[_e80] = delta_1;
                    }
                }
                continuing {
                    let _e82 = sign_1;
    ...
    let _e89 = derivatives[0];
    let _e91 = derivatives[1];
    let product = cross(_e89, _e91);
    let lengthSquared = dot(product, product);
    if (lengthSquared <= 0.0000000000000001f) {
        return normal_3;
    }
    let geometric = (product * inverseSqrt(lengthSquared));
    return select(-(geometric), geometric, (dot(geometric, normal_3) >= 0f));
}

fn ssrIntersectSurface(origin_1: vec3<f32>, direction_1: vec3<f32>, maxDistance: f32, fullSize_4: vec2<u32>, pixel_6: vec2<u32>, depth_1: f32, normal_4: vec3<f32>) -> SsrSurfaceIntersection {
    ...
    let centerUv_1 = ((vec2<f32>(pixel_6) + vec2(0.5f)) / vec2<f32>(fullSize_4));
    let _e9 = reconstructWorldPosition(centerUv_1, depth_1);
    let planeDistance_1 = (dot((_e9 - origin_1), normal_4) / dot(direction_1, normal_4));
    let _e17 = isFinite(planeDistance_1);
    if !(!(_e17)) {
    ...
    let _e37 = projectWorldPosition(intersection);
    if !(any((_e37 < vec2(0f)))) {
        local_55 = any((_e37 >= vec2(1f)));
    } else {
        local_55 = true;
    }
    let _e50 = local_55;
    ...
    let hitPixel = min(vec2<u32>((_e37 * vec2<f32>(fullSize_4))), (fullSize_4 - vec2(1u)));
    let hitDepth = textureLoad(sceneDepth, vec2<i32>(hitPixel), 0i);
    let _e65 = loadStandardNormalRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(sceneNormal, vec2<i32>(hitPixel));
    let hitNormal = _e65.xyz;
    if !((hitDepth >= 1f)) {
    ...
    let _e77 = local_56;
    if !(_e77) {
        let _e82 = textureLoad(reflectionFallback, vec2<i32>(hitPixel), 0i);
        local_57 = (_e82.w <= 0.5f);
    } else {
        local_57 = true;
    }
    let _e89 = local_57;
    ...
    return SsrSurfaceIntersection(true, intersection, _e37, hitPixel, hitDepth, hitNormal);
}

fn ssrValidateSurfaceHit(candidate_1: SsrSurfaceIntersection, validationNormal: vec3<f32>, thickness_1: f32, fullSize_5: vec2<u32>, result: SsrTraceHit) -> SsrTraceHit {
    var local_58: bool;

    if !(candidate_1.valid) {
        return result;
    }
    let hitCenterUv = ((vec2<f32>(candidate_1.pixel) + vec2(0.5f)) / vec2<f32>(fullSize_5));
    let _e13 = reconstructWorldPosition(hitCenterUv, candidate_1.depth);
    let _e22 = reconstructWorldPosition((hitCenterUv + vec2<f32>((1f / f32(fullSize_5.x)), 0f)), candidate_1.depth);
    let radius = max(thickness_1, (distance(_e13, _e22) * 3f));
    let separation = abs(dot((_e13 - candidate_1.position), normalize(validationNormal)));
    let _e34 = isFinite(separation);
    if !(!(_e34)) {
    ...
    return SsrTraceHit(1f, candidate_1.uv, clamp((1f - (separation / radius)), 0f, 1f), result.reactivity);
}

fn ssrRefineHit(origin_2: vec3<f32>, direction_2: vec3<f32>, maxDistance_1: f32, thickness_2: f32, fullSize_6: vec2<u32>, pixel_7: vec2<u32>, depth_2: f32, normal_5: vec3<f32>, result_1: SsrTraceHit) -> SsrTraceHit {
    let _e7 = ssrIntersectSurface(origin_2, direction_2, maxDistance_1, fullSize_6, pixel_7, depth_2, normal_5);
    let _e11 = ssrValidateSurfaceHit(_e7, _e7.normal, thickness_2, fullSize_6, result_1);
    return _e11;
}

fn ssrRefineGeometricHit(origin_3: vec3<f32>, direction_3: vec3<f32>, maxDistance_2: f32, thickness_3: f32, fullSize_7: vec2<u32>, pixel_8: vec2<u32>, depth_3: f32, normal_6: vec3<f32>, result_2: SsrTraceHit) -> SsrTraceHit {
    let _e2 = ssrGeometricNormal(pixel_8, normal_6);
    let _e8 = ssrIntersectSurface(origin_3, direction_3, maxDistance_2, fullSize_7, pixel_8, depth_3, _e2);
    if !(_e8.valid) {
        return result_2;
    }
    let _e14 = ssrGeometricNormal(_e8.pixel, _e8.normal);
    let _e16 = ssrValidateSurfaceHit(_e8, _e14, thickness_3, fullSize_7, result_2);
    return _e16;
}

fn traceScreenRay(origin_4: vec3<f32>, direction_4: vec3<f32>, maxDistance_3: f32, thickness_4: f32, fullSize_8: vec2<u32>, hizDepth: f32, hizMaxMip: u32) -> SsrTraceHit {
    var result_3: SsrTraceHit = SsrTraceHit(0f, vec2(0f), 0f, 0f);
    var local_59: bool;
    var local_60: bool;
    var local_61: bool;
    var local_62: bool;
    ...
    var candidate_2: SsrDepthCandidate;
    var local_66: bool;
    var local_67: bool;
    var local_68: bool;
    var local_69: bool;
    ...
    let _e8 = isFinite(maxDistance_3);
    if !(!(_e8)) {
        local_59 = (maxDistance_3 <= 0f);
    } else {
        local_59 = true;
    }
    let _e16 = local_59;
    if !(_e16) {
        let _e19 = isFinite(thickness_4);
        local_60 = !(_e19);
    } else {
        local_60 = true;
    }
    let _e24 = local_60;
    if !(_e24) {
        local_61 = (thickness_4 <= 0f);
    } else {
        local_61 = true;
    }
    let _e31 = local_61;
    if !(_e31) {
        local_62 = (maxDistance_3 < thickness_4);
    } else {
        local_62 = true;
    }
    let _e37 = local_62;
    ...
    let _e56 = projectWorldViewDistance(origin_4);
    let _e61 = view.worldViewProj[0][3];
    let _e66 = view.worldViewProj[1][3];
    let _e72 = view.worldViewProj[2][3];
    let depthDirection = dot(vec3<f32>(_e61, _e66, _e72), direction_4);
    rayLength_1 = maxDistance_3;
    ...
    let end = (origin_4 + (direction_4 * _e93));
    let _e96 = projectWorldViewDistance(end);
    let _e97 = projectWorldPosition(origin_4);
    ...
    let span_1 = abs(((deltaUv_3 * _e140) * vec2<f32>(fullSize_8)));
    ...
    loop {
        ...
        }
        {
            ...
            let pixel_11 = min(vec2<u32>((clamp(uv_8, vec2(0f), vec2(1f)) * vec2<f32>(fullSize_8))), (fullSize_8 - vec2(1u)));
            let rayDepth = (1f / mix(inverseStart, inverseEnd, fraction_1));
            let moving = (abs(deltaUv_3) > vec2(0.00000001f));
            let exitUv = ((vec2<f32>(pixel_11) + select(vec2(0f), vec2(1f), (deltaUv_3 > vec2(0f)))) / vec2<f32>(fullSize_8));
            let _e219 = endFraction_3;
            let exitFractions = select(vec2(_e219), ((exitUv - _e97) / select(vec2(1f), deltaUv_3, moving)), moving);
            let pixelSpan = max((abs(deltaUv_3.x) * f32(fullSize_8.x)), (abs(deltaUv_3.y) * f32(fullSize_8.y)));
            ...
            if (coarseMip_1 > 0u) {
                ...
                local_65 = (_e255 > (intervalDepth + thickness_4));
            } else {
                local_65 = false;
            }
            let _e261 = local_65;
            ...
            samplePixel = pixel_11;
            sampleUv = uv_8;
            let _e267 = textureLoad(sceneDepth, vec2<i32>(pixel_11), 0i);
            sampleDepth = _e267;
            ...
            if _e282 {
                ...
                let _e284 = locateSsrHiZCandidate(uv_8, coarseMip_1, fullSize_8, _e97, deltaUv_3, fraction_1, _e283);
                candidate_2 = _e284;
                let projectedStepPixels = (max(span_1.x, span_1.y) / f32(max(count, 1u)));
                let _e293 = candidate_2.valid;
                if !(_e293) {
                ...
                if _e306 {
                    ...
                    let _e308 = locateSsrLocalCandidate(uv_8, pixel_11, fullSize_8, _e97, deltaUv_3, fraction_1, _e307);
                    candidate_2 = _e308;
                }
                let _e310 = candidate_2.valid;
                if _e310 {
                    let _e312 = candidate_2.pixel;
                    samplePixel = _e312;
                    let _e314 = candidate_2.uv;
                    sampleUv = _e314;
                    let _e316 = candidate_2.depth;
                    sampleDepth = _e316;
                    let _e318 = candidate_2.fraction;
            ...
            if ((intervalDepth + thickness_4) < _e343) {
                continue;
            }
            let _e347 = result_3.reactivity;
            let _e348 = samplePixel;
            let _e350 = ssrSourceReactivity(vec2<i32>(_e348));
            result_3.reactivity = max(_e347, _e350);
            let _e352 = samplePixel;
            let _e355 = loadStandardNormalRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(sceneNormal, vec2<i32>(_e352));
            let normal_7 = _e355.xyz;
            if (dot(normal_7, -(direction_4)) <= 0f) {
                continue;
            }
            let _e361 = rayLength_1;
            let _e362 = samplePixel;
            let _e363 = sampleDepth;
            let _e364 = result_3;
            let _e365 = ssrRefineHit(origin_4, direction_4, _e361, thickness_4, fullSize_8, _e362, _e363, normal_7, _e364);
            if (_e365.hit > 0f) {
                let _e370 = sampleFraction;
                let _e371 = endFraction_3;
                let _e372 = ssrRayFraction(_e365.uv, _e97, deltaUv_3, _e370, _e371);
                let _e373 = step_;
                let _e379 = endFraction_3;
                if (_e372 >= max(0f, ((f32((_e373 - 1u)) / f32(count)) * _e379))) {
                    let _e384 = endFraction_3;
                    let _e385 = step_;
                    let _e391 = endFraction_3;
                    local_70 = (_e372 <= min(_e384, ((f32((_e385 + 1u)) / f32(count)) * _e391)));
                } else {
                    local_70 = false;
                }
                let _e398 = local_70;
            ...
            let _e400 = candidate_2.valid;
            if _e400 {
                let _e401 = rayLength_1;
                let _e402 = samplePixel;
                let _e403 = ssrRescueCandidateInRay(origin_4, direction_4, _e401, _e341, normal_7, fullSize_8, _e402);
                local_71 = !(_e403);
            } else {
                local_71 = false;
            }
            ...
            let _e409 = projectWorldViewDistance(_e341);
            if (sampleRayDepth < _e409) {
                let _e412 = candidate_2.valid;
                if !(_e412) {
                    let _e414 = rayLength_1;
                    let _e415 = samplePixel;
                    let _e416 = ssrRescueCandidateInRay(origin_4, direction_4, _e414, _e341, normal_7, fullSize_8, _e415);
                    local_72 = !(_e416);
                } else {
                    local_72 = false;
                }
            ...
            let _e422 = sampleUv;
            let _e430 = sampleDepth;
            let _e431 = reconstructWorldPosition((_e422 + vec2<f32>((1f / f32(fullSize_8.x)), 0f)), _e430);
            let radius_1 = max(thickness_4, (distance(_e341, _e431) * 3f));
            let separation_1 = length(cross((_e341 - origin_4), direction_4));
            let _e439 = isFinite(separation_1);
            if !(!(_e439)) {
                local_73 = (separation_1 > radius_1);
            } else {
                local_73 = true;
            }
            let _e446 = local_73;
            ...
            let _e460 = step_;
            let _e466 = endFraction_3;
            let _e468 = sampleFraction;
            let bracketLower = min(((f32((_e460 - 1u)) / f32(count)) * _e466), _e468);
            let _e470 = sampleFraction;
            let bracketUpper = max(exitFraction, _e470);
            let _e472 = sampleFraction;
            let _e473 = projectWorldViewDistance(_e341);
            lower = select(_e472, bracketLower, (sampleRayDepth >= _e473));
            let _e477 = sampleFraction;
            let _e478 = projectWorldViewDistance(_e341);
            upper = select(bracketUpper, _e477, (sampleRayDepth >= _e478));
            found = true;
            break;
        }
        continuing {
    ...
    loop {
        let _e491 = refine;
        if (_e491 < SSR_TRACE_MAX_REFINE_STEPS) {
        } else {
            break;
        }
        {
            let _e494 = lower;
            let _e495 = upper;
            let middle = ((_e494 + _e495) * 0.5f);
            let uv_9 = (_e97 + (deltaUv_3 * middle));
            let pixel_12 = min(vec2<u32>((clamp(uv_9, vec2(0f), vec2(1f)) * vec2<f32>(fullSize_8))), (fullSize_8 - vec2(1u)));
            let _e514 = sampleSsrHiZ(uv_9, 0u);
            let rayDepth_1 = (1f / mix(inverseStart, inverseEnd, middle));
            let _e520 = loadStandardNormalRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(sceneNormal, vec2<i32>(pixel_12));
            let normal_8 = _e520.xyz;
            let _e525 = textureLoad(reflectionFallback, vec2<i32>(pixel_12), 0i);
            let covered = (_e525.w > 0.5f);
            if (rayDepth_1 >= _e514) {
                local_74 = covered;
            } else {
                local_74 = false;
            ...
            let _e533 = local_74;
            if _e533 {
                local_75 = (dot(normal_8, -(direction_4)) > 0f);
            } else {
                local_75 = false;
            }
            let _e541 = local_75;
    ...
    let _e545 = upper;
    let uv_10 = (_e97 + (deltaUv_3 * _e545));
    let pixel_13 = min(vec2<u32>((clamp(uv_10, vec2(0f), vec2(1f)) * vec2<f32>(fullSize_8))), (fullSize_8 - vec2(1u)));
    let depth_8 = textureLoad(sceneDepth, vec2<i32>(pixel_13), 0i);
    let _e566 = loadStandardNormalRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(sceneNormal, vec2<i32>(pixel_13));
    let normal_9 = _e566.xyz;
    let _e571 = textureLoad(reflectionFallback, vec2<i32>(pixel_13), 0i);
    let covered_1 = (_e571.w > 0.5f);
    let _e575 = rescued;
    if _e575 {
        if !((depth_8 >= 1f)) {
            local_77 = (dot(normal_9, -(direction_4)) <= 0f);
        } else {
            local_77 = true;
        }
        let _e586 = local_77;
    ...
    if _e596 {
        let _e597 = rescuedUv;
        let _e598 = rescuedThickness;
        let _e600 = result_3.reactivity;
        return SsrTraceHit(1f, _e597, _e598, _e600);
    }
    if !((depth_8 >= 1f)) {
        local_79 = (dot(normal_9, -(direction_4)) <= 0f);
    } else {
        local_79 = true;
    }
    let _e613 = local_79;
    ...
    let _e621 = result_3;
    let _e622 = ssrRefineHit(origin_4, direction_4, maxDistance_3, thickness_4, fullSize_8, pixel_13, depth_8, normal_9, _e621);
    if !((_e622.hit > 0f)) {
        let _e627 = ssrShadingNormalVaries(pixel_13, normal_9);
        local_81 = !(_e627);
    } else {
        local_81 = true;
    }
    ...
    if _e632 {
        ...
        if _e640 {
            let _e641 = rescuedUv;
            let _e642 = rescuedThickness;
            let _e644 = result_3.reactivity;
            return SsrTraceHit(1f, _e641, _e642, _e644);
        }
        return _e622;
    }
    let _e647 = rescued;
    if _e647 {
        let _e648 = rescuedUv;
        let _e649 = rescuedThickness;
        let _e651 = result_3.reactivity;
        return SsrTraceHit(1f, _e648, _e649, _e651);
    }
    let _e654 = result_3;
    let _e655 = ssrRefineGeometricHit(origin_4, direction_4, maxDistance_3, thickness_4, fullSize_8, pixel_13, depth_8, normal_9, _e654);
    return _e655;
}

fn ssrHitTapAdmitted(pixel_9: vec2<i32>, rayDirection: vec3<f32>) -> bool {
    var local_83: bool;

    let _e2 = loadStandardNormalRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(sceneNormal, pixel_9);
    let normal_10 = _e2.xyz;
    let _e6 = textureLoad(reflectionFallback, pixel_9, 0i);
    if (_e6.w > 0.5f) {
        local_83 = (dot(normal_10, -(rayDirection)) > 0f);
    } else {
        local_83 = false;
    }
    let _e18 = local_83;
        ...
        }
        {
            ...
            loop {
                ...
                }
                {
                    let _e28 = x;
                    let _e29 = y;
                    let pixel_14 = clamp((first + vec2<i32>(_e28, _e29)), vec2(0i), last);
                    let _e36 = ssrHitTapAdmitted(pixel_14, rayDirection_1);
                    if !(_e36) {
                        continue;
                    }
                    let _e42 = x;
                    ...
                    let _e57 = textureLoad(sceneColor, pixel_14, 0i);
                    let color = _e57.xyz;
                    let _e60 = sum;
                    sum = (_e60 + vec4<f32>((color * weight), weight));
                    if (weight > 0f) {
                        let _e67 = reactivity;
                        let _e68 = ssrSourceReactivity(pixel_14);
...
fn ssr_trace(@builtin(global_invocation_id) globalId: vec3<u32>) {
    ...
    let fullSize_9 = textureDimensions(sceneColor, 0i);
    let traceSize = max((fullSize_9 / vec2(2u)), vec2(1u));
    ...
    let fullPixel = min((globalId.xy * vec2(2u)), (fullSize_9 - vec2(1u)));
    let _e35 = textureNumLevels(hizPyramid);
    let physicalMaxMip = (_e35 - 1u);
    let _e38 = maxSsrHiZMip(fullSize_9);
    ...
    let depth_9 = textureLoad(sceneDepth, vec2<i32>(fullPixel), 0i);
    let _e55 = loadStandardNormalRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(sceneNormal, vec2<i32>(fullPixel));
    let normalUnnormalized = _e55.xyz;
    let normalLength = length(normalUnnormalized);
    let _e58 = isFinite(depth_9);
    if !(!(_e58)) {
        local_1 = (depth_9 >= 1f);
    } else {
        local_1 = true;
    ...
    let _e66 = local_1;
    if !(_e66) {
        let _e68 = isFinite(normalLength);
        local_2 = !(_e68);
    } else {
        local_2 = true;
    }
    ...
    let _e80 = local_3;
    if !(_e80) {
        let _e85 = textureLoad(reflectionFallback, vec2<i32>(fullPixel), 0i);
        local_4 = (_e85.w <= 0.5f);
    } else {
        local_4 = true;
    }
    let _e92 = local_4;
    if !(_e92) {
        let _e97 = view.ssrParams.w;
        let _e98 = isFinite(_e97);
        local_5 = !(_e98);
    } else {
        local_5 = true;
    }
    ...
    let _e122 = local_7;
    if !(_e122) {
        let _e128 = view.ssrParams.z;
        local_8 = (_e55.w >= _e128);
    } else {
        local_8 = true;
    }
    let _e133 = local_8;
    ...
    let _e145 = isFinite(normalLength);
    if _e145 {
    ...
    let _e151 = local_9;
    let normal_11 = select(vec3<f32>(0f, 0f, 1f), (normalUnnormalized / vec3(normalLength)), _e151);
    let _e156 = textureLoad(hizPyramid, vec2<i32>(hizPixel), i32(physicalMaxMip));
    let hizDepth_1 = _e156.x;
    let uv_11 = ((vec2<f32>(fullPixel) + vec2(0.5f)) / vec2<f32>(fullSize_9));
    let _e167 = view.ssrParams.w;
    let _e168 = isFinite(_e167);
    if _e168 {
        let _e172 = view.ssrParams.w;
        local_10 = (_e172 > 0.5f);
    } else {
        local_10 = false;
    }
    let _e178 = local_10;
    let enabled = select(0f, 1f, _e178);
    let _e185 = view.ssrParams.x;
    let maxDistance_4 = (enabled * _e185);
    let _e190 = view.ssrParams.y;
    let thickness_5 = (enabled * _e190);
    let _e195 = view.ssrParams.z;
    let roughnessLimit = (enabled * clamp(_e195, 0f, 1f));
    let _e200 = reconstructWorldPosition(uv_11, depth_9);
    let _e203 = view.cameraPos;
    let viewDirectionRaw = (_e203 - _e200);
    let viewDirectionLength = length(viewDirectionRaw);
    let _e212 = isFinite(viewDirectionLength);
    if _e212 {
    ...
    let _e218 = local_11;
    let viewDirection = select(vec3<f32>(0f, 0f, 1f), (viewDirectionRaw / vec3(viewDirectionLength)), _e218);
    let facing_1 = clamp((dot(normal_11, viewDirection) * 8f), 0f, 1f);
    let reflectionDirection = normalize(reflect(-(viewDirection), normal_11));
    let _e229 = traceScreenRay(_e200, reflectionDirection, maxDistance_4, thickness_5, fullSize_9, hizDepth_1, hizMaxMip_1);
    let _e231 = traceEdge(_e229.uv);
    textureStore(hitReactivityOutput, vec2<i32>(globalId.xy), vec4(_e229.reactivity));
    let _e252 = finiteConfidence(select(0f, (1f - smoothstep((roughnessLimit * 0.8f), max(roughnessLimit, 0.00001f), clamp(_e55.w, 0f, 1f))), (roughnessLimit > 0f)));
    let _e256 = traceConfidence(_e229.hit, _e229.thickness, facing_1, _e231, _e252, 1f);
    if (_e256 <= 0f) {
        textureStore(traceOutput, vec2<i32>(globalId.xy), vec4(0f));
        return;
    }
    let _e265 = sampleSsrHitSample(_e229.uv, reflectionDirection);
    let hitColor = _e265.color;
    let _e268 = isFinite(hitColor.x);
    let _e270 = isFinite(hitColor.y);
    let _e272 = isFinite(hitColor.z);
    if !(all(vec3<bool>(_e268, _e270, _e272))) {
        textureStore(traceOutput, vec2<i32>(globalId.xy), vec4(0f));
        return;
    }
    textureStore(traceOutput, vec2<i32>(globalId.xy), vec4<f32>(hitColor.xyz, (_e256 * hitColor.w)));
    textureStore(hitReactivityOutput, vec2<i32>(globalId.xy), vec4(max(_e229.reactivity, _e265.reactivity)));
    return;
}
