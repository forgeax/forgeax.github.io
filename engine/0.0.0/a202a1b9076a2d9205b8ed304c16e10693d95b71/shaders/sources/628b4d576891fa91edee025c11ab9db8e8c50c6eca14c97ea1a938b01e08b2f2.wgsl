...
fn sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(normal_3: vec3<f32>, view_2: vec3<f32>, roughness_3: f32, F0_3: vec3<f32>, worldPosition_1: vec3<f32>, boxCenter_1: vec3<f32>, boxExtents_1: vec3<f32>, rotation_3: vec4<f32>, probeMap: texture_cube<f32>, probeSampler: sampler, brdfLut_2: texture_2d<f32>, brdfLutSampler_2: sampler, skylightMap: texture_cube<f32>, skylightRotation: vec4<f32>, skylightScale: vec3<f32>, probeIntensity: f32, boxProjection: bool) -> vec3<f32> {
    ...
    let local_36 = (_e55 * probeIntensity);
    let _e58 = sky;
    return mix(_e58, local_36, weight);
}
...
fn evaluateProbeDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX(shPreblend_1: array<vec4<f32>, 9>, localBlendFraction: f32, normal_5: vec3<f32>, e_sky: vec3<f32>, k_d: vec3<f32>, albedo: vec3<f32>, metallic: f32) -> vec3<f32> {
    ...
    let local_37 = (max(_e2, vec3(0f)) * PROBE_INV_PIX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX);
    let skyResidualFraction = (1f - localBlendFraction);
    return ((((local_37 + (skyResidualFraction * e_sky)) * k_d) * albedo) * (1f - metallic));
}
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_15: vec3<f32>, layer_5: u32, count_4: u32, normal_17: vec3<f32>, l_1: vec3<f32>) -> f32 {
    ...
    if (tileUv.x >= 0f) {
        local_28 = (tileUv.x <= 1f);
    } else {
        local_28 = false;
    }
    let _e40 = local_28;
    if _e40 {
        local_29 = (tileUv.y >= 0f);
    } else {
        local_29 = false;
    }
    let _e47 = local_29;
    if _e47 {
        local_30 = (tileUv.y <= 1f);
    } else {
        local_30 = false;
    }
    let _e54 = local_30;
    if _e54 {
        local_31 = (currentDepth <= 1f);
    } else {
        local_31 = false;
    }
    let _e60 = local_31;
    ...
    if (kernel == 3u) {
        if all((uv_6 >= (tileLo + texel_2))) {
            local_32 = all((uv_6 <= (tileHi - texel_2)));
        } else {
            local_32 = false;
        }
        let interior = local_32;
        if interior {
...
fn projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX(baseColorAlpha_1: f32, alphaCutoff: f32, baseColorTextureEnabled: bool, baseColorTexture: texture_2d<f32>, baseColorSampler: sampler, transform_1: vec4<f32>, metadata_3: vec4<f32>, currentClip_1: vec4<f32>, previousClip_1: vec4<f32>, temporalProjection_3: vec4<f32>, reactive_3: f32, motionValid_2: bool, uv0_1: vec2<f32>, uv1_1: vec2<f32>, uv2_1: vec2<f32>, uv3_1: vec2<f32>, uv4_1: vec2<f32>, uv5_1: vec2<f32>, uv6_1: vec2<f32>, uv7_1: vec2<f32>) -> vec4<f32> {
    ...
    if (alphaCutoff > 0f) {
        ...
        local_34 = ((baseColorAlpha_1 * _e24) <= alphaCutoff);
    } else {
        local_34 = false;
    }
    let _e30 = local_34;
...
fn alphaTestSurface(surface: SurfaceData) {
    ...
    if (surface.alphaClipThreshold > 0f) {
        local_35 = (surface.opacity <= surface.alphaClipThreshold);
    } else {
        local_35 = false;
    }
    let _e10 = local_35;
