...
fn sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(normal_3: vec3<f32>, view_2: vec3<f32>, roughness_3: f32, F0_3: vec3<f32>, worldPosition_1: vec3<f32>, boxCenter_1: vec3<f32>, boxExtents_1: vec3<f32>, rotation_3: vec4<f32>, probeMap: texture_cube<f32>, probeSampler: sampler, brdfLut_2: texture_2d<f32>, brdfLutSampler_2: sampler, skylightMap: texture_cube<f32>, skylightRotation: vec4<f32>, skylightScale: vec3<f32>, probeIntensity: f32, boxProjection: bool) -> vec3<f32> {
    ...
    let local_48 = (_e55 * probeIntensity);
    let _e58 = sky;
    return mix(_e58, local_48, weight);
}
...
fn evaluateProbeDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX(shPreblend_1: array<vec4<f32>, 9>, localBlendFraction: f32, normal_5: vec3<f32>, e_sky: vec3<f32>, k_d: vec3<f32>, albedo: vec3<f32>, metallic: f32) -> vec3<f32> {
    ...
    let local_49 = (max(_e2, vec3(0f)) * PROBE_INV_PIX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX);
    let skyResidualFraction = (1f - localBlendFraction);
    return ((((local_49 + (skyResidualFraction * e_sky)) * k_d) * albedo) * (1f - metallic));
}
...
fn evaluate_cluster_lightX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(light: DirectLightSlotX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX, world_pos_1: vec3<f32>, normal_12: vec3<f32>, view_dir: vec3<f32>, base_color: vec3<f32>, metallic_10: f32, alpha_sq: f32, f0_2: vec3<f32>, flat_2d: bool, receive_shadows: bool) -> vec3<f32> {
    ...
    if (kind == KIND_SPOTX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX) {
        ...
        tile = encoded_tile;
        if (encoded_tile >= 0i) {
            local_25 = ((encoded_tile & PROJECTOR_FLAGX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX) != 0i);
        } else {
            local_25 = false;
        }
        let _e94 = local_25;
...
fn _samplePcssForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_15: vec3<f32>, layer_6: u32, count_4: u32, normal_17: vec3<f32>, l_1: vec3<f32>, profile: u32) -> f32 {
    ...
    if (tileUv.x >= 0f) {
        local_29 = (tileUv.x <= 1f);
    } else {
        local_29 = false;
    }
    let _e46 = local_29;
    if _e46 {
        local_30 = (tileUv.y >= 0f);
    } else {
        local_30 = false;
    }
    let _e53 = local_30;
    if _e53 {
        local_31 = (tileUv.y <= 1f);
    } else {
        local_31 = false;
    }
    let _e60 = local_31;
    if _e60 {
        local_32 = (projCoords.z >= 0f);
    } else {
        local_32 = false;
    }
    let _e67 = local_32;
    if _e67 {
        local_33 = (projCoords.z <= 1f);
    } else {
        local_33 = false;
    }
    let _e74 = local_33;
    ...
    if (lightDepthWorldSpan > 0f) {
        local_37 = (worldUnitsPerTexel > 0f);
    } else {
        local_37 = false;
    }
    let _e213 = local_37;
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_16: vec3<f32>, layer_7: u32, count_5: u32, normal_18: vec3<f32>, l_2: vec3<f32>) -> f32 {
    ...
    if (tileUv_1.x >= 0f) {
        local_40 = (tileUv_1.x <= 1f);
    } else {
        local_40 = false;
    }
    let _e40 = local_40;
    if _e40 {
        local_41 = (tileUv_1.y >= 0f);
    } else {
        local_41 = false;
    }
    let _e47 = local_41;
    if _e47 {
        local_42 = (tileUv_1.y <= 1f);
    } else {
        local_42 = false;
    }
    let _e54 = local_42;
    if _e54 {
        local_43 = (currentDepth <= 1f);
    } else {
        local_43 = false;
    }
    let _e60 = local_43;
    ...
    if (kernel == 3u) {
        if all((uv_7 >= (tileLo + texel_5))) {
            local_44 = all((uv_7 <= (tileHi - texel_5)));
        } else {
            local_44 = false;
        }
        let interior = local_44;
        if interior {
...
fn projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX(baseColorAlpha_1: f32, alphaCutoff: f32, baseColorTextureEnabled: bool, baseColorTexture: texture_2d<f32>, baseColorSampler: sampler, transform_1: vec4<f32>, metadata_3: vec4<f32>, currentClip_1: vec4<f32>, previousClip_1: vec4<f32>, temporalProjection_3: vec4<f32>, reactive_3: f32, motionValid_2: bool, uv0_1: vec2<f32>, uv1_1: vec2<f32>, uv2_1: vec2<f32>, uv3_1: vec2<f32>, uv4_1: vec2<f32>, uv5_1: vec2<f32>, uv6_1: vec2<f32>, uv7_1: vec2<f32>) -> vec4<f32> {
    ...
    if (alphaCutoff > 0f) {
        ...
        local_46 = ((baseColorAlpha_1 * _e24) <= alphaCutoff);
    } else {
        local_46 = false;
    }
    let _e30 = local_46;
...
fn vs_main_impl(in_5: VsIn, meshIndex: u32, instanceIndex: u32, materialAddress: vec3<u32>) -> VsOut {
    var out_1: VsOut;
...
fn alphaTestSurface(surface: SurfaceData) {
    ...
    if (surface.alphaClipThreshold > 0f) {
        local_47 = (surface.opacity <= surface.alphaClipThreshold);
    } else {
        local_47 = false;
    }
    let _e10 = local_47;
...
fn fs_main(in_1: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let _e135 = probeBlendRecords[(0u + 1u)];
    let _e140 = probeBlendRecords[(0u + 2u)];
    let _e145 = probeBlendRecords[(0u + 3u)];
    let _e150 = probeBlendRecords[(0u + 4u)];
    let _e155 = probeBlendRecords[(0u + 5u)];
    let _e160 = probeBlendRecords[(0u + 6u)];
    let _e165 = probeBlendRecords[(0u + 7u)];
    let _e170 = probeBlendRecords[(0u + 8u)];
    let _e175 = probeBlendRecords[(0u + 9u)];
    probeShPreblend = array<vec4<f32>, 9>(_e135, _e140, _e145, _e150, _e155, _e160, _e165, _e170, _e175);
    let _e181 = probeBlendRecords[0u].z;
    probeLocalBlendFraction = select(0f, _e181, true);
    let _e187 = physicalNormal;
    let _e188 = diffuseAlbedo;
    let _e189 = f0_;
    let _e191 = skylight;
    let _e192 = probeShPreblend;
    let _e193 = probeLocalBlendFraction;
    let _e200 = evaluateStandardEnvironmentX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_1.worldPos, _e187, v_5, _e188, metallic_17, iblRoughness, _e189, _e191, irradianceMap_2, irradianceSampler_2, prefilterMap_2, prefilterSampler_2, brdfLut_4, skylightPrefilterMap_1, _e192, _e193);
    let ao = _e6.occlusion;
    let _e204 = skylight.colorR;
    let _e207 = skylight.colorG;
    let _e210 = skylight.colorB;
    let _e214 = skylight.intensity;
    let _e215 = decodeSpecularEnvironmentScaleX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(vec3<f32>(_e204, _e207, _e210), _e214);
    let _e222 = coatF;
    ambient = (((_e200.diffuse + _e200.specular) * (vec3(1f) - _e222)) * ao);
    let _e230 = coatF;
    reflectionFallback = ((_e200.specular * (vec3(1f) - _e230)) * ao);
    let _e238 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.worldViewProj;
    let _e241 = sampleStandardAmbientOcclusionX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_1.worldPos, _e238, ssaoBlurredTexture, ssaoBlurredSampler);
    let _e242 = ambient;
    ambient = (_e242 * _e241);
    let _e244 = reflectionFallback;
    reflectionFallback = (_e244 * _e241);
    let _e246 = ambient;
    color = _e246;
    ...
    let _e319 = material.thickness;
    let _e321 = finiteScalar(_e319, 0f);
    let _e324 = thicknessSample;
    let _e326 = finiteScalar(_e324, 1f);
    let worldThickness = max(((_e321 * length(worldRefractedDirection)) * _e326), 0f);
    ...
    let _e368 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(screenUv, vec2(0f), vec2(1f)), backdropLod);
    let unrefractedBackdrop = _e368.xyz;
    ...
    let _e376 = local_1;
    if _e376 {
        let _e384 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(refractedUv, vec2(0f), vec2(1f)), backdropLod);
        transmittedBackdrop = _e384.xyz;
    }
    let _e388 = material.attenuationColor;
    let _e391 = finiteColor(_e388, vec3(1f));
    let safeAttenuationColor = clamp(_e391, vec3(0f), vec3(1f));
    let _e399 = material.attenuationDistance;
    let _e401 = finiteScalar(_e399, 0f);
    let safeAttenuationDistance = max(_e401, 0f);
    ...
    let _e418 = local_2;
    let beerAttenuation = select(vec3(1f), pow(safeAttenuationColor, vec3(attenuationExponent)), _e418);
    let _e420 = color;
    let _e421 = transmittedBackdrop;
    let _e424 = finiteColor(_e421, vec3(0f));
    color = (_e420 + ((_e424 * transmittedEnergy) * beerAttenuation));
    let _e428 = physicalNormal;
    let _e430 = standardViewZ(in_1);
    let _e431 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e428, in_1.worldPos, _e430);
    let _e432 = color;
    let _e436 = standardViewZ(in_1);
    let _e437 = physicalNormal;
    let _e438 = diffuseAlbedo;
    let _e439 = f0_;
    let _e440 = evaluateStandardDirectX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_1.worldPos, in_1.ndc.xyz, _e436, _e437, v_5, _e438, metallic_17, a_4, _e439, _e431);
    color = (_e432 + _e440);
    let _e442 = color;
    color = (_e442 + _e6.emissive);
    let _e447 = color;
    output.color = vec4<f32>(_e447, alpha_3);
    let _e450 = reflectionFallback;
    let _e451 = standardSsrCoverage();
    output.reflectionFallback = vec4<f32>(_e450, _e451);
    let _e453 = output;
    return _e453;
}
...
fn fs_gbuffer(in_2: VsOut, @builtin(front_facing) frontFacing_1: bool) -> GBufferOutputX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX {
    ...
    if (header.z > 0f) {
        probeRow = (u32(header.x) + 1u);
    }
    let _e25 = standardSurfaceF0_(in_2, _e3);
    let _e32 = skylight.diffuseScale.w;
    let _e34 = probeRow;
    let _e35 = encodeStandardGBufferX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(_e3.normalWS, clamp(_e3.roughness, 0.04f, 1f), _e3.baseColor, clamp(_e3.metallic, 0f, 1f), _e25, _e3.occlusion, _e3.emissive, _e3.opacity, u32(_e32), _e34);
    return _e35;
}
...
fn fs_temporal(in_4: TemporalVsOut) -> @location(0) vec4<f32> {
    ...
    let _e21 = temporalVertexAlpha(in_4);
    ...
    let _e50 = projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX((_e19 * _e21), _e25, _e26, baseColorTexture_1, baseColorTexture_sampler, _e29, _e32, in_4.currentClip, in_4.previousClip, _e37, _e38, _e39, in_4.uv, in_4.uv1_, in_4.uv2_, in_4.uv3_, in_4.uv4_, in_4.uv5_, in_4.uv6_, in_4.uv7_);
    return vec4(1f);
}
