...
// src/preview/capture.ts
function notReady(unmet) {
...
function captureFailure(stage, message) {
...
async function screenshotBytes(screenshot) {
  try {
    ...
    if (typeof value === "object" && value !== null && "ok" in value) {
      ...
      return value.value instanceof Uint8Array ? { ok: true, value: value.value } : captureFailure("screenshot", "screenshot result was not a Uint8Array");
    }
...
function collectEvidence(instance, session, adapter, clock, readiness) {
  const shadow = instance.host.shadowRoot;
  const parts = shadow ? [...shadow.querySelectorAll("[data-ui-part]")].map((part) => part.dataset.uiPart).filter((part) => Boolean(part)) : [];
  ...
  const focused = shadow?.activeElement !== null && shadow?.activeElement !== void 0;
  return {
...
async function captureUiPreview(session, adapter) {
...
// src/preview/scenario.ts
function createDomPartScenario(options) {
  return {
...
export { captureUiPreview, createDomPartScenario, createUiPreviewSession };
