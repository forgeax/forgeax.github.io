...
// src/clock.ts
function createProfileClock(readMicros = defaultReadMicros) {
  let lastMicros = 0;
  return {
    nowMicros() {
...
function validateSemanticRules(value) {
  ...
  for (const [index, record] of records.entries()) {
    ...
    if (record.kind === "phase" && record.parentPhase !== void 0) {
    ...
    } else if (record.kind === "phase" && record.parentSource !== void 0) {
      return error(
        "profile-artifact-invalid",
  ...
  if (completeness.status === "complete") {
    if (completeness.incompleteReason !== void 0 || completeness.droppedEventCount !== 0) {
      return error(
        "profile-artifact-invalid",
        "/completeness",
  ...
  if (completeness.status === "partial" && completeness.incompleteReason === void 0) {
    return error(
      "profile-artifact-invalid",
  ...
  if (completeness.status === "overflow") {
    ...
    if (completeness.firstAffectedFrameId === void 0 || completeness.lastAffectedFrameId === void 0) {
      return error(
        "profile-artifact-invalid",
        "/completeness",
...
function validateProfileCapture(value) {
...
function isRootPhase(record) {
  return record.kind === "phase" && record.parentPhase === void 0;
}
function buildFrames(records) {
  const frameMap = /* @__PURE__ */ new Map();
  for (const record of records) {
    const frameRecords = frameMap.get(record.frameId);
    if (frameRecords === void 0) frameMap.set(record.frameId, [cloneRecord(record)]);
...
function buildPhases(records) {
  ...
  for (const record of records) {
    const parentSource = record.kind === "phase" ? record.parentSource : void 0;
    const parentPhase = record.kind === "phase" ? record.parentPhase : void 0;
  ...
  return [...phaseMap.values()].map(
    ({ source, phase, parentSource, parentPhase, durations, skipCount }) => ({
      source,
      phase,
      ...parentSource === void 0 ? {} : { parentSource },
      ...parentPhase === void 0 ? {} : { parentPhase },
...
function buildProfileModel(value) {
  ...
  const frameRange = frames.length === 0 ? null : { first: frames[0]?.frameId, last: frames[frames.length - 1]?.frameId };
  return {
    ok: true,
    value: {
      summary: {
...
// src/compare.ts
function withSide(side, error2) {
  return {
    ...error2,
    detail: { ...error2.detail, side }
  };
}
...
function phaseIdentity(phase) {
  return {
    source: phase.source,
    phase: phase.phase,
    ...phase.parentSource === void 0 ? {} : { parentSource: phase.parentSource },
    ...phase.parentPhase === void 0 ? {} : { parentPhase: phase.parentPhase }
  };
}
function phaseFact(phase) {
  return {
    count: phase.count,
...
function phaseDelta(left, right, comparableTimeUnit) {
...
function projectPhases(left, right) {
  ...
  const keys = [.../* @__PURE__ */ new Set([...leftByKey.keys(), ...rightByKey.keys()])].sort();
  ...
  return keys.map((key) => {
    ...
    const leftFact = leftPhase === void 0 ? void 0 : phaseFact(leftPhase);
    const rightFact = rightPhase === void 0 ? void 0 : phaseFact(rightPhase);
    const delta = phaseDelta(leftFact, rightFact, comparableTimeUnit);
    return {
      identity: phaseIdentity(leftPhase ?? rightPhase),
      ...leftFact === void 0 ? {} : { left: leftFact },
      ...rightFact === void 0 ? {} : { right: rightFact },
      ...delta === void 0 ? {} : { delta }
    };
  });
}
function projectModels(left, right) {
  return {
    left: {
...
function compareProfileCaptures(left, right) {
...
// src/errors.ts
function boundaryError(frameLimit, eventLimit) {
  return {
...
// src/recorder.ts
var PHASE_RECORD_KIND = 1;
var SKIP_RECORD_KIND = 2;
var INITIAL_RECORD_CAPACITY = 65536;
var OK_VOID = Object.freeze({ ok: true, value: void 0 });
function positiveSafeInteger(value) {
  return typeof value === "number" && Number.isSafeInteger(value) && value > 0;
}
function validateRecorderLimits(limits) {
...
function sourceError(source, phase, frameId) {
  return {
    ok: false,
    error: {
...
function requireRetained(value) {
  if (value === void 0) throw new Error("profiler record storage is incomplete");
  return value;
}
function reserveRecord(state, frameId) {
...
function retainPhaseRecord(state, source, frameId, phase, parentSource, parentPhase, startMicros, endMicros) {
...
function retainSkipRecord(state, source, frameId, phase, reason) {
  const index = reserveRecord(state, frameId);
  if (index === void 0) return;
  const records = state.recordStore;
...
function materializeRecords(state) {
  const retained = state.recordStore;
  const records = new Array(retained.count);
  for (let index = 0; index < retained.count; index += 1) {
    ...
    if (kind === PHASE_RECORD_KIND) {
      ...
      records[index] = parentSource === void 0 ? {
        kind: "phase",
        source,
        frameId,
        phase,
    ...
    if (state.allocationReport !== void 0) {
      state.allocationReport.profilerEventObjectAllocations += 1;
    }
  }
  return records;
}
function buildCapture(state) {
  const status = state.overflow ? "overflow" : state.frameCount < state.limits.frameLimit || state.currentFrameId !== void 0 ? "partial" : "complete";
  const completeness = {
    status,
    ...
    ...state.firstAffectedFrameId !== void 0 ? { firstAffectedFrameId: state.firstAffectedFrameId } : {},
    ...state.lastAffectedFrameId !== void 0 ? { lastAffectedFrameId: state.lastAffectedFrameId } : {}
  };
  return {
    schemaVersion: "1.0",
...
function createRecorder(captureId, limits, clock, phaseCatalog, allocationReport) {
  ...
  const state = {
    ...
    openDepth: 0,
    droppedEventCount: 0,
    firstAffectedFrameId: void 0,
    lastAffectedFrameId: void 0,
    overflow: false,
    finished: false
  };
  const session = {
    ...
    beginFrame(frameId) {
      ...
      if (!positiveSafeInteger(frameId) || state.currentFrameId !== void 0 || frameId <= state.lastFrameId) {
    ...
    beginPhase(inputOrSource, phaseName) {
      ...
      if (frameId === void 0) return { ok: false, error: stateError("beginPhase") };
      ...
      if (phase === void 0) return { ok: false, error: stateError("beginPhase") };
    ...
    endPhase() {
      ...
      if (source === void 0 || phase === void 0 || frameId === void 0 || startMicros === void 0) {
        return { ok: false, error: stateError("endPhase") };
      }
      const parentIndex = index - 1;
      const parentSource = parentIndex >= 0 ? state.openSources[parentIndex] : void 0;
      const parentPhase = parentIndex >= 0 ? state.openPhaseNames[parentIndex] : void 0;
      if (!state.overflow) {
    ...
    recordSkip(input) {
      ...
      if (!stateCheck.ok) return stateCheck;
      const frameId = state.currentFrameId;
      if (frameId === void 0 || state.openDepth > 0)
...
function createProfiler(options = {}) {
  ...
  const phaseCatalog = options.phaseCatalog === void 0 ? { app: [], render: [] } : { app: [...options.phaseCatalog.app], render: [...options.phaseCatalog.render] };
  const registeredSources = /* @__PURE__ */ new Set();
  if (options.phaseCatalog !== void 0) {
...
function wrapSink(session, sink, frameLimit, complete) {
  let frameCount = 0;
  let terminal;
  function finish() {
    ...
    try {
      ...
      if (sinkResult !== void 0 && !sinkResult.ok) {
        terminal = {
          ok: false,
          error: {
...
export { buildProfileModel, compareProfileCaptures, createProfileClock, createProfiler, createUserTimingProfiler, validateProfileCapture };
