...
fn evaluate_cluster_lightX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(light: DirectLightSlotX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX, world_pos_1: vec3<f32>, normal_12: vec3<f32>, view_dir: vec3<f32>, base_color: vec3<f32>, metallic_9: f32, alpha_sq: f32, f0_2: vec3<f32>, flat_2d: bool, receive_shadows: bool) -> vec3<f32> {
    ...
    if (kind == KIND_SPOTX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX) {
        ...
        tile = encoded_tile;
        if (encoded_tile >= 0i) {
            local_17 = ((encoded_tile & PROJECTOR_FLAGX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX) != 0i);
        } else {
            local_17 = false;
        }
        let _e94 = local_17;
...
fn _samplePcssForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_12: vec3<f32>, layer_6: u32, count_4: u32, normal_17: vec3<f32>, l_1: vec3<f32>, profile: u32) -> f32 {
    ...
    if (tileUv.x >= 0f) {
        local_21 = (tileUv.x <= 1f);
    } else {
        local_21 = false;
    }
    let _e46 = local_21;
    if _e46 {
        local_22 = (tileUv.y >= 0f);
    } else {
        local_22 = false;
    }
    let _e53 = local_22;
    if _e53 {
        local_23 = (tileUv.y <= 1f);
    } else {
        local_23 = false;
    }
    let _e60 = local_23;
    if _e60 {
        local_24 = (projCoords.z >= 0f);
    } else {
        local_24 = false;
    }
    let _e67 = local_24;
    if _e67 {
        local_25 = (projCoords.z <= 1f);
    } else {
        local_25 = false;
    }
    let _e74 = local_25;
    ...
    if (lightDepthWorldSpan > 0f) {
        local_29 = (worldUnitsPerTexel > 0f);
    } else {
        local_29 = false;
    }
    let _e213 = local_29;
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_13: vec3<f32>, layer_7: u32, count_5: u32, normal_18: vec3<f32>, l_2: vec3<f32>) -> f32 {
    ...
    if (tileUv_1.x >= 0f) {
        local_32 = (tileUv_1.x <= 1f);
    } else {
        local_32 = false;
    }
    let _e40 = local_32;
    if _e40 {
        local_33 = (tileUv_1.y >= 0f);
    } else {
        local_33 = false;
    }
    let _e47 = local_33;
    if _e47 {
        local_34 = (tileUv_1.y <= 1f);
    } else {
        local_34 = false;
    }
    let _e54 = local_34;
    if _e54 {
        local_35 = (currentDepth <= 1f);
    } else {
        local_35 = false;
    }
    let _e60 = local_35;
    ...
    if (kernel == 3u) {
        if all((uv_7 >= (tileLo + texel_5))) {
            local_36 = all((uv_7 <= (tileHi - texel_5)));
        } else {
            local_36 = false;
        }
        let interior = local_36;
        if interior {
...
fn projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX(baseColorAlpha_1: f32, alphaCutoff: f32, baseColorTextureEnabled: bool, baseColorTexture: texture_2d<f32>, baseColorSampler: sampler, transform_1: vec4<f32>, metadata_3: vec4<f32>, currentClip_1: vec4<f32>, previousClip_1: vec4<f32>, temporalProjection_3: vec4<f32>, reactive_3: f32, motionValid_2: bool, uv0_1: vec2<f32>, uv1_1: vec2<f32>, uv2_1: vec2<f32>, uv3_1: vec2<f32>, uv4_1: vec2<f32>, uv5_1: vec2<f32>, uv6_1: vec2<f32>, uv7_1: vec2<f32>) -> vec4<f32> {
    ...
    if (alphaCutoff > 0f) {
        ...
        local_38 = ((baseColorAlpha_1 * _e24) <= alphaCutoff);
    } else {
        local_38 = false;
    }
    let _e30 = local_38;
...
fn fs_main(in_1: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let metallic_16 = clamp(_e10, 0f, 1f);
    ...
    let _e117 = evaluateStandardEnvironmentX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_1.worldPos, _e104, v_2, _e105, metallic_16, iblRoughness, _e106, _e108, irradianceMap_2, irradianceSampler_2, prefilterMap_2, prefilterSampler_2, brdfLut_4, skylightPrefilterMap_1, _e109, _e110);
    ...
    let _e176 = f0_;
    let _e177 = evaluateStandardDirectX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_1.worldPos, in_1.ndc.xyz, _e173, _e174, v_2, _e175, metallic_16, a_3, _e176, _e168);
    color = (_e169 + _e177);
    let _e179 = color;
