...
fn evaluate_cluster_lightX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(light: DirectLightSlotX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX, world_pos_1: vec3<f32>, normal_11: vec3<f32>, view_dir: vec3<f32>, base_color: vec3<f32>, metallic_8: f32, alpha_sq: f32, f0_1: vec3<f32>, flat_2d: bool, receive_shadows: bool) -> vec3<f32> {
    ...
    if (kind == KIND_POINTX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX) {
        ...
        if _e25 {
            ...
            let _e44 = evalPointShadowedX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(light.position.xyz, light.colorTimesIntensity.xyz, light.position.w, world_pos_1, normal_11, view_dir, base_color, metallic_8, alpha_sq, f0_1, layer_6, shadow_2.x, shadow_2.y, shadow_2.z, shadow_2.w);
            return _e44;
        }
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_12: vec3<f32>, layer_5: u32, count_4: u32, normal_15: vec3<f32>, l_1: vec3<f32>) -> f32 {
    var local_12: bool;
    var local_13: bool;
    var local_14: bool;
    var local_15: bool;
    ...
    let lightClip = (_e4 * vec4<f32>(worldPos_12, 1f));
    ...
    if (tileUv.x >= 0f) {
        local_12 = (tileUv.x <= 1f);
    } else {
        local_12 = false;
    }
    let _e40 = local_12;
    if _e40 {
        local_13 = (tileUv.y >= 0f);
    } else {
        local_13 = false;
    }
    let _e47 = local_13;
    if _e47 {
        local_14 = (tileUv.y <= 1f);
    } else {
        local_14 = false;
    }
    let _e54 = local_14;
    ...
    if (kernel == 3u) {
        if all((uv_5 >= (tileLo + texel_2))) {
            local_16 = all((uv_5 <= (tileHi - texel_2)));
        } else {
            local_16 = false;
        }
...
fn evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_16: vec3<f32>, worldPos_13: vec3<f32>, viewZ: f32) -> f32 {
    var shadow_1: f32;
    var local_17: bool;
    ...
    let receiverPosition = (worldPos_13 + (normal_16 * _e33));
...
fn cloud_direct_solar_factorX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX(worldPosition_3: vec3<f32>, shadowOrigin: vec3<f32>, shadowRight: vec3<f32>, shadowUp: vec3<f32>, shadowProjection: vec4<f32>) -> f32 {
    ...
    if (worldPosition_3.x == worldPosition_3.x) {
        local_18 = (worldPosition_3.y == worldPosition_3.y);
    } else {
        local_18 = false;
    }
    let _e10 = local_18;
    if _e10 {
        local_19 = (worldPosition_3.z == worldPosition_3.z);
    } else {
        local_19 = false;
    }
    let _e17 = local_19;
    if _e17 {
        local_20 = all((abs(worldPosition_3) < vec3(3.402823e38f)));
    } else {
        local_20 = false;
    }
    let validReceiver = local_20;
    ...
    if all((uv_6 >= vec2(0f))) {
        local_21 = all((uv_6 <= vec2(1f)));
    } else {
        local_21 = false;
    }
    let inRange = local_21;
    if (shadowProjection.y > 0.5f) {
        local_22 = (shadowProjection.z < 0.5f);
    } else {
        local_22 = false;
    }
    let _e64 = local_22;
    if _e64 {
        local_23 = (shadowProjection.x > 0f);
    } else {
        local_23 = false;
    }
    let available = local_23;
