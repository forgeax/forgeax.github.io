...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos: vec3<f32>, layer_4: u32, count_4: u32, normal_10: vec3<f32>, l_1: vec3<f32>) -> f32 {
    ...
    if (kernel == 3u) {
        if all((uv_4 >= (tileLo + texel))) {
            local_13 = all((uv_4 <= (tileHi - texel)));
        } else {
            local_13 = false;
        }
...
fn fs_main(in_1: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let _e165 = f0_;
    let _e166 = evaluateStandardDirectX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_1.worldPos, in_1.ndc.xyz, _e162, _e163, v, _e164, metallic_7, a_3, _e165, _e157);
    color = (_e158 + _e166);
    let _e168 = color;
    color = (_e168 + _e5.emissive);
    let _e173 = color;
    output.color = vec4<f32>(_e173, alpha_3);
