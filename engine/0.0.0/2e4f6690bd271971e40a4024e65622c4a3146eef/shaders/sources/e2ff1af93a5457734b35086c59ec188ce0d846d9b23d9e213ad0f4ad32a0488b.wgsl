...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos: vec3<f32>, layer_4: u32, count_4: u32, normal_10: vec3<f32>, l_1: vec3<f32>) -> f32 {
    ...
    if (kernel == 3u) {
        if all((uv_4 >= (tileLo + texel))) {
            local_16 = all((uv_4 <= (tileHi - texel)));
        } else {
            local_16 = false;
        }
...
fn fs_main(in_1: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let _e357 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(screenUv, vec2(0f), vec2(1f)), backdropLod);
    ...
    let _e365 = local_1;
    if _e365 {
        let _e373 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(refractedUv, vec2(0f), vec2(1f)), backdropLod);
    ...
    let _e428 = f0_;
    let _e429 = evaluateStandardDirectX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_1.worldPos, in_1.ndc.xyz, _e425, _e426, v, _e427, metallic_7, a_3, _e428, _e420);
    color = (_e421 + _e429);
    let _e431 = color;
    color = (_e431 + _e6.emissive);
    let _e436 = color;
    output.color = vec4<f32>(_e436, alpha_3);
