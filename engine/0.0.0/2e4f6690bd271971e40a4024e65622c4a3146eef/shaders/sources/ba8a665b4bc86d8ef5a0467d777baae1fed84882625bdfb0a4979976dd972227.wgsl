...
fn evaluateProbeDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX(shPreblend_1: array<vec4<f32>, 9>, localBlendFraction: f32, normal_5: vec3<f32>, e_sky: vec3<f32>, k_d: vec3<f32>, albedo: vec3<f32>, metallic: f32) -> vec3<f32> {
    ...
    let local_46 = (max(_e2, vec3(0f)) * PROBE_INV_PIX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX);
    let skyResidualFraction = (1f - localBlendFraction);
    return ((((local_46 + (skyResidualFraction * e_sky)) * k_d) * albedo) * (1f - metallic));
}
...
fn _samplePcssForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_15: vec3<f32>, layer_6: u32, count_4: u32, normal_17: vec3<f32>, l_1: vec3<f32>, profile: u32) -> f32 {
    ...
    if (tileUv.x >= 0f) {
        local_26 = (tileUv.x <= 1f);
    } else {
        local_26 = false;
    }
    let _e46 = local_26;
    if _e46 {
        local_27 = (tileUv.y >= 0f);
    } else {
        local_27 = false;
    }
    let _e53 = local_27;
    if _e53 {
        local_28 = (tileUv.y <= 1f);
    } else {
        local_28 = false;
    }
    let _e60 = local_28;
    if _e60 {
        local_29 = (projCoords.z >= 0f);
    } else {
        local_29 = false;
    }
    let _e67 = local_29;
    if _e67 {
        local_30 = (projCoords.z <= 1f);
    } else {
        local_30 = false;
    }
    let _e74 = local_30;
    ...
    if (lightDepthWorldSpan > 0f) {
        local_34 = (worldUnitsPerTexel > 0f);
    } else {
        local_34 = false;
    }
    let _e213 = local_34;
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_16: vec3<f32>, layer_7: u32, count_5: u32, normal_18: vec3<f32>, l_2: vec3<f32>) -> f32 {
    ...
    if (tileUv_1.x >= 0f) {
        local_37 = (tileUv_1.x <= 1f);
    } else {
        local_37 = false;
    }
    let _e40 = local_37;
    if _e40 {
        local_38 = (tileUv_1.y >= 0f);
    } else {
        local_38 = false;
    }
    let _e47 = local_38;
    if _e47 {
        local_39 = (tileUv_1.y <= 1f);
    } else {
        local_39 = false;
    }
    let _e54 = local_39;
    if _e54 {
        local_40 = (currentDepth <= 1f);
    } else {
        local_40 = false;
    }
    let _e60 = local_40;
    ...
    if (kernel == 3u) {
        if all((uv_7 >= (tileLo + texel_5))) {
            local_41 = all((uv_7 <= (tileHi - texel_5)));
        } else {
            local_41 = false;
        }
        let interior = local_41;
        if interior {
...
fn projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX(baseColorAlpha_1: f32, alphaCutoff: f32, baseColorTextureEnabled: bool, baseColorTexture: texture_2d<f32>, baseColorSampler: sampler, transform_1: vec4<f32>, metadata_3: vec4<f32>, currentClip_1: vec4<f32>, previousClip_1: vec4<f32>, temporalProjection_3: vec4<f32>, reactive_3: f32, motionValid_2: bool, uv0_1: vec2<f32>, uv1_1: vec2<f32>, uv2_1: vec2<f32>, uv3_1: vec2<f32>, uv4_1: vec2<f32>, uv5_1: vec2<f32>, uv6_1: vec2<f32>, uv7_1: vec2<f32>) -> vec4<f32> {
    ...
    if (alphaCutoff > 0f) {
        ...
        local_43 = ((baseColorAlpha_1 * _e24) <= alphaCutoff);
    } else {
        local_43 = false;
    }
    let _e30 = local_43;
...
fn vs_main_impl(in_5: VsIn, meshIndex: u32, instanceIndex: u32, materialAddress: vec3<u32>) -> VsOut {
    ...
    out_1.positionOSAndViewZ = vec4<f32>(in_5.pos, _e29);
...
fn alphaTestSurface(surface: SurfaceData) {
    ...
    if (surface.alphaClipThreshold > 0f) {
        local_44 = (surface.opacity <= surface.alphaClipThreshold);
    } else {
        local_44 = false;
    }
    let _e10 = local_44;
...
fn fs_main(in_1: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let _e5 = evaluateStandardSurface(in_1, frontFacing);
    alphaTestSurface(_e5);
    let alpha_3 = _e5.opacity;
    let albedo_5 = _e5.baseColor;
    let _e10 = finiteScalar(_e5.metallic, 0f);
    let metallic_17 = clamp(_e10, 0f, 1f);
    let _e16 = finiteScalar(_e5.roughness, 0.5f);
    let iblRoughness = clamp(_e16, 0.04f, 1f);
    let a_4 = (iblRoughness * iblRoughness);
    let n_6 = normalize(_e5.normalWS);
    let _e25 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cameraPos;
    let v_5 = normalize((_e25 - in_1.worldPos));
    let _e31 = material.ior;
    let _e33 = finiteScalar(_e31, 1.5f);
    let safeIor_1 = max(_e33, 1f);
    let dielectricF0_1 = pow(((safeIor_1 - 1f) / (safeIor_1 + 1f)), 2f);
    let _e43 = standardSurfaceF0_(in_1, _e5);
    f0_ = _e43;
    physicalNormal = n_6;
    diffuseAlbedo = albedo_5;
    let _e52 = probeBlendRecords[(0u + 1u)];
    let _e57 = probeBlendRecords[(0u + 2u)];
    let _e62 = probeBlendRecords[(0u + 3u)];
    let _e67 = probeBlendRecords[(0u + 4u)];
    let _e72 = probeBlendRecords[(0u + 5u)];
    let _e77 = probeBlendRecords[(0u + 6u)];
    let _e82 = probeBlendRecords[(0u + 7u)];
    let _e87 = probeBlendRecords[(0u + 8u)];
    let _e92 = probeBlendRecords[(0u + 9u)];
    probeShPreblend = array<vec4<f32>, 9>(_e52, _e57, _e62, _e67, _e72, _e77, _e82, _e87, _e92);
    let _e98 = probeBlendRecords[0u].z;
    probeLocalBlendFraction = select(0f, _e98, true);
    let _e104 = physicalNormal;
    let _e105 = diffuseAlbedo;
    let _e106 = f0_;
    let _e108 = skylight;
    let _e109 = probeShPreblend;
    let _e110 = probeLocalBlendFraction;
    let _e117 = evaluateStandardEnvironmentX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_1.worldPos, _e104, v_5, _e105, metallic_17, iblRoughness, _e106, _e108, irradianceMap_2, irradianceSampler_2, prefilterMap_2, prefilterSampler_2, brdfLut_4, skylightPrefilterMap_1, _e109, _e110);
    let ao = _e5.occlusion;
    let _e121 = skylight.colorR;
    let _e124 = skylight.colorG;
    let _e127 = skylight.colorB;
    let _e131 = skylight.intensity;
    let _e132 = decodeSpecularEnvironmentScaleX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(vec3<f32>(_e121, _e124, _e127), _e131);
    let _e139 = coatF;
    ambient = (((_e117.diffuse + _e117.specular) * (vec3(1f) - _e139)) * ao);
    let _e147 = coatF;
    reflectionFallback = ((_e117.specular * (vec3(1f) - _e147)) * ao);
    let _e155 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.worldViewProj;
    let _e158 = sampleStandardAmbientOcclusionX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_1.worldPos, _e155, ssaoBlurredTexture, ssaoBlurredSampler);
    let _e159 = ambient;
    ambient = (_e159 * _e158);
    let _e161 = reflectionFallback;
    reflectionFallback = (_e161 * _e158);
    let _e163 = ambient;
    color = _e163;
    let _e165 = physicalNormal;
    let _e167 = standardViewZ(in_1);
    let _e168 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e165, in_1.worldPos, _e167);
    let _e169 = color;
    let _e173 = standardViewZ(in_1);
    let _e174 = physicalNormal;
    let _e175 = diffuseAlbedo;
    let _e176 = f0_;
    let _e177 = evaluateStandardDirectX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_1.worldPos, in_1.ndc.xyz, _e173, _e174, v_5, _e175, metallic_17, a_4, _e176, _e168);
    color = (_e169 + _e177);
    let _e179 = color;
    color = (_e179 + _e5.emissive);
    let _e184 = color;
    output.color = vec4<f32>(_e184, alpha_3);
    let _e187 = reflectionFallback;
    let _e188 = standardSsrCoverage();
    output.reflectionFallback = vec4<f32>(_e187, _e188);
    let _e190 = output;
    return _e190;
}
