...
@group(0) @binding(2) 
var currentNormal: texture_2d<u32>;
@group(0) @binding(3) 
...
fn ssrHistoryTap(pixel_1: vec2<i32>, expectedDepth: f32, normal_1: vec3<f32>, footprintWeight: f32) -> vec4<f32> {
    var local_12: bool;

    let history_1 = textureLoad(previousHistory, pixel_1, 0i);
    let surface = textureLoad(previousSurface, pixel_1, 0i);
    let _e9 = depthReject(expectedDepth, history_1.w);
    if !(_e9) {
        let _e18 = normalReject(normal_1, ((surface.xyz * 2f) - vec3(1f)));
        local_12 = _e18;
    } else {
        local_12 = true;
    }
...
fn sampleSsrHistory(uv_2: vec2<f32>, fullSize_1: vec2<u32>, expectedDepth_1: f32, normal_2: vec3<f32>) -> vec4<f32> {
    ...
    let _e16 = sum_1;
    let _e33 = ssrHistoryTap(clamp((first + vec2<i32>(0i, 0i)), vec2(0i), last_1), expectedDepth_1, normal_2, ((1f - fraction.x) * (1f - fraction.y)));
    sum_1 = (_e16 + _e33);
    let _e35 = sum_1;
    let _e48 = ssrHistoryTap(clamp((first + vec2<i32>(1i, 0i)), vec2(0i), last_1), expectedDepth_1, normal_2, (fraction.x * (1f - fraction.y)));
    sum_1 = (_e35 + _e48);
    let _e50 = sum_1;
    let _e63 = ssrHistoryTap(clamp((first + vec2<i32>(0i, 1i)), vec2(0i), last_1), expectedDepth_1, normal_2, ((1f - fraction.x) * fraction.y));
    sum_1 = (_e50 + _e63);
    let _e65 = sum_1;
    let _e76 = ssrHistoryTap(clamp((first + vec2<i32>(1i, 1i)), vec2(0i), last_1), expectedDepth_1, normal_2, (fraction.x * fraction.y));
    sum_1 = (_e65 + _e76);
    let _e78 = sum_1;
    let _e81 = sum_1.w;
    let _e87 = sum_1.w;
...
fn ssrCurrentTap(pixel_2: vec2<i32>, expectedDepth_2: f32, normal_3: vec3<f32>, footprintWeight_1: f32) -> SsrCurrentSample {
    var local_13: bool;
    var reactivity: f32 = 0f;

    let _e5 = loadStandardNormalRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(currentNormal, (pixel_2 * 2i));
    let sourceNormal = _e5.xyz;
    let _e11 = textureLoad(currentDepth, (pixel_2 * 2i), 0i);
    let _e12 = viewDistance(_e11);
    let _e14 = depthReject(expectedDepth_2, _e12);
    if !(_e14) {
        let _e17 = normalReject(normal_3, sourceNormal);
        local_13 = _e17;
    } else {
        local_13 = true;
    }
    ...
    let sample = textureLoad(currentTrace, pixel_2, 0i);
    let _e30 = finiteUnit(sample.w);
    let weight_1 = (footprintWeight_1 * _e30);
    if (footprintWeight_1 > 0f) {
        let _e37 = textureLoad(currentHitReactivity, pixel_2, 0i);
        let _e39 = finiteUnit(_e37.x);
        reactivity = _e39;
    }
    let _e44 = reactivity;
    return SsrCurrentSample(vec4<f32>((sample.xyz * weight_1), weight_1), _e44);
}

fn sampleCurrentSsr(uv_3: vec2<f32>, fullSize_2: vec2<u32>, expectedDepth_3: f32, normal_4: vec3<f32>) -> SsrCurrentSample {
    ...
    let _e31 = ssrCurrentTap(clamp((first_1 + vec2<i32>(0i, 0i)), vec2(0i), last_2), expectedDepth_3, normal_4, ((1f - fraction_1.x) * (1f - fraction_1.y)));
    let _e33 = sum_2;
    sum_2 = (_e33 + _e31.color);
    let _e37 = reactivity_1;
    reactivity_1 = max(_e37, _e31.reactivity);
    let _e52 = ssrCurrentTap(clamp((first_1 + vec2<i32>(1i, 0i)), vec2(0i), last_2), expectedDepth_3, normal_4, (fraction_1.x * (1f - fraction_1.y)));
    let _e53 = sum_2;
    sum_2 = (_e53 + _e52.color);
    let _e56 = reactivity_1;
    reactivity_1 = max(_e56, _e52.reactivity);
    let _e71 = ssrCurrentTap(clamp((first_1 + vec2<i32>(0i, 1i)), vec2(0i), last_2), expectedDepth_3, normal_4, ((1f - fraction_1.x) * fraction_1.y));
    let _e72 = sum_2;
    sum_2 = (_e72 + _e71.color);
    let _e75 = reactivity_1;
    reactivity_1 = max(_e75, _e71.reactivity);
    let _e88 = ssrCurrentTap(clamp((first_1 + vec2<i32>(1i, 1i)), vec2(0i), last_2), expectedDepth_3, normal_4, (fraction_1.x * fraction_1.y));
    let _e89 = sum_2;
...
fn ssrNeighborhoodRow(pixel_3: vec2<i32>, last: vec2<i32>) -> NeighborhoodBounds {
    let _e11 = textureLoad(currentTrace, clamp((pixel_3 + vec2<i32>(-1i, 0i)), vec2(0i), last), 0i);
    let left = _e11.xyz;
    let _e18 = textureLoad(currentTrace, clamp(pixel_3, vec2(0i), last), 0i);
    let center = _e18.xyz;
    let _e29 = textureLoad(currentTrace, clamp((pixel_3 + vec2<i32>(1i, 0i)), vec2(0i), last), 0i);
    let right = _e29.xyz;
    return NeighborhoodBounds(min(min(left, center), right), max(max(left, center), right));
}

fn neighborhoodClamp(pixel_4: vec2<i32>, size: vec2<u32>) -> NeighborhoodBounds {
    let last_3 = (vec2<i32>(size) - vec2(1i));
    let _e10 = ssrNeighborhoodRow((pixel_4 + vec2<i32>(0i, -1i)), last_3);
    let _e11 = ssrNeighborhoodRow(pixel_4, last_3);
    let _e16 = ssrNeighborhoodRow((pixel_4 + vec2<i32>(0i, 1i)), last_3);
...
fn resolveSsrAt(uv_4: vec2<f32>, fullSize_3: vec2<u32>) -> SsrTemporalSample {
    ...
    let _e40 = loadStandardNormalRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(currentNormal, vec2<i32>(fullPixel));
    let normal_5 = _e40.xyz;
    let sourceUv = ((vec2<f32>(fullPixel) + vec2(0.5f)) / vec2<f32>(fullSize_3));
    let _e50 = view.inverseViewProj;
    let worldH = (_e50 * vec4<f32>(((sourceUv.x * 2f) - 1f), (1f - (sourceUv.y * 2f)), depthNdc, 1f));
    let _e66 = view.temporalPreviousViewProj;
    let priorClip = (_e66 * vec4<f32>((worldH.xyz / vec3(worldH.w)), 1f));
    let _e74 = sampleCurrentSsr(currentUv, fullSize_3, _e20, normal_5);
    let _e76 = sampleSsrHistory(_e25, fullSize_3, priorClip.w, normal_5);
    let size_1 = textureDimensions(currentTrace, 0i);
    let _e80 = ssrLatticeCoordinate(currentUv, fullSize_3);
    let pixel_5 = clamp(vec2<i32>(floor((_e80 + vec2(0.5f)))), vec2(0i), (vec2<i32>(size_1) - vec2(1i)));
    let _e93 = neighborhoodClamp(pixel_5, size_1);
    let _e106 = resolveSsrTemporal(_e74.color, _e76, _e76.w, _e93.lower, _e93.upper, historyInBounds_1, (_e20 > 0f), true, vec4<f32>(temporal_1.xyz, max(temporal_1.w, _e74.reactivity)));
    let _e107 = finitePositive(_e20);
    return SsrTemporalSample(_e106, normal_5, _e107);
}
...
fn ssr_reflection_mip(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let _e24 = local;
    if _e24 {
        let source_1 = (id.xy * vec2(2u));
        let topLeft = textureLoad(currentTrace, vec2<i32>(source_1), 0i);
        let topRight = textureLoad(currentTrace, vec2<i32>((source_1 + vec2<u32>(1u, 0u))), 0i);
        let bottomLeft = textureLoad(currentTrace, vec2<i32>((source_1 + vec2<u32>(0u, 1u))), 0i);
        let bottomRight = textureLoad(currentTrace, vec2<i32>((source_1 + vec2<u32>(1u, 1u))), 0i);
...
fn ssr_temporal(@builtin(global_invocation_id) globalId: vec3<u32>) {
    ...
    let pixel_6 = vec2<i32>(globalId.xy);
    ...
    textureStore(historyOutput, pixel_6, vec4<f32>(_e21.color.xyz, _e21.depth));
    textureStore(surfaceOutput, pixel_6, vec4<f32>(((_e21.normal * 0.5f) + vec3(0.5f)), _e21.color.w));
    ...
    textureStore(resolvedOutput, pixel_6, vec4<f32>((_e52.xyz * _e55), _e58));
    return;
}
