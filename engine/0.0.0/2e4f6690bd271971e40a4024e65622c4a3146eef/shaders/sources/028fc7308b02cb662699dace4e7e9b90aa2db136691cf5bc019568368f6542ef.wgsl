...
fn sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(normal_3: vec3<f32>, view_2: vec3<f32>, roughness_3: f32, F0_3: vec3<f32>, worldPosition_1: vec3<f32>, boxCenter_1: vec3<f32>, boxExtents_1: vec3<f32>, rotation_3: vec4<f32>, probeMap: texture_cube<f32>, probeSampler: sampler, brdfLut_2: texture_2d<f32>, brdfLutSampler_2: sampler, skylightMap: texture_cube<f32>, skylightRotation: vec4<f32>, skylightScale: vec3<f32>, probeIntensity: f32, boxProjection: bool) -> vec3<f32> {
    ...
    let local_51 = (_e55 * probeIntensity);
    let _e58 = sky;
    return mix(_e58, local_51, weight);
}
...
fn evaluateProbeDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX(shPreblend_1: array<vec4<f32>, 9>, localBlendFraction: f32, normal_5: vec3<f32>, e_sky: vec3<f32>, k_d: vec3<f32>, albedo: vec3<f32>, metallic: f32) -> vec3<f32> {
    ...
    let local_52 = (max(_e2, vec3(0f)) * PROBE_INV_PIX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX);
    let skyResidualFraction = (1f - localBlendFraction);
    return ((((local_52 + (skyResidualFraction * e_sky)) * k_d) * albedo) * (1f - metallic));
}
...
fn cloud_direct_solar_factorX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX(worldPosition_2: vec3<f32>, shadowOrigin: vec3<f32>, shadowRight: vec3<f32>, shadowUp: vec3<f32>, shadowProjection: vec4<f32>) -> f32 {
    ...
    if (worldPosition_2.x == worldPosition_2.x) {
        local_6 = (worldPosition_2.y == worldPosition_2.y);
    } else {
        local_6 = false;
    }
    let _e10 = local_6;
    if _e10 {
        local_7 = (worldPosition_2.z == worldPosition_2.z);
    } else {
        local_7 = false;
    }
    let _e17 = local_7;
    if _e17 {
        local_8 = all((abs(worldPosition_2) < vec3(3.402823e38f)));
    } else {
        local_8 = false;
    }
    let validReceiver = local_8;
    let offset_1 = (worldPosition_2 - shadowOrigin);
    let uv_4 = ((vec2<f32>(dot(offset_1, shadowRight), dot(offset_1, shadowUp)) / vec2(max(shadowProjection.x, 0.000001f))) + vec2(0.5f));
    if all((uv_4 >= vec2(0f))) {
        local_9 = all((uv_4 <= vec2(1f)));
    } else {
        local_9 = false;
    }
    let inRange = local_9;
    if (shadowProjection.y > 0.5f) {
        local_10 = (shadowProjection.z < 0.5f);
    } else {
        local_10 = false;
    }
    let _e64 = local_10;
    if _e64 {
        local_11 = (shadowProjection.x > 0f);
    } else {
        local_11 = false;
    }
    let available = local_11;
    let _e80 = textureSampleLevel(cloudShadowMapX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX, cloudShadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX, clamp(uv_4, vec2(0f), vec2(1f)), 0f);
    let sampled = _e80.x;
    if validReceiver {
        local_12 = inRange;
    } else {
...
fn sampleThreeR184DfgLutX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(roughness_4: f32, dotNV: f32) -> vec2<f32> {
    let uv_5 = clamp(vec2<f32>(roughness_4, dotNV), vec2(0f), vec2(1f));
    let samplePosition = ((uv_5 * 16f) - vec2(0.5f));
...
fn evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_6: vec3<f32>, viewDir: vec3<f32>, baseColor: vec3<f32>, metallic_1: f32, alphaSq: f32, F0_5: vec3<f32>) -> vec3<f32> {
    ...
    let _e31 = threeR184DirectMultiScatterX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(roughness_10, nDotV_2, nDotL_5, F0_5);
...
fn evalPunctualBodyX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(lightPos_1: vec3<f32>, colorTimesIntensity: vec3<f32>, invRangeSquared_2: f32, worldPos_2: vec3<f32>, normal_7: vec3<f32>, viewDir_1: vec3<f32>, baseColor_1: vec3<f32>, metallic_2: f32, alphaSq_1: f32, F0_6: vec3<f32>) -> vec3<f32> {
    ...
    let _e31 = threeR184DirectMultiScatterX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(roughness_11, nDotV_3, nDotL_6, F0_6);
...
fn spotModifierFactorsX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXG4DPORPW233ENFTGSZLSOMX(lightPos_8: vec3<f32>, lightDir_4: vec3<f32>, cosOuter_4: f32, worldPos_9: vec3<f32>, rollDeg: f32, metadata: vec4<u32>) -> vec4<f32> {
    ...
    if (metadata.w != 4294967295u) {
        if !((metadata.y == 4294967295u)) {
            local_20 = ((metadata.y & PROJECTOR_FLAGX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXG4DPORPW233ENFTGSZLSOMX) == 0u);
        } else {
            local_20 = true;
        }
        let _e90 = local_20;
    ...
    if _e94 {
        if (depth <= 0f) {
            ...
            if !((cookieUv.x < 0f)) {
                local_21 = (cookieUv.x > 1f);
            } else {
                local_21 = true;
            }
            let _e137 = local_21;
            if !(_e137) {
                local_22 = (cookieUv.y < 0f);
            } else {
                local_22 = true;
            }
            let _e145 = local_22;
            if !(_e145) {
                local_23 = (cookieUv.y > 1f);
            } else {
                local_23 = true;
            }
            let _e153 = local_23;
...
fn ltcEvaluateX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXEZLDORPWC4TFMEX(n_1: vec3<f32>, v_1: vec3<f32>, worldPos_10: vec3<f32>, mInv: mat3x3<f32>, lightPos_9: vec3<f32>, axisX: vec3<f32>, axisY: vec3<f32>, halfWidth: f32, halfHeight: f32) -> f32 {
    ...
    let c0_ = normalize((transform_4 * (rect0_ - worldPos_10)));
    let c1_ = normalize((transform_4 * (rect1_ - worldPos_10)));
    let c2_ = normalize((transform_4 * (rect2_ - worldPos_10)));
    let c3_ = normalize((transform_4 * (rect3_ - worldPos_10)));
...
fn evalRectAreaLtcDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXEZLDORPWC4TFMEX(lightPos_11: vec3<f32>, lightColor: vec3<f32>, axisX_1: vec3<f32>, axisY_1: vec3<f32>, halfWidth_1: f32, halfHeight_1: f32, invRangeSquared_11: f32, worldPos_12: vec3<f32>, n_2: vec3<f32>, v_2: vec3<f32>, baseColor_8: vec3<f32>, metallic_7: f32, alphaSq_6: f32, F0_11: vec3<f32>) -> vec3<f32> {
    let identity_1 = mat3x3<f32>(vec3<f32>(1f, 0f, 0f), vec3<f32>(0f, 1f, 0f), vec3<f32>(0f, 0f, 1f));
    let _e21 = ltcEvaluateX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXEZLDORPWC4TFMEX(n_2, v_2, worldPos_12, identity_1, lightPos_11, axisX_1, axisY_1, halfWidth_1, halfHeight_1);
    let diffuse_3 = (baseColor_8 * (1f - metallic_7));
...
fn evalRectAreaLtcStandardX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXEZLDORPWC4TFMEX(lightPos_12: vec3<f32>, lightColor_1: vec3<f32>, axisX_2: vec3<f32>, axisY_2: vec3<f32>, halfWidth_2: f32, halfHeight_2: f32, invRangeSquared_12: f32, worldPos_13: vec3<f32>, n_3: vec3<f32>, v_3: vec3<f32>, baseColor_9: vec3<f32>, metallic_8: f32, alphaSq_7: f32, F0_12: vec3<f32>) -> vec3<f32> {
    ...
    let fresnel_1 = ((F0_12 * fresnelSample.x) + ((vec3(1f) - F0_12) * fresnelSample.y));
    ...
    let _e48 = evalRectAreaLtcDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXEZLDORPWC4TFMEX(lightPos_12, lightColor_1, axisX_2, axisY_2, halfWidth_2, halfHeight_2, invRangeSquared_12, worldPos_13, n_3, v_3, baseColor_9, metallic_8, alphaSq_7, F0_12);
...
fn evalRectAreaLtcGgxX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXEZLDORPWC4TFMEX(lightPos_13: vec3<f32>, lightColor_2: vec3<f32>, axisX_3: vec3<f32>, axisY_3: vec3<f32>, halfWidth_3: f32, halfHeight_3: f32, invRangeSquared_13: f32, worldPos_14: vec3<f32>, n_4: vec3<f32>, v_4: vec3<f32>, baseColor_10: vec3<f32>, metallic_9: f32, alphaSq_8: f32, F0_13: vec3<f32>) -> vec3<f32> {
    let _e14 = evalRectAreaLtcStandardX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXEZLDORPWC4TFMEX(lightPos_13, lightColor_2, axisX_3, axisY_3, halfWidth_3, halfHeight_3, invRangeSquared_13, worldPos_14, n_4, v_4, baseColor_10, metallic_9, alphaSq_8, F0_13);
    return _e14;
}
...
fn evaluate_cluster_lightX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(light: DirectLightSlotX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX, world_pos_1: vec3<f32>, normal_12: vec3<f32>, view_dir: vec3<f32>, base_color: vec3<f32>, metallic_10: f32, alpha_sq: f32, f0_2: vec3<f32>, flat_2d: bool, receive_shadows: bool) -> vec3<f32> {
    var local_27: bool;
    ...
    if (kind == KIND_POINTX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX) {
        ...
        if _e25 {
            ...
            let _e44 = evalPointShadowedX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(light.position.xyz, light.colorTimesIntensity.xyz, light.position.w, world_pos_1, normal_12, view_dir, base_color, metallic_10, alpha_sq, f0_2, layer_8, shadow_3.x, shadow_3.y, shadow_3.z, shadow_3.w);
            return _e44;
        }
        let _e51 = evalPointX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(light.position.xyz, light.colorTimesIntensity.xyz, light.position.w, world_pos_1, normal_12, view_dir, base_color, metallic_10, alpha_sq, f0_2);
        return _e51;
    }
    if (kind == KIND_SPOTX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX) {
        ...
        tile = encoded_tile;
        if (encoded_tile >= 0i) {
            local_28 = ((encoded_tile & PROJECTOR_FLAGX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX) != 0i);
        } else {
            local_28 = false;
        }
        let _e94 = local_28;
        ...
        if _e108 {
            ...
            let _e126 = tile;
            let _e133 = evalSpotShadowedX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(light.position.xyz, light.direction.xyz, light.colorTimesIntensity.xyz, light.colorTimesIntensity.w, light.direction.w, light.position.w, world_pos_1, normal_12, view_dir, base_color, metallic_10, alpha_sq, f0_2, _e125, _e126, 0.005f, 0.05f, 3f, select(0f, 1f, receive_shadows));
            let _e139 = tile;
        ...
        let _e159 = evalSpotX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(light.position.xyz, light.direction.xyz, light.colorTimesIntensity.xyz, light.colorTimesIntensity.w, light.direction.w, light.position.w, world_pos_1, normal_12, view_dir, base_color, metallic_10, alpha_sq, f0_2);
        return (_e159 * modifier);
    }
    if (kind == KIND_RECT_AREAX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX) {
        let _e177 = evalRectAreaLtcGgxX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXEZLDORPWC4TFMEX(light.position.xyz, light.colorTimesIntensity.xyz, light.auxiliary.xyz, light.direction.xyz, light.colorTimesIntensity.w, light.direction.w, light.position.w, world_pos_1, normal_12, view_dir, base_color, metallic_10, alpha_sq, f0_2);
        return _e177;
    }
    return vec3(0f);
}

fn evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(ndc_1: vec3<f32>, view_z_2: f32, world_pos_2: vec3<f32>, normal_13: vec3<f32>, view_dir_1: vec3<f32>, base_color_1: vec3<f32>, metallic_11: f32, alpha_sq_1: f32, f0_3: vec3<f32>, flat_2d_1: bool, receive_shadows_1: bool) -> vec3<f32> {
    ...
    loop {
        ...
        }
        {
            ...
            let _e70 = evaluate_cluster_lightX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(light_1, world_pos_2, normal_13, view_dir_1, base_color_1, metallic_11, alpha_sq_1, f0_3, flat_2d_1, receive_shadows_1);
...
fn evaluateStandardEnvironmentX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(worldPosition_4: vec3<f32>, normal_14: vec3<f32>, direction_2: vec3<f32>, albedo_1: vec3<f32>, metallic_12: f32, roughness_7: f32, f0_4: vec3<f32>, sky_1: SkylightUniformsX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX, irradianceMap_1: texture_cube<f32>, irradianceSampler_1: sampler, prefilterMap_1: texture_cube<f32>, prefilterSampler_1: sampler, brdfLut_3: texture_2d<f32>, skylightPrefilterMap: texture_cube<f32>, sh: array<vec4<f32>, 9>, localBlend: f32) -> StandardEnvironmentX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX {
    ...
    if (sky_1.intensity < 0f) {
        ...
        irradiance = _e45;
        let _e47 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(normal_14, direction_2, roughness_7, f0_4, sky_1.rotation, prefilterMap_1, prefilterSampler_1, brdfLut_3, irradianceSampler_1);
        specular = _e47;
    }
    ...
    let _e63 = fresnelSchlickRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXG2DBOJSWIX(max(dot(normal_14, direction_2), 0f), f0_4, roughness_7);
    let kD = ((vec3(1f) - _e63) * (1f - metallic_12));
    let _e72 = irradiance;
    diffuse = (((kD * _e72) * albedo_1) * diffuseScale);
    if (localBlend > 0f) {
        let _e80 = irradiance;
        let _e90 = evaluateProbeDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX(sh, localBlend, normal_14, (_e80 * diffuseScale), (kD / max(vec3((1f - metallic_12)), vec3(0.0001f))), albedo_1, metallic_12);
        diffuse = _e90;
    }
    let _e91 = diffuse;
    let _e92 = specular;
    let _e94 = decodeSpecularEnvironmentScaleX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(tint, sky_1.intensity);
    let _e98 = projectSpecularRadianceX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(vec3(1f), normal_14, direction_2, roughness_7, f0_4, brdfLut_3, irradianceSampler_1);
    return StandardEnvironmentX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(_e91, (_e92 * _e94), _e98);
}

fn evaluateStandardDirectX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(worldPosition_5: vec3<f32>, ndc_2: vec3<f32>, viewZ: f32, normal_15: vec3<f32>, direction_3: vec3<f32>, albedo_2: vec3<f32>, metallic_13: f32, alpha: f32, f0_5: vec3<f32>, shadow_1: f32) -> vec3<f32> {
    var direct: vec3<f32>;

    let _e6 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_15, direction_3, albedo_2, metallic_13, alpha, f0_5);
    ...
    direct = _e25;
    let _e27 = direct;
    let _e32 = evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(ndc_2, viewZ, worldPosition_5, normal_15, direction_3, albedo_2, metallic_13, alpha, f0_5, false, true);
    direct = (_e27 + _e32);
    let _e34 = direct;
    return _e34;
}
...
fn sampleStandardAmbientOcclusionX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(worldPosition_6: vec3<f32>, worldViewProjection: mat4x4<f32>, aoTexture: texture_2d<f32>, aoSampler: sampler) -> f32 {
    ...
    let uv_6 = (((clip_4.xy / vec2(clip_4.w)) * vec2<f32>(0.5f, -0.5f)) + vec2(0.5f));
    let _e23 = textureSampleLevel(aoTexture, aoSampler, uv_6, 0f);
...
fn _directionalReceiverDepthBiasX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(layer_5: u32, normal_16: vec3<f32>, l: vec3<f32>, radius: f32) -> f32 {
    var local_30: bool;

    let nDotL_9 = dot(normal_16, l);
    ...
    let slope = ((abs(dot(normal_16, right_1)) + abs(dot(normal_16, up_1))) / nDotL_9);
...
fn _samplePcssForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_15: vec3<f32>, layer_6: u32, count_4: u32, normal_17: vec3<f32>, l_1: vec3<f32>, profile: u32) -> f32 {
    var local_31: bool;
    var local_32: bool;
    var local_33: bool;
    var local_34: bool;
    ...
    if (tileUv.x >= 0f) {
        local_32 = (tileUv.x <= 1f);
    } else {
        local_32 = false;
    }
    let _e46 = local_32;
    if _e46 {
        local_33 = (tileUv.y >= 0f);
    } else {
        local_33 = false;
    }
    let _e53 = local_33;
    if _e53 {
        local_34 = (tileUv.y <= 1f);
    } else {
        local_34 = false;
    }
    let _e60 = local_34;
    if _e60 {
        local_35 = (projCoords.z >= 0f);
    } else {
        local_35 = false;
    }
    let _e67 = local_35;
    if _e67 {
        local_36 = (projCoords.z <= 1f);
    } else {
        local_36 = false;
    }
    let _e74 = local_36;
    ...
    let nDotL_10 = dot(normal_17, l_1);
    if (nDotL_10 >= -1f) {
        local_37 = (nDotL_10 <= 1f);
    } else {
        local_37 = false;
    }
    let _e87 = local_37;
    ...
    let _e118 = _directionalReceiverDepthBiasX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(layer_6, normal_17, l_1, (rawRadius + 1f));
    ...
    if (lightDepthWorldSpan > 0f) {
        local_40 = (worldUnitsPerTexel > 0f);
    } else {
        local_40 = false;
    }
    let _e213 = local_40;
    ...
    let _e258 = _directionalReceiverDepthBiasX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(layer_6, normal_17, l_1, (compareRadius + 1.5f));
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_16: vec3<f32>, layer_7: u32, count_5: u32, normal_18: vec3<f32>, l_2: vec3<f32>) -> f32 {
    var local_43: bool;
    var local_44: bool;
    var local_45: bool;
    var local_46: bool;
    ...
    let uv_7 = ((tileUv_1 * _e14) + _e15);
    let currentDepth = projCoords_1.z;
    if (tileUv_1.x >= 0f) {
        local_43 = (tileUv_1.x <= 1f);
    } else {
        local_43 = false;
    }
    let _e40 = local_43;
    if _e40 {
        local_44 = (tileUv_1.y >= 0f);
    } else {
        local_44 = false;
    }
    let _e47 = local_44;
    if _e47 {
        local_45 = (tileUv_1.y <= 1f);
    } else {
        local_45 = false;
    }
    let _e54 = local_45;
    if _e54 {
        local_46 = (currentDepth <= 1f);
    } else {
        local_46 = false;
    }
    let _e60 = local_46;
    ...
    if (filterProfile >= 4u) {
        let _e90 = _samplePcssForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_16, layer_7, count_5, normal_18, l_2, filterProfile);
        return _e90;
    }
    let kernel = select(select(3u, 5u, (filterProfile == 3u)), 1u, (filterProfile == 1u));
    let _e105 = _directionalReceiverDepthBiasX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(layer_7, normal_18, l_2, f32(((kernel / 2u) + 1u)));
    let adjustedDepth_1 = (currentDepth - _e105);
    if (kernel == 1u) {
        let lit_1 = textureSampleCompareLevel(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, clamp(uv_7, tileLo, tileHi), adjustedDepth_1);
        return lit_1;
    }
    if (kernel == 3u) {
        if all((uv_7 >= (tileLo + texel_5))) {
            local_47 = all((uv_7 <= (tileHi - texel_5)));
        } else {
            local_47 = false;
        }
        let interior = local_47;
        if interior {
            let pcfFraction = fract(((uv_7 / texel_5) - vec2(0.5f)));
            ...
            let litLoLo = textureSampleCompareLevel(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, (uv_7 + (vec2<f32>(loOffset.x, loOffset.y) * texel_5)), adjustedDepth_1);
            let litHiLo = textureSampleCompareLevel(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, (uv_7 + (vec2<f32>(hiOffset.x, loOffset.y) * texel_5)), adjustedDepth_1);
            let litLoHi = textureSampleCompareLevel(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, (uv_7 + (vec2<f32>(loOffset.x, hiOffset.y) * texel_5)), adjustedDepth_1);
            let litHiHi = textureSampleCompareLevel(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, (uv_7 + (vec2<f32>(hiOffset.x, hiOffset.y) * texel_5)), adjustedDepth_1);
            ...
            }
            {
                ...
                loop {
                    ...
                    }
                    {
                        let _e210 = x_1;
                        let _e212 = y_1;
                        let offsetUv_1 = clamp((uv_7 + (vec2<f32>(f32(_e210), f32(_e212)) * texel_5)), tileLo, tileHi);
        ...
        }
        {
            ...
            loop {
                ...
                }
                {
                    let _e246 = x_2;
                    let _e248 = y_2;
                    let offsetUv_2 = clamp((uv_7 + (vec2<f32>(f32(_e246), f32(_e248)) * texel_5)), tileLo, tileHi);
...
fn evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_19: vec3<f32>, worldPos_17: vec3<f32>, viewZ_1: f32) -> f32 {
    var shadow_2: f32;
    var local_48: bool;
    ...
    let receiverPosition = (worldPos_17 + (normal_19 * _e33));
    let _e36 = _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(receiverPosition, _e28, count_6, normal_19, l_7);
    shadow_2 = _e36;
    let _e40 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cascadeBlend;
    if (_e40 > 0f) {
        local_48 = ((_e28 + 1u) < count_6);
    } else {
        local_48 = false;
    }
    let _e49 = local_48;
    if _e49 {
        ...
        if (blendWidth > 0f) {
            ...
            if (t_1 > 0f) {
                let _e72 = _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(receiverPosition, (_e28 + 1u), count_6, normal_19, l_7);
...
fn encodeStandardNormalRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(normal_20: vec3<f32>, roughness_8: f32) -> u32 {
    var oct: vec2<f32>;

    let n_5 = normalize(normal_20);
    ...
    let _e33 = oct;
    let uv_8 = vec2<u32>(round((clamp(((_e33 * 0.5f) + vec2(0.5f)), vec2(0f), vec2(1f)) * 4095f)));
    return ((uv_8.x | (uv_8.y << 12u)) | (u32(round((clamp(roughness_8, 0f, 1f) * 255f))) << 24u));
}
...
fn encodeStandardGBufferX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(normal_21: vec3<f32>, roughness_9: f32, albedo_3: vec3<f32>, metallic_14: f32, f0_6: vec3<f32>, occlusion: f32, emissive: vec3<f32>, opacity: f32, reflection: u32, probeRow_1: u32) -> GBufferOutputX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX {
    let _e5 = encodeStandardNormalRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(normal_21, roughness_9);
    let _e8 = encodeStandardReflectanceX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(f0_6, occlusion);
    let _e11 = encodeStandardReflectanceX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(albedo_3, metallic_14);
...
fn projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX(baseColorAlpha_1: f32, alphaCutoff: f32, baseColorTextureEnabled: bool, baseColorTexture: texture_2d<f32>, baseColorSampler: sampler, transform_1: vec4<f32>, metadata_3: vec4<f32>, currentClip_1: vec4<f32>, previousClip_1: vec4<f32>, temporalProjection_3: vec4<f32>, reactive_3: f32, motionValid_2: bool, uv0_1: vec2<f32>, uv1_1: vec2<f32>, uv2_1: vec2<f32>, uv3_1: vec2<f32>, uv4_1: vec2<f32>, uv5_1: vec2<f32>, uv6_1: vec2<f32>, uv7_1: vec2<f32>) -> vec4<f32> {
    ...
    if (alphaCutoff > 0f) {
        ...
        local_49 = ((baseColorAlpha_1 * _e24) <= alphaCutoff);
    } else {
        local_49 = false;
    }
    let _e30 = local_49;
    ...
    let _e39 = packSceneTemporalV1WithValidityX_naga_oil_mod_XMZXXEZ3FMF4F643DMVXGKX3UMVWXA33SMFWAX(currentClip_1, previousClip_1, temporalProjection_3, _e34, motionValid_2);
    return _e39;
}
...
fn vs_main_impl(in_6: VsIn, meshIndex: u32, instanceIndex: u32, materialAddress: vec3<u32>) -> VsOut {
    var out_1: VsOut;
    ...
    out_1.clip = (_e20 * world);
    out_1.worldPos = world.xyz;
    ...
    out_1.worldNormal = normalize(entityNormal);
    ...
    out_1.color = in_6.color;
    out_1.transmissionBasis0_ = vec4<f32>(localToWorld[0].x, localToWorld[0].y, localToWorld[0].z, localToWorld[1].x);
    out_1.transmissionBasis1_ = vec4<f32>(localToWorld[1].y, localToWorld[1].z, localToWorld[2].x, localToWorld[2].y);
    out_1.materialAddress = materialAddress;
    let clipPos = out_1.clip;
    out_1.ndc = vec4<f32>((clipPos.xy / vec2(clipPos.w)), (clipPos.z / clipPos.w), localToWorld[2].z);
    let _e122 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.temporalProjection;
    let _e123 = sceneViewZX_naga_oil_mod_XMZXXEZ3FMF4F643DMVXGKX3UMVWXA33SMFWAX(clipPos, _e122);
    out_1.viewZ = _e123;
    let _e124 = out_1;
    return _e124;
}
...
fn transformedMaterialUv(transform_2: vec4<f32>, metadata_4: vec4<f32>, in_8: VsOut) -> vec2<f32> {
...
fn evaluate_standard_surface(input_2: SurfaceInput, materialValue: MaterialParameters) -> SurfaceData {
    ...
    normal_22 = normalize(input_2.geometricNormalWS);
    ...
    return SurfaceData(baseColor_11, _e106, metallic_15, roughness_12, emissive_1, occlusion_1, clamp(((materialValue.baseColor.w * _e110) * vertexColor.w), 0f, 1f), clamp(materialValue.alphaCutoff, 0f, 1f));
}
...
fn blendLinearTransparent(source_3: vec3<f32>, destination: vec3<f32>, alpha_2: f32) -> vec3<f32> {
    return ((source_3 * alpha_2) + (destination * (1f - alpha_2)));
}
...
fn alphaTestSurface(surface: SurfaceData) {
    ...
    if (surface.alphaClipThreshold > 0f) {
        local_50 = (surface.opacity <= surface.alphaClipThreshold);
    } else {
        local_50 = false;
    }
    let _e10 = local_50;
...
fn standardSurfaceF0_(in_11: VsOut, surface_1: SurfaceData) -> vec3<f32> {
    ...
    let metallic_16 = clamp(_e4, 0f, 1f);
    ...
    return mix(((vec3(dielectricF0_) * _e36) * _e38), albedo_4, metallic_16);
}
...
fn vs_main(in: VsIn, @builtin(instance_index) idx: u32) -> VsOut {
    let _e7 = vs_main_impl(in, 0u, idx, vec3<u32>(4294967295u, 0u, 0u));
    return _e7;
}
...
fn vs_scene_index(in_1: VsIn, @builtin(instance_index) idx_1: u32) -> VsOut {
    ...
    let _e8 = vs_main_impl(in_1, visible.x, 0u, visible.yzw);
    return _e8;
}
...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let v_5 = normalize((_e26 - in_2.worldPos));
    ...
    let transmittedEnergy = select(0f, ((transmissionFactor * (1f - metallic_17)) * (1f - fresnel_2)), (refractedLengthSquared > 0.000001f));
    diffuseAlbedo = (albedo_5 * (1f - transmittedEnergy));
    let probeBase = (in_2.materialAddress.y * 16u);
    let probeHeader = probeBlendRecords[probeBase];
    if ((u32(probeHeader.x) + 1u) == in_2.materialAddress.y) {
        local = (u32(probeHeader.y) == in_2.materialAddress.z);
    } else {
        local = false;
    }
    let probeIdentityValid = local;
    let _e157 = probeBlendRecords[(probeBase + 1u)];
    let _e162 = probeBlendRecords[(probeBase + 2u)];
    let _e167 = probeBlendRecords[(probeBase + 3u)];
    let _e172 = probeBlendRecords[(probeBase + 4u)];
    let _e177 = probeBlendRecords[(probeBase + 5u)];
    let _e182 = probeBlendRecords[(probeBase + 6u)];
    let _e187 = probeBlendRecords[(probeBase + 7u)];
    let _e192 = probeBlendRecords[(probeBase + 8u)];
    let _e197 = probeBlendRecords[(probeBase + 9u)];
    probeShPreblend = array<vec4<f32>, 9>(_e157, _e162, _e167, _e172, _e177, _e182, _e187, _e192, _e197);
    let _e203 = probeBlendRecords[probeBase].z;
    probeLocalBlendFraction = select(0f, _e203, probeIdentityValid);
    let _e208 = physicalNormal;
    let _e209 = diffuseAlbedo;
    let _e210 = f0_;
    let _e212 = skylight;
    let _e213 = probeShPreblend;
    let _e214 = probeLocalBlendFraction;
    let _e221 = evaluateStandardEnvironmentX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_2.worldPos, _e208, v_5, _e209, metallic_17, iblRoughness, _e210, _e212, irradianceMap_2, irradianceSampler_2, prefilterMap_2, prefilterSampler_2, brdfLut_4, skylightPrefilterMap_1, _e213, _e214);
    let ao = _e6.occlusion;
    let _e225 = skylight.colorR;
    let _e228 = skylight.colorG;
    let _e231 = skylight.colorB;
    let _e235 = skylight.intensity;
    let _e236 = decodeSpecularEnvironmentScaleX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(vec3<f32>(_e225, _e228, _e231), _e235);
    let _e243 = coatF;
    ambient = (((_e221.diffuse + _e221.specular) * (vec3(1f) - _e243)) * ao);
    let _e251 = coatF;
    reflectionFallback = ((_e221.specular * (vec3(1f) - _e251)) * ao);
    let _e259 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.worldViewProj;
    let _e262 = sampleStandardAmbientOcclusionX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_2.worldPos, _e259, ssaoBlurredTexture, ssaoBlurredSampler);
    let _e263 = ambient;
    ambient = (_e263 * _e262);
    let _e265 = reflectionFallback;
    reflectionFallback = (_e265 * _e262);
    let _e267 = ambient;
    color = _e267;
    ...
    let _e340 = material.thickness;
    let _e342 = finiteScalar(_e340, 0f);
    let _e345 = thicknessSample;
    let _e347 = finiteScalar(_e345, 1f);
    let worldThickness = max(((_e342 * length(worldRefractedDirection)) * _e347), 0f);
    let refractedUv = (screenUv + (((refracted.xy - incident.xy) * worldThickness) * 0.25f));
    if all((refractedUv >= vec2(0.02f))) {
        local_1 = all((refractedUv <= vec2((1f - 0.02f))));
    } else {
        local_1 = false;
    }
    let insideGuardBand = local_1;
    ...
    let _e389 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(screenUv, vec2(0f), vec2(1f)), backdropLod);
    let unrefractedBackdrop = _e389.xyz;
    transmittedBackdrop = unrefractedBackdrop;
    if (refractedLengthSquared > 0.000001f) {
        local_2 = insideGuardBand;
    } else {
        local_2 = false;
    }
    let _e397 = local_2;
    if _e397 {
        let _e405 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(refractedUv, vec2(0f), vec2(1f)), backdropLod);
        transmittedBackdrop = _e405.xyz;
    }
    let _e409 = material.attenuationColor;
    let _e412 = finiteColor(_e409, vec3(1f));
    let safeAttenuationColor = clamp(_e412, vec3(0f), vec3(1f));
    let _e420 = material.attenuationDistance;
    let _e422 = finiteScalar(_e420, 0f);
    let safeAttenuationDistance = max(_e422, 0f);
    let attenuationExponent = (worldThickness / max(safeAttenuationDistance, 0.000001f));
    if (safeAttenuationDistance > 0.000001f) {
        local_3 = (worldThickness > 0f);
    } else {
        local_3 = false;
    }
    let _e439 = local_3;
    let beerAttenuation = select(vec3(1f), pow(safeAttenuationColor, vec3(attenuationExponent)), _e439);
    let _e441 = color;
    let _e442 = transmittedBackdrop;
    let _e445 = finiteColor(_e442, vec3(0f));
    color = (_e441 + ((_e445 * transmittedEnergy) * beerAttenuation));
    let _e449 = physicalNormal;
    let _e451 = standardViewZ(in_2);
    let _e452 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e449, in_2.worldPos, _e451);
    let _e453 = color;
    let _e457 = standardViewZ(in_2);
    let _e458 = physicalNormal;
    let _e459 = diffuseAlbedo;
    let _e460 = f0_;
    let _e461 = evaluateStandardDirectX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_2.worldPos, in_2.ndc.xyz, _e457, _e458, v_5, _e459, metallic_17, a_4, _e460, _e452);
    color = (_e453 + _e461);
    let _e463 = color;
    color = (_e463 + _e6.emissive);
    let _e468 = color;
    output.color = vec4<f32>(_e468, alpha_3);
    let _e471 = reflectionFallback;
    let _e472 = standardSsrCoverage();
    output.reflectionFallback = vec4<f32>(_e471, _e472);
    let _e474 = output;
    return _e474;
}
...
fn fs_gbuffer(in_3: VsOut, @builtin(front_facing) frontFacing_1: bool) -> GBufferOutputX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX {
    ...
    let base_1 = (in_3.materialAddress.y * 16u);
    let header = probeBlendRecords[base_1];
    if ((u32(header.x) + 1u) == in_3.materialAddress.y) {
        local_4 = (u32(header.y) == in_3.materialAddress.z);
    } else {
        local_4 = false;
    }
    let _e26 = local_4;
    ...
    let _e33 = local_5;
    if _e33 {
        probeRow = in_3.materialAddress.y;
    }
    let _e47 = standardSurfaceF0_(in_3, _e3);
    let _e54 = skylight.diffuseScale.w;
    let _e56 = probeRow;
    let _e57 = encodeStandardGBufferX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(_e3.normalWS, clamp(_e3.roughness, 0.04f, 1f), _e3.baseColor, clamp(_e3.metallic, 0f, 1f), _e47, _e3.occlusion, _e3.emissive, _e3.opacity, u32(_e54), _e56);
    return _e57;
}
...
fn fs_temporal(in_5: TemporalVsOut) -> @location(0) vec4<f32> {
    ...
    let _e6 = meshesX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX[0].temporal.x;
    reactive = _e6;
    let _e12 = meshesX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX[0].temporal.y;
    motionValid = (_e12 >= 0.5f);
    let _e19 = material.baseColor.w;
    let _e21 = temporalVertexAlpha(in_5);
    let _e25 = material.alphaCutoff;
    let _e26 = standardUsesBaseColorTexture();
    let _e29 = material.baseColorTextureCoordinatesTransform;
    let _e32 = material.baseColorTextureCoordinatesMetadata;
    let _e37 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.temporalProjection;
    let _e38 = reactive;
    let _e39 = motionValid;
    let _e50 = projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX((_e19 * _e21), _e25, _e26, baseColorTexture_1, baseColorTexture_sampler, _e29, _e32, in_5.currentClip, in_5.previousClip, _e37, _e38, _e39, in_5.uv, in_5.uv1_, in_5.uv2_, in_5.uv3_, in_5.uv4_, in_5.uv5_, in_5.uv6_, in_5.uv7_);
    return _e50;
}
