...
每个 Marketplace **模块各自独立**一个目录，**不是**都传到 UI 工坊。
...
| 模块类型 | slug 规则 | 示例 |
|----------|-----------|------|
| 工作台 | 插件目录名 | `wb-ui`、`wb-reel`、`wb-character`、`wb-gen3d` |
...
**用法**：文件夹名 = slug（如 `wb-reel/`），粘贴后只改 `slug = ...` 一行。

```
我已附上 Marketplace 预览素材文件夹，请按规范提交到 forgeax-website。

【模块】
- slug = wb-reel          ← 改成你的模块，如 wb-character / agent-reia / wb-gen3d
...
3. 按规范命名并放入 assets/marketplace/previews/{slug}/：
   - 01-studio-main.png     → category: studio（工作台 / Studio 界面）
...
4. panelLabel 按模块填写（不是固定 UI）：
   - wb-ui → UI；wb-reel → REEL；wb-character → CHARACTER
   - agent-* → CHAT；其他 wb-* → 去掉 wb- 前缀大写
...
- slug：wb-character          ← 改成你的模块
- 目标：assets/marketplace/previews/wb-character/01-studio-main.png
- 若宽度 <1600px 先警告我
- preview-meta.json 里 category / panelLabel 按该模块填写（Agent 用 chat+CHAT，工作台用 studio+对应 WB 名）
