...
fn sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(normal_3: vec3<f32>, view_2: vec3<f32>, roughness_3: f32, F0_3: vec3<f32>, worldPosition_1: vec3<f32>, boxCenter_1: vec3<f32>, boxExtents_1: vec3<f32>, rotation_3: vec4<f32>, probeMap: texture_cube<f32>, probeSampler: sampler, brdfLut_2: texture_2d<f32>, brdfLutSampler_2: sampler, skylightMap: texture_cube<f32>, skylightRotation: vec4<f32>, skylightScale: vec3<f32>, probeIntensity: f32, boxProjection: bool) -> vec3<f32> {
    ...
    let local_47 = (_e55 * probeIntensity);
    let _e58 = sky;
    return mix(_e58, local_47, weight);
}
...
fn evaluateProbeDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX(shPreblend_1: array<vec4<f32>, 9>, localBlendFraction: f32, normal_5: vec3<f32>, e_sky: vec3<f32>, k_d: vec3<f32>, albedo: vec3<f32>, metallic: f32) -> vec3<f32> {
    ...
    let local_48 = (max(_e2, vec3(0f)) * PROBE_INV_PIX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX);
    let skyResidualFraction = (1f - localBlendFraction);
    return ((((local_48 + (skyResidualFraction * e_sky)) * k_d) * albedo) * (1f - metallic));
}
...
fn evaluate_cluster_lightX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(light: DirectLightSlotX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX, world_pos_1: vec3<f32>, normal_11: vec3<f32>, view_dir: vec3<f32>, base_color: vec3<f32>, metallic_8: f32, alpha_sq: f32, f0_2: vec3<f32>, flat_2d: bool, receive_shadows: bool) -> vec3<f32> {
    ...
    if (kind == KIND_SPOTX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX) {
        ...
        tile = encoded_tile;
        if (encoded_tile >= 0i) {
            local_19 = ((encoded_tile & PROJECTOR_FLAGX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX) != 0i);
        } else {
            local_19 = false;
        }
        let _e70 = local_19;
...
fn _samplePcssForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_11: vec3<f32>, layer_5: u32, count_4: u32, normal_16: vec3<f32>, l_1: vec3<f32>, profile: u32) -> f32 {
    ...
    if (tileUv.x >= 0f) {
        local_27 = (tileUv.x <= 1f);
    } else {
        local_27 = false;
    }
    let _e46 = local_27;
    if _e46 {
        local_28 = (tileUv.y >= 0f);
    } else {
        local_28 = false;
    }
    let _e53 = local_28;
    if _e53 {
        local_29 = (tileUv.y <= 1f);
    } else {
        local_29 = false;
    }
    let _e60 = local_29;
    if _e60 {
        local_30 = (projCoords.z >= 0f);
    } else {
        local_30 = false;
    }
    let _e67 = local_30;
    if _e67 {
        local_31 = (projCoords.z <= 1f);
    } else {
        local_31 = false;
    }
    let _e74 = local_31;
    ...
    if (lightDepthWorldSpan > 0f) {
        local_35 = (worldUnitsPerTexel > 0f);
    } else {
        local_35 = false;
    }
    let _e213 = local_35;
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_12: vec3<f32>, layer_6: u32, count_5: u32, normal_17: vec3<f32>, l_2: vec3<f32>) -> f32 {
    ...
    if (tileUv_1.x >= 0f) {
        local_38 = (tileUv_1.x <= 1f);
    } else {
        local_38 = false;
    }
    let _e40 = local_38;
    if _e40 {
        local_39 = (tileUv_1.y >= 0f);
    } else {
        local_39 = false;
    }
    let _e47 = local_39;
    if _e47 {
        local_40 = (tileUv_1.y <= 1f);
    } else {
        local_40 = false;
    }
    let _e54 = local_40;
    if _e54 {
        local_41 = (currentDepth <= 1f);
    } else {
        local_41 = false;
    }
    let _e60 = local_41;
    ...
    if (kernel == 3u) {
        if all((uv_7 >= (tileLo + texel_5))) {
            local_42 = all((uv_7 <= (tileHi - texel_5)));
        } else {
            local_42 = false;
        }
        let interior = local_42;
        if interior {
...
fn projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX(baseColorAlpha_1: f32, alphaCutoff: f32, alphaHash: f32, positionOS: vec3<f32>, baseColorTextureEnabled: bool, baseColorTexture: texture_2d<f32>, baseColorSampler: sampler, transform_1: vec4<f32>, metadata_3: vec4<f32>, alphaTextureEnabled: bool, alphaTexture: texture_2d<f32>, alphaSampler: sampler, alphaTransform: vec4<f32>, alphaMetadata: vec4<f32>, alphaChannel: f32, currentClip_1: vec4<f32>, previousClip_1: vec4<f32>, temporalProjection_3: vec4<f32>, reactive_3: f32, motionValid_2: bool, uv0_1: vec2<f32>, uv1_1: vec2<f32>, uv2_1: vec2<f32>, uv3_1: vec2<f32>, uv4_1: vec2<f32>, uv5_1: vec2<f32>, uv6_1: vec2<f32>, uv7_1: vec2<f32>) -> vec4<f32> {
    ...
    if (alphaCutoff > 0f) {
        local_44 = (coverage_2 <= alphaCutoff);
    } else {
        local_44 = false;
    }
    let _e49 = local_44;
...
fn alphaTestSurface(surface: SurfaceData) {
    ...
    if (surface.alphaClipThreshold > 0f) {
        local_46 = (surface.opacity <= surface.alphaClipThreshold);
    } else {
        local_46 = false;
    }
    let _e10 = local_46;
...
fn fs_main(in_1: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let v_2 = normalize((_e26 - in_1.worldPos));
    ...
    let _e200 = evaluateStandardEnvironmentX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_1.worldPos, _e187, v_2, _e188, metallic_15, iblRoughness, _e189, _e191, irradianceMap_2, irradianceSampler_2, prefilterMap_2, prefilterSampler_2, brdfLut_4, skylightPrefilterMap_1, _e192, _e193);
    ...
    let _e368 = textureSampleLevel(transmissionBackdropTexture, prefilterSampler_2, clamp(screenUv, vec2(0f), vec2(1f)), backdropLod);
    ...
    let _e376 = local_1;
    if _e376 {
        let _e384 = textureSampleLevel(transmissionBackdropTexture, prefilterSampler_2, clamp(refractedUv, vec2(0f), vec2(1f)), backdropLod);
    ...
    let _e439 = f0_;
    let _e440 = evaluateStandardDirectX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_1.worldPos, in_1.ndc.xyz, _e436, _e437, v_2, _e438, metallic_15, a_4, _e439, _e431);
    color = (_e432 + _e440);
    let _e442 = color;
    color = (_e442 + _e6.emissive);
    let _e447 = color;
    output.color = vec4<f32>(_e447, alpha_5);
...
fn fs_temporal(in_4: TemporalVsOut) -> @location(0) vec4<f32> {
    ...
    let _e23 = temporalVertexAlpha(in_4);
    ...
    let _e68 = projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX((_e22 * _e23), _e27, _e30, in_4.positionOS, _e32, baseColorTexture_1, baseColorTexture_sampler, _e35, _e38, _e39, alphaTexture_1, alphaTexture_sampler, _e42, _e45, _e48, in_4.currentClip, in_4.previousClip, _e53, _e54, _e55, in_4.uv, in_4.uv1_, in_4.uv2_, in_4.uv3_, in_4.uv4_, in_4.uv5_, in_4.uv6_, in_4.uv7_);
    return _e68;
}
