...
fn prefilterEnv_fs(in0_1: CubemapVsOut) -> @location(0) vec4<f32> {
    ...
    loop {
        ...
        }
        {
            ...
            if (NdotL > 0f) {
                ...
                let _e81 = textureSampleLevel(envCube, envSamplerS, L, mipLevel);
                prefilteredColor = (_e78 + (_e81.xyz * NdotL));
                let _e86 = totalWeight;
                totalWeight = (_e86 + NdotL);
            }
        }
        continuing {
            let _e88 = i;
    ...
    let _e91 = prefilteredColor;
    let _e92 = totalWeight;
    prefilteredColor = (_e91 / vec3(max(_e92, 0.001f)));
    let _e97 = prefilteredColor;
    return vec4<f32>(_e97, 1f);
}
