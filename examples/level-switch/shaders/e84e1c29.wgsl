...
fn fs_main(in_1: VsOut) -> @location(0) vec4<f32> {
    ...
    let albedo = (_e21.xyz * _e12.xyz);
    let _e27 = material.metallicRoughnessCoordinates;
    let _e28 = transformedMaterialUv(_e27, in_1);
    ...
    let _e60 = a;
    a = (_e59 * _e60);
    let _e64 = material.normalCoordinates;
    let _e65 = transformedMaterialUv(_e64, in_1);
    let _e69 = material.normalCoordinates.metadata;
    let _e73 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(normalTexture, normalSampler, _e65, _e69.zw);
    let normSampleRg = _e73.xy;
    let _e75 = decodeTangentSpaceNormalRgX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DCNYX(normSampleRg);
    let _e78 = material.normalScale;
    let _e79 = scaleTangentSpaceNormalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DCNYX(_e75, _e78);
    let _e82 = applyTBNX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DCNYX(in_1.worldNormal, in_1.worldTangent, _e79);
    let _e85 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cameraPos;
    let v = normalize((_e85 - in_1.worldPos));
    let _e91 = material.specularTintCoordinates;
    let _e92 = transformedMaterialUv(_e91, in_1);
    let _e95 = material.specularTint;
    let _e99 = material.specularTintCoordinates.metadata;
    let _e103 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(specularTintTexture, specularTintSampler, _e92, _e99.zw);
    let specularTint = (_e95 * _e103.xyz);
    let f0_1 = mix((vec3(0.04f) * specularTint), albedo, metallic_5);
    let _e112 = material.clearcoatRoughness;
    let coatRoughness = max(_e112, 0.04f);
    let coatAlpha = (coatRoughness * coatRoughness);
    let _e121 = f_schlickX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(max(dot(_e82, v), 0f), vec3(0.04f));
    let _e124 = material.clearcoat;
    let coatF = (_e121 * _e124);
    let _e131 = f_schlickX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(max(dot(_e82, v), 0f), f0_1);
    let kD_2 = ((vec3(1f) - _e131) * (1f - metallic_5));
    let _e138 = material.roughness;
    let iblRoughness = (max(_e138, 0.04f) * _e50);
    let _e144 = skylight.rotation;
    let _e147 = sampleIblDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e82, _e144, irradianceMap_1, irradianceSampler_1);
    let _e150 = skylight.rotation;
    let _e155 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e82, v, iblRoughness, f0_1, _e150, prefilterMap_1, prefilterSampler_1, brdfLut_1, brdfLutSampler_1);
    let _e160 = skylight.rotation;
    let _e165 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e82, v, coatRoughness, vec3(0.04f), _e160, prefilterMap_1, prefilterSampler_1, brdfLut_1, brdfLutSampler_1);
    let _e168 = material.occlusionCoordinates;
    let _e169 = transformedMaterialUv(_e168, in_1);
    let _e173 = material.occlusionCoordinates.metadata;
    let _e177 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(occlusionTexture, occlusionSampler, _e169, _e173.zw);
    let _e181 = material.occlusionStrength;
    let ao = mix(1f, _e177.x, _e181);
    let _e186 = skylight.colorR;
    let _e189 = skylight.colorG;
    let _e192 = skylight.colorB;
    let skyColor = vec3<f32>(_e186, _e189, _e192);
    let _e203 = material.clearcoat;
    let _e209 = skylight.intensity;
    ambient = ((((((((kD_2 * _e147) * albedo) + _e155) * (vec3(1f) - coatF)) + (_e165 * _e203)) * skyColor) * _e209) * ao);
    let ssaoUv = ((in_1.ndc.xy * vec2<f32>(0.5f, -0.5f)) + vec2<f32>(0.5f, 0.5f));
    let _e225 = textureSample(ssaoBlurredTexture, ssaoBlurredSampler, ssaoUv);
    let ssaoFactor = _e225.x;
    let _e227 = get_ssao_intensityX_naga_oil_mod_XMZXXEZ3FMF4F62DEOJYDUOTDNR2XG5DFOJPWM33SO5QXEZAX();
    let _e228 = ambient;
    ambient = (_e228 * mix(1f, (ssaoFactor * ao), _e227));
    let _e233 = ambient;
    color = _e233;
    let _e235 = color;
    let _e236 = a;
    let _e239 = evalDirectionalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e82, v, albedo, metallic_5, _e236, f0_1, in_1.worldPos, in_1.viewZ);
    color = (_e235 + _e239);
    let _e241 = color;
    let _e244 = material.clearcoat;
    let _e252 = evalDirectionalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e82, v, vec3(0f), 1f, coatAlpha, vec3(0.04f), in_1.worldPos, in_1.viewZ);
    color = (_e241 + (_e244 * _e252));
    let _e255 = color;
    let _e259 = a;
    let _e260 = evaluate_cluster_lightsX_naga_oil_mod_XMZXXEZ3FMF4F62DEOJYDUOTDNR2XG5DFOJPWM33SO5QXEZAX(in_1.ndc, in_1.viewZ, in_1.worldPos, _e82, v, albedo, metallic_5, _e259);
    color = (_e255 + _e260);
    let _e262 = color;
    let _e265 = material.clearcoat;
    let _e272 = evaluate_cluster_lightsX_naga_oil_mod_XMZXXEZ3FMF4F62DEOJYDUOTDNR2XG5DFOJPWM33SO5QXEZAX(in_1.ndc, in_1.viewZ, in_1.worldPos, _e82, v, vec3(0f), 1f, coatRoughness);
    color = (_e262 + (_e265 * _e272));
    let _e277 = material.emissiveCoordinates;
    let _e278 = transformedMaterialUv(_e277, in_1);
    let _e282 = material.emissiveCoordinates.metadata;
    let _e286 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(emissiveTexture, emissiveSampler, _e278, _e282.zw);
    let emissiveSample = _e286.xyz;
    let _e288 = color;
    let _e291 = material.emissive;
    let _e294 = material.emissiveIntensity;
    color = (_e288 + ((_e291 * _e294) * emissiveSample));
    let _e298 = color;
    let _e302 = material.baseColor.w;
    return vec4<f32>(_e298, (_e302 * _e12.w));
}
...
fn fs_gbuffer(in_2: VsOut) -> GBufferOutput {
    ...
    let _e75 = material.normalScale;
    let _e76 = scaleTangentSpaceNormalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DCNYX(_e72, _e75);
    let _e79 = applyTBNX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DCNYX(in_2.worldNormal, in_2.worldTangent, _e76);
    let _e82 = material.emissiveCoordinates;
    let _e83 = transformedMaterialUv(_e82, in_2);
    let _e87 = material.emissiveCoordinates.metadata;
    let _e91 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(emissiveTexture, emissiveSampler, _e83, _e87.zw);
    let emissiveSample_1 = _e91.xyz;
    let _e95 = material.emissive;
    let _e98 = material.emissiveIntensity;
    let emissive = ((_e95 * _e98) * emissiveSample_1);
    let _e103 = material.occlusionCoordinates;
    let _e104 = transformedMaterialUv(_e103, in_2);
    let _e108 = material.occlusionCoordinates.metadata;
    let _e112 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(occlusionTexture, occlusionSampler, _e104, _e108.zw);
    let _e116 = material.occlusionStrength;
    let ao_1 = mix(1f, _e112.x, _e116);
    let _e126 = a_1;
    out_1.normal_roughness = vec4<f32>(((_e79 * 0.5f) + vec3(0.5f)), _e126);
