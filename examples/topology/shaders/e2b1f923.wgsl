...
fn fs_main(in_1: VsOut) -> @location(0) vec4<f32> {
    ...
    let _e78 = applyTBNX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DCNYX(in_1.worldNormal, in_1.worldTangent, _e75);
    let _e81 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cameraPos;
    let v = normalize((_e81 - in_1.worldPos));
    let _e87 = material.specularTintCoordinates;
    let _e88 = transformedMaterialUv(_e87, in_1);
    let _e91 = material.specularTint;
    let _e95 = material.specularTintCoordinates.metadata;
    let _e99 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(specularTintTexture, specularTintSampler, _e88, _e95.zw);
    let specularTint = (_e91 * _e99.xyz);
    let f0_1 = mix((vec3(0.04f) * specularTint), albedo, metallic_5);
    let _e108 = material.clearcoatRoughness;
    let coatRoughness = max(_e108, 0.04f);
    let coatAlpha = (coatRoughness * coatRoughness);
    let _e117 = f_schlickX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(max(dot(_e78, v), 0f), vec3(0.04f));
    let _e120 = material.clearcoat;
    let coatF = (_e117 * _e120);
    let _e127 = f_schlickX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(max(dot(_e78, v), 0f), f0_1);
    let kD_2 = ((vec3(1f) - _e127) * (1f - metallic_5));
    let _e134 = material.roughness;
    let iblRoughness = (max(_e134, 0.04f) * _e50);
    let _e140 = skylight.rotation;
    let _e143 = sampleIblDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e78, _e140, irradianceMap_1, irradianceSampler_1);
    let _e146 = skylight.rotation;
    let _e151 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e78, v, iblRoughness, f0_1, _e146, prefilterMap_1, prefilterSampler_1, brdfLut_1, brdfLutSampler_1);
    let _e156 = skylight.rotation;
    let _e161 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e78, v, coatRoughness, vec3(0.04f), _e156, prefilterMap_1, prefilterSampler_1, brdfLut_1, brdfLutSampler_1);
    let _e164 = material.occlusionCoordinates;
    let _e165 = transformedMaterialUv(_e164, in_1);
    let _e169 = material.occlusionCoordinates.metadata;
    let _e173 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(occlusionTexture, occlusionSampler, _e165, _e169.zw);
    let _e177 = material.occlusionStrength;
    let ao = mix(1f, _e173.x, _e177);
    let _e182 = skylight.colorR;
    let _e185 = skylight.colorG;
    let _e188 = skylight.colorB;
    let skyColor = vec3<f32>(_e182, _e185, _e188);
    let _e199 = material.clearcoat;
    let _e205 = skylight.intensity;
    ambient = ((((((((kD_2 * _e143) * albedo) + _e151) * (vec3(1f) - coatF)) + (_e161 * _e199)) * skyColor) * _e205) * ao);
    let ssaoUv = ((in_1.ndc.xy * vec2<f32>(0.5f, -0.5f)) + vec2<f32>(0.5f, 0.5f));
    let _e221 = textureSample(ssaoBlurredTexture, ssaoBlurredSampler, ssaoUv);
    let ssaoFactor = _e221.x;
    let _e223 = get_ssao_intensityX_naga_oil_mod_XMZXXEZ3FMF4F62DEOJYDUOTDNR2XG5DFOJPWM33SO5QXEZAX();
    let _e224 = ambient;
    ambient = (_e224 * mix(1f, (ssaoFactor * ao), _e223));
    let _e229 = ambient;
    color = _e229;
    let _e231 = color;
    let _e232 = a;
    let _e235 = evalDirectionalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e78, v, albedo, metallic_5, _e232, f0_1, in_1.worldPos, in_1.viewZ);
    color = (_e231 + _e235);
    let _e237 = color;
    let _e240 = material.clearcoat;
    let _e248 = evalDirectionalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e78, v, vec3(0f), 1f, coatAlpha, vec3(0.04f), in_1.worldPos, in_1.viewZ);
    color = (_e237 + (_e240 * _e248));
    let _e251 = color;
    let _e255 = a;
    let _e256 = evaluate_cluster_lightsX_naga_oil_mod_XMZXXEZ3FMF4F62DEOJYDUOTDNR2XG5DFOJPWM33SO5QXEZAX(in_1.ndc, in_1.viewZ, in_1.worldPos, _e78, v, albedo, metallic_5, _e255);
    color = (_e251 + _e256);
    let _e258 = color;
    let _e261 = material.clearcoat;
    let _e268 = evaluate_cluster_lightsX_naga_oil_mod_XMZXXEZ3FMF4F62DEOJYDUOTDNR2XG5DFOJPWM33SO5QXEZAX(in_1.ndc, in_1.viewZ, in_1.worldPos, _e78, v, vec3(0f), 1f, coatRoughness);
    color = (_e258 + (_e261 * _e268));
    let _e273 = material.emissiveCoordinates;
    let _e274 = transformedMaterialUv(_e273, in_1);
    let _e278 = material.emissiveCoordinates.metadata;
    let _e282 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(emissiveTexture, emissiveSampler, _e274, _e278.zw);
    let emissiveSample = _e282.xyz;
    let _e284 = color;
    let _e287 = material.emissive;
    let _e290 = material.emissiveIntensity;
    color = (_e284 + ((_e287 * _e290) * emissiveSample));
    let _e294 = color;
    let _e298 = material.baseColor.w;
    return vec4<f32>(_e294, (_e298 * _e12.w));
}
