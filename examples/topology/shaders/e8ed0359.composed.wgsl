...
fn applySceneFog(viewParams: ViewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, color_1: vec3<f32>, alpha: f32, worldPos_3: vec3<f32>) -> vec4<f32> {
    var origin: vec3<f32>;
    ...
    let _e4 = origin;
    direction = normalize((worldPos_3 - _e4));
    let _e8 = origin;
    rayDistance = length((worldPos_3 - _e8));
    if (viewParams.temporalProjection.z >= 0.5f) {
        ...
        let _e40 = direction;
        let _e43 = direction;
        origin = (worldPos_3 - (_e40 * dot((worldPos_3 - viewParams.cameraPos), _e43)));
        let _e47 = origin;
        let _e49 = direction;
        rayDistance = max(dot((worldPos_3 - _e47), _e49), 0f);
    }
    let _e56 = origin;
    let _e57 = direction;
...
fn fs_main(in_1: VsOut) -> @location(0) vec4<f32> {
    ...
    let _e15 = spriteLitShadeAccum(albedo4_.xyz, in_1.worldPos);
    let lit_rgba = clamp(vec4<f32>(_e15, albedo4_.w), vec4(0f), vec4(1f));
    let _e24 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX;
    let _e28 = applySceneFog(_e24, lit_rgba.xyz, lit_rgba.w, in_1.worldPos);
    let premult = vec4<f32>((_e28.xyz * _e28.w), _e28.w);
    let _e35 = linear_to_srgb(premult.x);
    let _e37 = linear_to_srgb(premult.y);
    let _e39 = linear_to_srgb(premult.z);
    return vec4<f32>(_e35, _e37, _e39, premult.w);
}
...
fn fs_main_hdr(in_2: VsOut) -> @location(0) vec4<f32> {
    ...
    let _e15 = spriteLitShadeAccum(albedo4_1.xyz, in_2.worldPos);
    let alpha_1 = clamp(albedo4_1.w, 0f, 1f);
    let _e21 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX;
    let _e23 = applySceneFog(_e21, _e15, alpha_1, in_2.worldPos);
...
fn fs_temporal(in_4: TemporalVsOut) -> @location(0) vec4<f32> {
    ...
    let alpha_2 = clamp((_e8.w * _e13), 0f, 1f);
    if (alpha_2 <= 0f) {
        discard;
    }
