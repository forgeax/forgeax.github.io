...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_1: vec3<f32>, layer_5: u32, count_5: u32, normal_8: vec3<f32>, l_1: vec3<f32>, useNormalBias: bool) -> f32 {
    ...
    if (kernel == 3u) {
        if all((uv_4 >= (tileLo + texel_3))) {
            local_19 = all((uv_4 <= (tileHi - texel_3)));
        } else {
            local_19 = false;
        }
...
fn evaluate_standard_surface(input_2: SurfaceInput, materialValue: MaterialParameters) -> SurfaceData {
    ...
    normal_11 = normalize(input_2.geometricNormalWS);
...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> @location(0) vec4<f32> {
    ...
    f0_ = mix(((vec3(dielectricF0_) * _e59) * _e61), albedo_2, metallic_4);
    physicalNormal = n;
    ...
    let _e115 = finiteScalar(dot(n, v), 0f);
    ...
    let _e155 = f0_;
    let _e156 = f_schlickX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(max(dot(_e151, v), 0f), _e155);
    let kD_1 = ((vec3(1f) - _e156) * (1f - metallic_4));
    ...
    if (_e168 < 0f) {
        ...
        let _e197 = sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e171, v, iblRoughness, _e172, in_2.worldPos, vec3<f32>(_e176, _e179, _e182), _e186.xyz, vec4<f32>(0f, 0f, 0f, 1f), prefilterMap_1, prefilterSampler_1, brdfLut_3, brdfLutSampler_3);
        specularIbl = _e197;
    } else {
        ...
        let _e216 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e207, v, iblRoughness, _e208, _e211, prefilterMap_1, prefilterSampler_1, brdfLut_3, brdfLutSampler_3);
        specularIbl = _e216;
    }
    ...
    let probeDiffuseK = (kD_1 / max(vec3((1f - metallic_4)), vec3(0.0001f)));
    let _e288 = physicalNormal;
    let _e289 = irradiance;
    let _e291 = diffuseAlbedo;
    let _e292 = composeProbeDiffuse(probeShPreblend, probeLocalBlendFraction, _e288, (_e289 * skyFactor), probeDiffuseK, _e291, metallic_4);
    ...
    let _e494 = f0_;
    let _e495 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e492, v, _e493, metallic_4, a_2, _e494);
    let _e496 = color;
