...
fn vs_main_impl(in_5: VsIn, meshIndex: u32, instanceIndex: u32, materialIndex: u32) -> VsOut {
    ...
    out_2.positionOSAndViewZ = vec4<f32>(in_5.pos, _e29);
...
fn fs_main(in_1: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    if (_e67 < 0f) {
        ...
        let _e96 = sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e70, v_2, iblRoughness, _e71, in_1.worldPos, vec3<f32>(_e75, _e78, _e81), _e85.xyz, vec4<f32>(0f, 0f, 0f, 1f), prefilterMap_1, prefilterSampler_1, brdfLut_3, brdfLutSampler_3);
        specularIbl = _e96;
    } else {
    ...
    let _e198 = composeProbeDiffuse(probeShPreblend, probeLocalBlendFraction, _e194, (_e195 * skyFactor), probeDiffuseK, _e197, metallic_13);
    ...
    let _e224 = textureSample(ssaoBlurredTexture, ssaoBlurredSampler, ssaoUv);
    let ssaoFactor = _e224.x;
    let _e226 = get_ssao_intensityX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX();
    let _e227 = ambient;
    ambient = (_e227 * mix(1f, (ssaoFactor * ao), _e226));
    let ssaoScale = mix(1f, (ssaoFactor * ao), _e226);
    let _e235 = reflectionFallback;
    reflectionFallback = (_e235 * ssaoScale);
    let _e237 = ambient;
    color = _e237;
    let _e239 = physicalNormal;
    let _e241 = standardViewZ(in_1);
    let _e242 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e239, in_1.worldPos, _e241);
    let _e243 = physicalNormal;
    let _e244 = diffuseAlbedo;
    let _e245 = f0_;
    let _e246 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e243, v_2, _e244, metallic_13, a_3, _e245);
    let _e247 = color;
    color = (_e247 + (_e242 * _e246));
    let _e250 = color;
    let _e253 = standardViewZ(in_1);
    let _e255 = physicalNormal;
    let _e256 = diffuseAlbedo;
    let _e257 = f0_;
    let _e260 = evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_1.ndc.xyz, _e253, in_1.worldPos, _e255, v_2, _e256, metallic_13, a_3, _e257, false, true);
    color = (_e250 + _e260);
    let _e262 = color;
    color = (_e262 + _e6.emissive);
    let _e267 = color;
    output.color = vec4<f32>(_e267, alpha_1);
    let _e270 = reflectionFallback;
    let _e271 = standardSsrCoverage();
    output.reflectionFallback = vec4<f32>(_e270, _e271);
    let _e273 = output;
    return _e273;
}
