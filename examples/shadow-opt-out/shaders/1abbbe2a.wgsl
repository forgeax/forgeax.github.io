...
fn _directionalReceiverDepthBiasX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(layer_3: u32, normal_7: vec3<f32>, l: vec3<f32>, radius: f32) -> f32 {
    var local: bool;
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos: vec3<f32>, layer_4: u32, count_4: u32, normal_8: vec3<f32>, l_1: vec3<f32>) -> f32 {
    var local_1: bool;
    var local_2: bool;
    var local_3: bool;
    var local_4: bool;
    ...
    let _e102 = _directionalReceiverDepthBiasX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(layer_4, normal_8, l_1, f32(((kernel / 2u) + 1u)));
            ...
            }
            {
                ...
                loop {
                    ...
                    }
                    {
                        let _e207 = x;
                        let _e209 = y;
                        let offsetUv = clamp((uv_3 + (vec2<f32>(f32(_e207), f32(_e209)) * texel)), tileLo, tileHi);
        ...
        }
        {
            ...
            loop {
                ...
                }
                {
                    let _e243 = x_1;
                    let _e245 = y_1;
                    let offsetUv_1 = clamp((uv_3 + (vec2<f32>(f32(_e243), f32(_e245)) * texel)), tileLo, tileHi);
...
fn evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_9: vec3<f32>, worldPos_1: vec3<f32>, viewZ: f32) -> f32 {
    var shadow: f32;
    var local_6: bool;
    ...
    let receiverPosition = (worldPos_1 + (normal_9 * _e33));
    let _e36 = _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(receiverPosition, _e28, count_5, normal_9, l_3);
    shadow = _e36;
    ...
    if _e49 {
        ...
        if (blendWidth > 0f) {
            ...
            if (t_1 > 0f) {
                let _e72 = _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(receiverPosition, (_e28 + 1u), count_5, normal_9, l_3);
...
fn cloud_direct_solar_factorX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX(worldPosition_2: vec3<f32>, shadowOrigin: vec3<f32>, shadowRight: vec3<f32>, shadowUp: vec3<f32>, shadowProjection: vec4<f32>) -> f32 {
    ...
    if (worldPosition_2.x == worldPosition_2.x) {
        local_7 = (worldPosition_2.y == worldPosition_2.y);
    } else {
        local_7 = false;
    }
    let _e10 = local_7;
    if _e10 {
        local_8 = (worldPosition_2.z == worldPosition_2.z);
    } else {
        local_8 = false;
    }
    let _e17 = local_8;
    if _e17 {
        local_9 = all((abs(worldPosition_2) < vec3(3.402823e38f)));
    } else {
        local_9 = false;
    }
    let validReceiver = local_9;
    let offset = (worldPosition_2 - shadowOrigin);
    let uv_4 = ((vec2<f32>(dot(offset, shadowRight), dot(offset, shadowUp)) / vec2(max(shadowProjection.x, 0.000001f))) + vec2(0.5f));
    if all((uv_4 >= vec2(0f))) {
        local_10 = all((uv_4 <= vec2(1f)));
    } else {
        local_10 = false;
    }
    let inRange = local_10;
    if (shadowProjection.y > 0.5f) {
        local_11 = (shadowProjection.z < 0.5f);
    } else {
        local_11 = false;
    }
    let _e64 = local_11;
    if _e64 {
        local_12 = (shadowProjection.x > 0f);
    } else {
        local_12 = false;
    }
    let available = local_12;
