...
fn composeProbeDiffuse(shPreblend_2: array<vec4<f32>, 9>, localBlendFraction_1: f32, normal_11: vec3<f32>, skyIrradiance: vec3<f32>, kD: vec3<f32>, albedo_1: vec3<f32>, metallic_2: f32) -> vec3<f32> {
    let _e7 = evaluateProbeDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX(shPreblend_2, localBlendFraction_1, normal_11, skyIrradiance, kD, albedo_1, metallic_2);
    return _e7;
}
...
fn fs_main(in_1: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let metallic_5 = clamp(_e12, 0f, 1f);
    ...
    let _e97 = finiteScalar(dot(n, v), 0f);
    ...
    let _e137 = f0_;
    let _e138 = f_schlickX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(max(dot(_e133, v), 0f), _e137);
    let kD_1 = ((vec3(1f) - _e138) * (1f - metallic_5));
    ...
    if (_e150 < 0f) {
        ...
        let _e179 = sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e153, v, iblRoughness, _e154, in_1.worldPos, vec3<f32>(_e158, _e161, _e164), _e168.xyz, vec4<f32>(0f, 0f, 0f, 1f), prefilterMap_1, prefilterSampler_1, brdfLut_3, brdfLutSampler_3);
        specularIbl = _e179;
    } else {
        ...
        let _e198 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e189, v, iblRoughness, _e190, _e193, prefilterMap_1, prefilterSampler_1, brdfLut_3, brdfLutSampler_3);
        specularIbl = _e198;
    }
    ...
    let _e316 = material.thickness;
    let _e318 = finiteScalar(_e316, 0f);
    let _e321 = thicknessSample;
    let _e323 = finiteScalar(_e321, 1f);
    let worldThickness = max(((_e318 * length(worldRefractedDirection)) * _e323), 0f);
    ...
    let _e365 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(screenUv, vec2(0f), vec2(1f)), backdropLod);
    let unrefractedBackdrop = _e365.xyz;
    ...
    let _e373 = local_1;
    if _e373 {
        let _e381 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(refractedUv, vec2(0f), vec2(1f)), backdropLod);
        transmittedBackdrop = _e381.xyz;
    }
    let _e385 = material.attenuationColor;
    let _e388 = finiteColor(_e385, vec3(1f));
    let safeAttenuationColor = clamp(_e388, vec3(0f), vec3(1f));
    let _e396 = material.attenuationDistance;
    let _e398 = finiteScalar(_e396, 0f);
    let safeAttenuationDistance = max(_e398, 0f);
    ...
    let _e415 = local_2;
    let beerAttenuation = select(vec3(1f), pow(safeAttenuationColor, vec3(attenuationExponent)), _e415);
    let _e417 = color;
    let _e418 = transmittedBackdrop;
    let _e421 = finiteColor(_e418, vec3(0f));
    color = (_e417 + ((_e421 * transmittedEnergy) * beerAttenuation));
    let _e425 = physicalNormal;
    let _e427 = standardViewZ(in_1);
    let _e428 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e425, in_1.worldPos, _e427);
    let _e429 = physicalNormal;
    let _e430 = diffuseAlbedo;
    let _e431 = f0_;
    let _e432 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e429, v, _e430, metallic_5, a_3, _e431);
    let _e433 = color;
    color = (_e433 + (_e428 * _e432));
    let _e436 = color;
    color = (_e436 + _e7.emissive);
    let _e441 = color;
    output.color = vec4<f32>(_e441, alpha_1);
    let _e444 = reflectionFallback;
    let _e445 = standardSsrCoverage();
    output.reflectionFallback = vec4<f32>(_e444, _e445);
    let _e447 = output;
    return _e447;
}
...
fn fs_gbuffer(in_2: VsOut, @builtin(front_facing) frontFacing_1: bool) -> GBufferOutput {
    ...
    let _e35 = projectSpecularRadianceX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(vec3(1f), n_1, normalize((_e28 - in_2.worldPos)), roughness_8, _e32, brdfLut_3, brdfLutSampler_3);
...
fn fs_temporal(in_4: TemporalVsOut) -> @location(0) vec4<f32> {
    ...
    let _e11 = standardUsesBaseColorTexture();
    let _e14 = material.baseColorTextureCoordinatesTransform;
    let _e17 = material.baseColorTextureCoordinatesMetadata;
    let _e23 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.temporalProjection;
    let _e24 = reactive;
    let _e35 = projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX((_e4 * _e6), _e10, _e11, baseColorTexture_1, baseColorTexture_sampler, _e14, _e17, in_4.currentClip, in_4.previousClip, _e23, _e24, in_4.uv, in_4.uv1_, in_4.uv2_, in_4.uv3_, in_4.uv4_, in_4.uv5_, in_4.uv6_, in_4.uv7_);
    return _e35;
}
