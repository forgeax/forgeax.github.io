...
fn evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_4: vec3<f32>, viewDir: vec3<f32>, baseColor: vec3<f32>, metallic: f32, alphaSq: f32, F0_5: vec3<f32>) -> vec3<f32> {
    ...
    let nDotL_2 = max(dot(normal_4, l_1), 0f);
...
fn evaluate_standard_surface(input_2: SurfaceInput, materialValue: MaterialParameters) -> SurfaceData {
    ...
    normal_7 = normalize(input_2.geometricNormalWS);
...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> @location(0) vec4<f32> {
    ...
    let metallic_2 = clamp(_e12, 0f, 1f);
    ...
    f0_ = mix(((vec3(dielectricF0_) * _e59) * _e61), albedo, metallic_2);
    physicalNormal = n;
    ...
    let transmittedEnergy = select(0f, ((transmissionFactor * (1f - metallic_2)) * (1f - fresnel_1)), (refractedLengthSquared > 0.000001f));
    ...
    let kD = ((vec3(1f) - _e156) * (1f - metallic_2));
    ...
    let _e443 = f0_;
    let _e444 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e441, v, _e442, metallic_2, a_2, _e443);
    let _e445 = color;
    color = (_e445 + (_e440 * _e444));
    let _e448 = color;
    return vec4<f32>(_e448, alpha);
}
