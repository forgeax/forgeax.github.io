...
fn sampleStandardAmbientOcclusionX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(worldPosition_5: vec3<f32>, worldViewProjection: mat4x4<f32>, aoTexture: texture_2d<f32>, aoSampler: sampler) -> f32 {
    ...
    let uv_4 = (((clip_4.xy / vec2(clip_4.w)) * vec2<f32>(0.5f, -0.5f)) + vec2(0.5f));
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_11: vec3<f32>, layer_4: u32, count_4: u32, normal_16: vec3<f32>, l_1: vec3<f32>) -> f32 {
    var local_18: bool;
    var local_19: bool;
    var local_20: bool;
    var local_21: bool;
    ...
    if (kernel == 3u) {
        if all((uv_5 >= (tileLo + texel_2))) {
            local_22 = all((uv_5 <= (tileHi - texel_2)));
        } else {
            local_22 = false;
        }
...
fn evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_17: vec3<f32>, worldPos_12: vec3<f32>, viewZ_1: f32) -> f32 {
    var shadow_4: f32;
    var local_23: bool;
    ...
    let receiverPosition = (worldPos_12 + (normal_17 * _e33));
...
fn encodeStandardNormalRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(normal_18: vec3<f32>, roughness_7: f32) -> u32 {
    var oct: vec2<f32>;

    let n_2 = normalize(normal_18);
    ...
    let _e33 = oct;
    let uv_6 = vec2<u32>(round((clamp(((_e33 * 0.5f) + vec2(0.5f)), vec2(0f), vec2(1f)) * 4095f)));
    return ((uv_6.x | (uv_6.y << 12u)) | (u32(round((clamp(roughness_7, 0f, 1f) * 255f))) << 24u));
}
...
fn encodeStandardGBufferX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(normal_19: vec3<f32>, roughness_8: f32, albedo_3: vec3<f32>, metallic_12: f32, f0_6: vec3<f32>, occlusion: f32, emissive: vec3<f32>, opacity: f32, reflection: u32, probeRow_1: u32) -> GBufferOutputX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX {
    let _e5 = encodeStandardNormalRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(normal_19, roughness_8);
...
fn projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX(baseColorAlpha_1: f32, alphaCutoff: f32, alphaHash: f32, positionOS: vec3<f32>, baseColorTextureEnabled: bool, baseColorTexture: texture_2d<f32>, baseColorSampler: sampler, transform_1: vec4<f32>, metadata_2: vec4<f32>, alphaTextureEnabled: bool, alphaTexture: texture_2d<f32>, alphaSampler: sampler, alphaTransform: vec4<f32>, alphaMetadata: vec4<f32>, alphaChannel: f32, currentClip_1: vec4<f32>, previousClip_1: vec4<f32>, temporalProjection_3: vec4<f32>, reactive_3: f32, motionValid_2: bool, uv0_1: vec2<f32>, uv1_1: vec2<f32>, uv2_1: vec2<f32>, uv3_1: vec2<f32>, uv4_1: vec2<f32>, uv5_1: vec2<f32>, uv6_1: vec2<f32>, uv7_1: vec2<f32>) -> vec4<f32> {
    ...
    if (alphaCutoff > 0f) {
        local_24 = (coverage_2 <= alphaCutoff);
    } else {
        local_24 = false;
    }
    let _e49 = local_24;
...
fn evaluate_standard_surface(input_1: SurfaceInput, materialValue: MaterialParameters) -> SurfaceData {
    ...
    if _e119 {
        ...
        let uv_7 = (_e122 * materialValue.bumpTextureCoordinatesMetadata.zw);
        let dx_1 = dpdx(uv_7);
        let dy_1 = dpdy(uv_7);
        let _e130 = textureSampleGrad(normalTexture, normalTexture_sampler, uv_7, dx_1, dy_1);
        let height = _e130.x;
        let _e135 = textureSampleGrad(normalTexture, normalTexture_sampler, (uv_7 + dx_1), dx_1, dy_1);
        let heightX = _e135.x;
        let _e140 = textureSampleGrad(normalTexture, normalTexture_sampler, (uv_7 + dy_1), dx_1, dy_1);
        let heightY = _e140.x;
        let _e143 = normal_20;
