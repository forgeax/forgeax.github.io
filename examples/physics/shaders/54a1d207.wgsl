...
fn fs_main(in: FullscreenOutputX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX) -> @location(0) vec4<f32> {
    ...
    let _e25 = ditherUnorm8X_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(_e22, in.position.xy);
    let _e28 = params.ditherEnabled;
    let output = select(_e22, _e25, (_e28 > 0.5f));
    return vec4<f32>(output, source.w);
}
