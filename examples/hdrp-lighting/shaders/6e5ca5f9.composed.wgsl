...
fn cloud_direct_solar_factorX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX(worldPosition_2: vec3<f32>, shadowOrigin: vec3<f32>, shadowRight: vec3<f32>, shadowUp: vec3<f32>, shadowProjection: vec4<f32>) -> f32 {
    ...
    let uv_2 = ((vec2<f32>(dot(offset, shadowRight), dot(offset, shadowUp)) / vec2(max(shadowProjection.x, 0.000001f))) + vec2(0.5f));
    if all((uv_2 >= vec2(0f))) {
        local_9 = all((uv_2 <= vec2(1f)));
    } else {
        local_9 = false;
    }
    let inRange = local_9;
    ...
    let _e80 = textureSampleLevel(cloudShadowMapX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX, cloudShadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX, clamp(uv_2, vec2(0f), vec2(1f)), 0f);
    let sampled = _e80.x;
    if validReceiver {
        local_12 = inRange;
    } else {
...
fn evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_6: vec3<f32>, viewDir: vec3<f32>, baseColor: vec3<f32>, metallic_1: f32, alphaSq: f32, F0_5: vec3<f32>) -> vec3<f32> {
    ...
    let nDotL_2 = max(dot(normal_6, l_2), 0f);
    ...
    let _e31 = threeR184DirectMultiScatterX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(roughness_9, nDotV_2, nDotL_2, F0_5);
...
fn evaluateStandardDirectX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(worldPosition_5: vec3<f32>, ndc: vec3<f32>, viewZ: f32, normal_8: vec3<f32>, direction_3: vec3<f32>, albedo_2: vec3<f32>, metallic_3: f32, alpha: f32, f0_3: vec3<f32>, shadow: f32) -> vec3<f32> {
    var direct: vec3<f32>;
    ...
    let _e25 = cloud_apply_direct_solarX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX((shadow * _e6), worldPosition_5, _e11.xyz, _e15.xyz, _e19.xyz, _e23);
    direct = _e25;
    let _e27 = direct;
    return _e27;
}
...
fn applyLocalClippingX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MNFYHA2LOM45DU4DMMFXGK4YX(positionWS_1: vec3<f32>, shadow_1: bool, planes_1: array<vec4<f32>, 6>, control_1: vec4<f32>) {
    var local_16: bool;
    var local_17: bool;

    if !(!(shadow_1)) {
        local_16 = (control_1.z > 0.5f);
    } else {
        local_16 = true;
    }
    let _e10 = local_16;
...
fn applyViewClippingX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MNFYHA2LOM45DU4DMMFXGK4YX(positionWS_2: vec3<f32>, shadow_2: bool) {
    ...
    applyLocalClippingX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MNFYHA2LOM45DU4DMMFXGK4YX(positionWS_2, shadow_2, _e2, _e5);
    return;
}
...
fn _directionalReceiverDepthBiasX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(layer_3: u32, normal_9: vec3<f32>, l: vec3<f32>, radius: f32) -> f32 {
    var local_18: bool;
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos: vec3<f32>, layer_4: u32, count_4: u32, normal_10: vec3<f32>, l_1: vec3<f32>) -> f32 {
    var local_19: bool;
    var local_20: bool;
    var local_21: bool;
    var local_22: bool;
    ...
    let _e102 = _directionalReceiverDepthBiasX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(layer_4, normal_10, l_1, f32(((kernel / 2u) + 1u)));
    ...
    if (kernel == 3u) {
        if all((uv_4 >= (tileLo + texel))) {
            local_23 = all((uv_4 <= (tileHi - texel)));
        } else {
            local_23 = false;
        }
        let interior = local_23;
        if interior {
            let pcfFraction = fract(((uv_4 / texel) - vec2(0.5f)));
            ...
            let litLoLo = textureSampleCompareLevel(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, (uv_4 + (vec2<f32>(loOffset.x, loOffset.y) * texel)), adjustedDepth);
            let litHiLo = textureSampleCompareLevel(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, (uv_4 + (vec2<f32>(hiOffset.x, loOffset.y) * texel)), adjustedDepth);
            let litLoHi = textureSampleCompareLevel(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, (uv_4 + (vec2<f32>(loOffset.x, hiOffset.y) * texel)), adjustedDepth);
            let litHiHi = textureSampleCompareLevel(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, (uv_4 + (vec2<f32>(hiOffset.x, hiOffset.y) * texel)), adjustedDepth);
            ...
            }
            {
                ...
                loop {
                    ...
                    }
                    {
                        let _e207 = x;
                        let _e209 = y;
                        let offsetUv = clamp((uv_4 + (vec2<f32>(f32(_e207), f32(_e209)) * texel)), tileLo, tileHi);
        ...
        }
        {
            ...
            loop {
                ...
                }
                {
                    let _e243 = x_1;
                    let _e245 = y_1;
                    let offsetUv_1 = clamp((uv_4 + (vec2<f32>(f32(_e243), f32(_e245)) * texel)), tileLo, tileHi);
...
fn evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_11: vec3<f32>, worldPos_1: vec3<f32>, viewZ_1: f32) -> f32 {
    var shadow_3: f32;
    var local_24: bool;
    ...
    let receiverPosition = (worldPos_1 + (normal_11 * _e33));
    let _e36 = _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(receiverPosition, _e28, count_6, normal_11, l_3);
    shadow_3 = _e36;
    ...
    if _e49 {
        ...
        if (blendWidth > 0f) {
            ...
            if (t_1 > 0f) {
                let _e72 = _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(receiverPosition, (_e28 + 1u), count_6, normal_11, l_3);
                shadow_3 = mix(_e36, _e72, t_1);
            }
        }
    }
    let _e74 = shadow_3;
...
fn encodeStandardNormalRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(normal_12: vec3<f32>, roughness_7: f32) -> u32 {
    var oct: vec2<f32>;

    let n = normalize(normal_12);
    oct = (n.xy / vec2(((abs(n.x) + abs(n.y)) + abs(n.z))));
    if (n.z < 0f) {
        let _e17 = oct;
        let _e27 = oct;
    ...
    let _e33 = oct;
    let uv_5 = vec2<u32>(round((clamp(((_e33 * 0.5f) + vec2(0.5f)), vec2(0f), vec2(1f)) * 4095f)));
    return ((uv_5.x | (uv_5.y << 12u)) | (u32(round((clamp(roughness_7, 0f, 1f) * 255f))) << 24u));
}
...
fn encodeStandardGBufferX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(normal_13: vec3<f32>, roughness_8: f32, albedo_3: vec3<f32>, metallic_4: f32, f0_4: vec3<f32>, occlusion: f32, emissive: vec3<f32>, opacity: f32, reflection: u32, probeRow_1: u32) -> GBufferOutputX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX {
    let _e5 = encodeStandardNormalRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(normal_13, roughness_8);
    let _e8 = encodeStandardReflectanceX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(f0_4, occlusion);
    let _e11 = encodeStandardReflectanceX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(albedo_3, metallic_4);
...
fn alphaHashThresholdX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DUYLMOBUGCX3IMFZWQX(position: vec3<f32>, derivative: f32) -> f32 {
    ...
    let t_2 = fract(level);
    let x_3 = mix(noise.x, noise.y, t_2);
    let a_2 = min(t_2, (1f - t_2));
    threshold = x_3;
    if (a_2 > 0f) {
        if (x_3 < a_2) {
            threshold = ((x_3 * x_3) / ((2f * a_2) * (1f - a_2)));
        } else {
            if (x_3 < (1f - a_2)) {
                threshold = ((x_3 - (0.5f * a_2)) / (1f - a_2));
            } else {
                threshold = (1f - (((1f - x_3) * (1f - x_3)) / ((2f * a_2) * (1f - a_2))));
            }
        }
    }
    let _e60 = threshold;
...
fn projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX(baseColorAlpha_1: f32, alphaCutoff: f32, alphaHash: f32, positionOS: vec3<f32>, baseColorTextureEnabled: bool, baseColorTexture: texture_2d<f32>, baseColorSampler: sampler, transform_1: vec4<f32>, metadata_1: vec4<f32>, alphaTextureEnabled: bool, alphaTexture: texture_2d<f32>, alphaSampler: sampler, alphaTransform: vec4<f32>, alphaMetadata: vec4<f32>, alphaChannel: f32, currentClip_1: vec4<f32>, previousClip_1: vec4<f32>, temporalProjection_3: vec4<f32>, reactive_3: f32, motionValid_2: bool, uv0_1: vec2<f32>, uv1_1: vec2<f32>, uv2_1: vec2<f32>, uv3_1: vec2<f32>, uv4_1: vec2<f32>, uv5_1: vec2<f32>, uv6_1: vec2<f32>, uv7_1: vec2<f32>) -> vec4<f32> {
    ...
    if (alphaCutoff > 0f) {
        local_25 = (coverage_2 <= alphaCutoff);
    } else {
        local_25 = false;
    }
    let _e49 = local_25;
...
fn evaluate_standard_surface(input_1: SurfaceInput, materialValue: MaterialParameters) -> SurfaceData {
    ...
    normal_14 = (normalize(input_1.geometricNormalWS) * faceDirection_1);
    ...
    if _e93 {
        ...
        normal_14 = (_e109 * faceDirection_1);
        usesNormal = true;
    }
    ...
    if _e119 {
        ...
        let uv_6 = (_e122 * materialValue.bumpTextureCoordinatesMetadata.zw);
        let dx_1 = dpdx(uv_6);
        let dy_1 = dpdy(uv_6);
        let _e130 = textureSampleGrad(normalTexture, normalTexture_sampler, uv_6, dx_1, dy_1);
        let height = _e130.x;
        let _e135 = textureSampleGrad(normalTexture, normalTexture_sampler, (uv_6 + dx_1), dx_1, dy_1);
        let heightX = _e135.x;
        let _e140 = textureSampleGrad(normalTexture, normalTexture_sampler, (uv_6 + dy_1), dx_1, dy_1);
        let heightY = _e140.x;
        let _e143 = normal_14;
    ...
    let baseColor_1 = ((materialValue.baseColor.xyz * _e175.xyz) * vertexColor.xyz);
    let _e181 = metallicSample;
    let _e184 = surfaceChannel(_e181, u32(materialValue.metallicChannel));
    let metallic_5 = clamp((materialValue.metallic * _e184), 0f, 1f);
    let _e190 = roughnessSample;
    let _e193 = surfaceChannel(_e190, u32(materialValue.roughnessChannel));
    let roughness_10 = clamp((materialValue.roughness * _e193), 0.04f, 1f);
    ...
    let _e234 = alpha_3;
    return SurfaceData(baseColor_1, _e226, metallic_5, roughness_10, emissive_1, occlusion_1, clamp((((materialValue.baseColor.w * _e230) * vertexColor.w) * _e234), 0f, 1f), clamp(materialValue.alphaCutoff, 0f, 1f));
}
...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let n_1 = normalize(_e6.normalWS);
    ...
    let _e96 = finiteScalar(dot(n_1, v), 0f);
    let refractionFromInside = (_e96 < 0f);
    let refractionNormal = select(n_1, -(n_1), refractionFromInside);
    ...
    let _e221 = evaluateStandardEnvironmentX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_2.worldPos, _e208, v, _e209, metallic_7, iblRoughness, _e210, _e212, irradianceMap_2, irradianceSampler_2, prefilterMap_2, prefilterSampler_2, brdfLut_4, skylightPrefilterMap_1, _e213, _e214);
    ...
    let _e329 = material.thickness;
    let _e331 = finiteScalar(_e329, 0f);
    let _e334 = thicknessSample;
    let _e336 = finiteScalar(_e334, 1f);
    let worldThickness = max(((_e331 * length(worldRefractedDirection)) * _e336), 0f);
    ...
    let _e378 = textureSampleLevel(transmissionBackdropTexture, prefilterSampler_2, clamp(screenUv, vec2(0f), vec2(1f)), backdropLod);
    let unrefractedBackdrop = _e378.xyz;
    ...
    let _e386 = local_2;
    if _e386 {
        let _e394 = textureSampleLevel(transmissionBackdropTexture, prefilterSampler_2, clamp(refractedUv, vec2(0f), vec2(1f)), backdropLod);
        transmittedBackdrop = _e394.xyz;
    }
    let _e398 = material.attenuationColor;
    let _e401 = finiteColor(_e398, vec3(1f));
    let safeAttenuationColor = clamp(_e401, vec3(0f), vec3(1f));
    let _e409 = material.attenuationDistance;
    let _e411 = finiteScalar(_e409, 0f);
    let safeAttenuationDistance = max(_e411, 0f);
    ...
    let _e428 = local_3;
    let beerAttenuation = select(vec3(1f), pow(safeAttenuationColor, vec3(attenuationExponent)), _e428);
    let _e430 = color;
    let _e431 = transmittedBackdrop;
    let _e434 = finiteColor(_e431, vec3(0f));
    color = (_e430 + ((_e434 * transmittedEnergy) * beerAttenuation));
    let _e438 = physicalNormal;
    let _e440 = standardViewZ(in_2);
    let _e441 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e438, in_2.worldPos, _e440);
    let _e442 = color;
    let _e446 = standardViewZ(in_2);
    let _e447 = physicalNormal;
    let _e448 = diffuseAlbedo;
    let _e449 = f0_;
    let _e450 = evaluateStandardDirectX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_2.worldPos, in_2.ndc.xyz, _e446, _e447, v, _e448, metallic_7, a_4, _e449, _e441);
    color = (_e442 + _e450);
    let _e452 = color;
    color = (_e452 + _e6.emissive);
    let _e457 = color;
    output.color = vec4<f32>(_e457, alpha_5);
    let _e460 = reflectionFallback;
    let _e461 = standardSsrCoverage();
    output.reflectionFallback = vec4<f32>(_e460, _e461);
    let _e463 = output;
    return _e463;
}
