...
fn evaluate_cluster_lightX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(light: DirectLightSlotX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX, world_pos_1: vec3<f32>, normal_10: vec3<f32>, view_dir: vec3<f32>, base_color: vec3<f32>, metallic_7: f32, alpha_sq: f32, f0_2: vec3<f32>, flat_2d: bool, receive_shadows: bool) -> vec3<f32> {
    ...
    if (kind == KIND_POINTX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX) {
        ...
        let _e27 = evalPointX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(light.position.xyz, light.colorTimesIntensity.xyz, light.position.w, world_pos_1, normal_10, view_dir, base_color, metallic_7, alpha_sq, f0_2);
        return _e27;
    }
    if (kind == KIND_SPOTX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX) {
        ...
        if _e84 {
            ...
            let _e102 = tile;
            let _e110 = evalSpotShadowedX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(light.position.xyz, light.direction.xyz, light.colorTimesIntensity.xyz, light.colorTimesIntensity.w, light.direction.w, light.position.w, world_pos_1, normal_10, view_dir, base_color, metallic_7, alpha_sq, f0_2, _e101, _e102, 0.005f, 0.05f, 3f, select(0f, 1f, receive_shadows));
            let _e116 = tile;
        ...
        let _e136 = evalSpotX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(light.position.xyz, light.direction.xyz, light.colorTimesIntensity.xyz, light.colorTimesIntensity.w, light.direction.w, light.position.w, world_pos_1, normal_10, view_dir, base_color, metallic_7, alpha_sq, f0_2);
        return (_e136 * modifier);
    }
    if (kind == KIND_RECT_AREAX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX) {
        let _e154 = evalRectAreaLtcGgxX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXEZLDORPWC4TFMEX(light.position.xyz, light.colorTimesIntensity.xyz, light.auxiliary.xyz, light.direction.xyz, light.colorTimesIntensity.w, light.direction.w, light.position.w, world_pos_1, normal_10, view_dir, base_color, metallic_7, alpha_sq, f0_2);
        return _e154;
    }
    return vec3(0f);
}

fn evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(ndc_1: vec3<f32>, view_z_2: f32, world_pos_2: vec3<f32>, normal_11: vec3<f32>, view_dir_1: vec3<f32>, base_color_1: vec3<f32>, metallic_8: f32, alpha_sq_1: f32, f0_3: vec3<f32>, flat_2d_1: bool, receive_shadows_1: bool) -> vec3<f32> {
    ...
    loop {
        ...
        }
        {
            ...
            let _e70 = evaluate_cluster_lightX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(light_1, world_pos_2, normal_11, view_dir_1, base_color_1, metallic_8, alpha_sq_1, f0_3, flat_2d_1, receive_shadows_1);
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_11: vec3<f32>, layer_3: u32, count_4: u32, normal_13: vec3<f32>, l: vec3<f32>, useNormalBias: bool) -> f32 {
    var local_13: bool;
    var local_14: bool;
    var local_15: bool;
    var local_16: bool;
    ...
    if (kernel == 3u) {
        if all((uv_4 >= (tileLo + texel_2))) {
            local_17 = all((uv_4 <= (tileHi - texel_2)));
        } else {
            local_17 = false;
        }
...
fn evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_14: vec3<f32>, worldPos_12: vec3<f32>, viewZ: f32) -> f32 {
    var shadow_1: f32;
    var local_18: bool;
    ...
    let _e32 = _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_12, _e28, count_5, normal_14, l_5, true);
    shadow_1 = _e32;
    let _e36 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cascadeBlend;
    if (_e36 > 0f) {
        local_18 = ((_e28 + 1u) < count_5);
    } else {
        local_18 = false;
    }
    let _e45 = local_18;
    if _e45 {
        ...
        if (blendWidth > 0f) {
            ...
            if (t_1 > 0f) {
                let _e69 = _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_12, (_e28 + 1u), count_5, normal_14, l_5, true);
...
fn projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX(baseColorAlpha_1: f32, alphaCutoff: f32, baseColorTextureEnabled: bool, baseColorTexture: texture_2d<f32>, baseColorSampler: sampler, transform_1: vec4<f32>, metadata_3: vec4<f32>, currentClip_1: vec4<f32>, previousClip_1: vec4<f32>, temporalProjection_3: vec4<f32>, reactive_3: f32, uv0_1: vec2<f32>, uv1_1: vec2<f32>, uv2_1: vec2<f32>, uv3_1: vec2<f32>, uv4_1: vec2<f32>, uv5_1: vec2<f32>, uv6_1: vec2<f32>, uv7_1: vec2<f32>) -> vec4<f32> {
    ...
    if (alphaCutoff > 0f) {
        ...
        local_19 = ((baseColorAlpha_1 * _e24) <= alphaCutoff);
    } else {
        local_19 = false;
    }
    let _e30 = local_19;
...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let _e45 = standardSurfaceF0_(in_2, _e7);
    f0_ = _e45;
    physicalNormal = n_2;
    ...
    if _e49 {
        ...
        let _e56 = transformedMaterialUv(_e52, _e55, in_2);
    ...
    if _e66 {
        ...
        let _e73 = transformedMaterialUv(_e69, _e72, in_2);
    ...
    let _e97 = finiteScalar(dot(n_2, v_2), 0f);
    ...
    let fresnel_1 = clamp(_e113.x, 0f, 1f);
    let transmittedEnergy = select(0f, ((transmissionFactor * (1f - metallic_13)) * (1f - fresnel_1)), (refractedLengthSquared > 0.000001f));
    diffuseAlbedo = (albedo_3 * (1f - transmittedEnergy));
    let _e133 = physicalNormal;
    let _e137 = f0_;
    let _e138 = f_schlickX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(max(dot(_e133, v_2), 0f), _e137);
    let kD_1 = ((vec3(1f) - _e138) * (1f - metallic_13));
    ...
    if (_e150 < 0f) {
        ...
        let _e179 = sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e153, v_2, iblRoughness, _e154, in_2.worldPos, vec3<f32>(_e158, _e161, _e164), _e168.xyz, vec4<f32>(0f, 0f, 0f, 1f), prefilterMap_1, prefilterSampler_1, brdfLut_3, brdfLutSampler_3);
        specularIbl = _e179;
    } else {
        ...
        let _e198 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e189, v_2, iblRoughness, _e190, _e193, prefilterMap_1, prefilterSampler_1, brdfLut_3, brdfLutSampler_3);
        specularIbl = _e198;
    }
    ...
    let environmentScale = (_e217 * ao);
    let _e239 = ambient;
    ambient = (_e239 * environmentScale);
    let _e241 = reflectionFallback;
    reflectionFallback = (_e241 * environmentScale);
    let ssaoUv = ((in_2.ndc.xy * vec2<f32>(0.5f, -0.5f)) + vec2<f32>(0.5f, 0.5f));
    let _e255 = textureSample(ssaoBlurredTexture, ssaoBlurredSampler, ssaoUv);
    let ssaoFactor = _e255.x;
    let _e257 = get_ssao_intensityX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX();
    let _e258 = ambient;
    ambient = (_e258 * mix(1f, (ssaoFactor * ao), _e257));
    let ssaoScale = mix(1f, (ssaoFactor * ao), _e257);
    let _e266 = reflectionFallback;
    reflectionFallback = (_e266 * ssaoScale);
    let _e268 = ambient;
    color = _e268;
    ...
    let _e341 = material.thickness;
    let _e343 = finiteScalar(_e341, 0f);
    let _e346 = thicknessSample;
    let _e348 = finiteScalar(_e346, 1f);
    let worldThickness = max(((_e343 * length(worldRefractedDirection)) * _e348), 0f);
    ...
    let _e390 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(screenUv, vec2(0f), vec2(1f)), backdropLod);
    let unrefractedBackdrop = _e390.xyz;
    ...
    let _e398 = local_1;
    if _e398 {
        let _e406 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(refractedUv, vec2(0f), vec2(1f)), backdropLod);
        transmittedBackdrop = _e406.xyz;
    }
    let _e410 = material.attenuationColor;
    let _e413 = finiteColor(_e410, vec3(1f));
    let safeAttenuationColor = clamp(_e413, vec3(0f), vec3(1f));
    let _e421 = material.attenuationDistance;
    let _e423 = finiteScalar(_e421, 0f);
    let safeAttenuationDistance = max(_e423, 0f);
    ...
    let _e440 = local_2;
    let beerAttenuation = select(vec3(1f), pow(safeAttenuationColor, vec3(attenuationExponent)), _e440);
    let _e442 = color;
    let _e443 = transmittedBackdrop;
    let _e446 = finiteColor(_e443, vec3(0f));
    color = (_e442 + ((_e446 * transmittedEnergy) * beerAttenuation));
    let _e450 = physicalNormal;
    let _e452 = standardViewZ(in_2);
    let _e453 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e450, in_2.worldPos, _e452);
    let _e454 = physicalNormal;
    let _e455 = diffuseAlbedo;
    let _e456 = f0_;
    let _e457 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e454, v_2, _e455, metallic_13, a_3, _e456);
    let _e458 = color;
    color = (_e458 + (_e453 * _e457));
    let _e461 = color;
    let _e464 = standardViewZ(in_2);
    let _e466 = physicalNormal;
    let _e467 = diffuseAlbedo;
    let _e468 = f0_;
    let _e471 = evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_2.ndc.xyz, _e464, in_2.worldPos, _e466, v_2, _e467, metallic_13, a_3, _e468, false, true);
    color = (_e461 + _e471);
    let _e473 = color;
    color = (_e473 + _e7.emissive);
    let _e478 = color;
    output.color = vec4<f32>(_e478, alpha_1);
    let _e481 = reflectionFallback;
    let _e482 = standardSsrCoverage();
    output.reflectionFallback = vec4<f32>(_e481, _e482);
    let _e484 = output;
    return _e484;
}
