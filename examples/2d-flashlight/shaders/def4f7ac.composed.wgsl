...
fn fs_main(in_1: VsOut, @builtin(front_facing) frontFacing: bool) -> @location(0) vec4<f32> {
    ...
    if (_e85 < 0f) {
        ...
        let _e114 = sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e88, v_5, iblRoughness, _e89, in_1.worldPos, vec3<f32>(_e93, _e96, _e99), _e103.xyz, vec4<f32>(0f, 0f, 0f, 1f), prefilterMap_1, prefilterSampler_1, brdfLut_3, brdfLutSampler_3);
        specularIbl = _e114;
    } else {
        ...
        let _e133 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e124, v_5, iblRoughness, _e125, _e128, prefilterMap_1, prefilterSampler_1, brdfLut_3, brdfLutSampler_3);
        specularIbl = _e133;
    }
    ...
    let _e162 = coatF;
    ambient = ((((kD_1 * _e153) * _e155) + _e157) * (vec3(1f) - _e162));
    let _e168 = probeBlendRecords[1];
    let _e171 = probeBlendRecords[2];
    let _e174 = probeBlendRecords[3];
    let _e177 = probeBlendRecords[4];
    let _e180 = probeBlendRecords[5];
    let _e183 = probeBlendRecords[6];
    let _e186 = probeBlendRecords[7];
    let _e189 = probeBlendRecords[8];
    let _e192 = probeBlendRecords[9];
    let probeShPreblend = array<vec4<f32>, 9>(_e168, _e171, _e174, _e177, _e180, _e183, _e186, _e189, _e192);
    let probeLocalBlendFraction = probeBlendRecords[0].z;
    let probeDiffuseK = (kD_1 / max(vec3((1f - metallic_13)), vec3(0.0001f)));
    let _e205 = physicalNormal;
    let _e206 = irradiance;
    let _e208 = diffuseAlbedo;
    let _e209 = composeProbeDiffuse(probeShPreblend, probeLocalBlendFraction, _e205, (_e206 * skyFactor), probeDiffuseK, _e208, metallic_13);
    let _e210 = specularIbl;
    let _e215 = coatF;
    ambient = ((_e209 + (_e210 * _e152)) * (vec3(1f) - _e215));
    let _e218 = ambient;
    ambient = (_e218 * ao);
    let _e220 = ambient;
    color = _e220;
    let _e222 = color;
    color = (_e222 + _e6.emissive);
    let _e225 = physicalNormal;
    let _e227 = standardViewZ(in_1);
    let _e228 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e225, in_1.worldPos, _e227);
    let _e229 = physicalNormal;
    let _e230 = diffuseAlbedo;
    let _e231 = f0_;
    let _e232 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e229, v_5, _e230, metallic_13, a_3, _e231);
    let _e233 = color;
    color = (_e233 + (_e228 * _e232));
    let _e236 = color;
    let _e239 = standardViewZ(in_1);
    let _e241 = physicalNormal;
    let _e242 = diffuseAlbedo;
    let _e243 = f0_;
    let _e246 = evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_1.ndc.xyz, _e239, in_1.worldPos, _e241, v_5, _e242, metallic_13, a_3, _e243, false, true);
    color = (_e236 + _e246);
    let _e248 = color;
    return vec4<f32>(_e248, alpha);
}
