...
fn sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(normal_3: vec3<f32>, view_2: vec3<f32>, roughness_3: f32, F0_3: vec3<f32>, worldPosition_1: vec3<f32>, boxCenter_1: vec3<f32>, boxExtents_1: vec3<f32>, rotation_3: vec4<f32>, probeMap: texture_cube<f32>, probeSampler: sampler, brdfLut_2: texture_2d<f32>, brdfLutSampler_2: sampler, skylightMap: texture_cube<f32>, skylightRotation: vec4<f32>, skylightScale: vec3<f32>, probeIntensity: f32, boxProjection: bool) -> vec3<f32> {
    ...
    let local_25 = (_e55 * probeIntensity);
    let _e58 = sky;
    return mix(_e58, local_25, weight);
}
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos: vec3<f32>, layer_4: u32, count_4: u32, normal_10: vec3<f32>, l_1: vec3<f32>) -> f32 {
    ...
    if (kernel == 3u) {
        if all((uv_4 >= (tileLo + texel))) {
            local_20 = all((uv_4 <= (tileHi - texel)));
        } else {
            local_20 = false;
        }
...
fn projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX(baseColorAlpha_1: f32, alphaCutoff: f32, alphaHash: f32, positionOS: vec3<f32>, baseColorTextureEnabled: bool, baseColorTexture: texture_2d<f32>, baseColorSampler: sampler, transform_1: vec4<f32>, metadata_1: vec4<f32>, alphaTextureEnabled: bool, alphaTexture: texture_2d<f32>, alphaSampler: sampler, alphaTransform: vec4<f32>, alphaMetadata: vec4<f32>, alphaChannel: f32, currentClip_1: vec4<f32>, previousClip_1: vec4<f32>, temporalProjection_3: vec4<f32>, reactive_3: f32, motionValid_2: bool, uv0_1: vec2<f32>, uv1_1: vec2<f32>, uv2_1: vec2<f32>, uv3_1: vec2<f32>, uv4_1: vec2<f32>, uv5_1: vec2<f32>, uv6_1: vec2<f32>, uv7_1: vec2<f32>) -> vec4<f32> {
    ...
    if (alphaCutoff > 0f) {
        local_22 = (coverage_2 <= alphaCutoff);
    } else {
        local_22 = false;
    }
    let _e49 = local_22;
...
fn alphaTestSurface(surface: SurfaceData) {
    ...
    if (surface.alphaClipThreshold > 0f) {
        local_24 = (surface.opacity <= surface.alphaClipThreshold);
    } else {
        local_24 = false;
    }
    let _e10 = local_24;
