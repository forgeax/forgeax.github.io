...
fn projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX(baseColorAlpha_1: f32, alphaCutoff: f32, baseColorTextureEnabled: bool, baseColorTexture: texture_2d<f32>, baseColorSampler: sampler, transform_1: vec4<f32>, metadata_2: vec4<f32>, currentClip_1: vec4<f32>, previousClip_1: vec4<f32>, temporalProjection_3: vec4<f32>, reactive_2: f32, uv0_1: vec2<f32>, uv1_1: vec2<f32>, uv2_1: vec2<f32>, uv3_1: vec2<f32>, uv4_1: vec2<f32>, uv5_1: vec2<f32>, uv6_1: vec2<f32>, uv7_1: vec2<f32>) -> vec4<f32> {
    ...
    if (alphaCutoff > 0f) {
        ...
        local_30 = ((baseColorAlpha_1 * _e24) <= alphaCutoff);
    } else {
        local_30 = false;
    }
    let _e30 = local_30;
...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> @location(0) vec4<f32> {
    ...
    let alpha = _e8.opacity;
    let albedo_2 = _e8.baseColor;
    ...
    let n_5 = normalize(_e8.normalWS);
    let _e28 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cameraPos;
    let v_5 = normalize((_e28 - in_2.worldPos));
    specularColor = _e6.specularColor;
    let _e36 = finiteScalar(_e6.specular, 1f);
    specularWeight = clamp(_e36, 0f, 1f);
    let _e43 = finiteScalar(_e6.ior, 1.5f);
    let safeIor = max(_e43, 1f);
    let dielectricF0_ = pow(((safeIor - 1f) / (safeIor + 1f)), 2f);
    let _e54 = specularColor;
    let _e56 = specularWeight;
    f0_ = mix(((vec3(dielectricF0_) * _e54) * _e56), albedo_2, metallic_13);
    physicalNormal = n_5;
    diffuseAlbedo = albedo_2;
    let _e64 = physicalNormal;
    let _e68 = f0_;
    let _e69 = f_schlickX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(max(dot(_e64, v_5), 0f), _e68);
    let kD_1 = ((vec3(1f) - _e69) * (1f - metallic_13));
    let _e75 = coatRoughness;
    let _e76 = coatRoughness;
    coatAlpha = (_e75 * _e76);
    let _e81 = skylight.intensity;
    if (_e81 < 0f) {
        let _e84 = physicalNormal;
        let _e85 = f0_;
        let _e89 = skylight.colorR;
        let _e92 = skylight.colorG;
        let _e95 = skylight.colorB;
        let _e99 = skylight.rotation;
        let _e110 = sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e84, v_5, iblRoughness, _e85, in_2.worldPos, vec3<f32>(_e89, _e92, _e95), _e99.xyz, vec4<f32>(0f, 0f, 0f, 1f), prefilterMap_1, prefilterSampler_1, brdfLut_3, brdfLutSampler_3);
        specularIbl = _e110;
    } else {
        let _e112 = physicalNormal;
        let _e115 = skylight.rotation;
        let _e118 = sampleIblDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e112, _e115, irradianceMap_1, irradianceSampler_1);
        irradiance = _e118;
        let _e120 = physicalNormal;
        let _e121 = f0_;
        let _e124 = skylight.rotation;
        let _e129 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e120, v_5, iblRoughness, _e121, _e124, prefilterMap_1, prefilterSampler_1, brdfLut_3, brdfLutSampler_3);
        specularIbl = _e129;
    }
    let ao = _e8.occlusion;
    let _e133 = skylight.colorR;
    let _e136 = skylight.colorG;
    let _e139 = skylight.colorB;
    let skyColor_1 = vec3<f32>(_e133, _e136, _e139);
    let _e143 = skylight.intensity;
    let skyFactor = (skyColor_1 * _e143);
    let _e147 = skylight.intensity;
    let _e148 = decodeSpecularEnvironmentScaleX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(skyColor_1, _e147);
    ...
    let _e211 = coatF;
    ambient = ((_e205 + (_e206 * _e148)) * (vec3(1f) - _e211));
    let _e214 = ambient;
    ambient = (_e214 * ao);
    let _e216 = ambient;
    color = _e216;
    let _e218 = color;
    color = (_e218 + _e8.emissive);
    ...
    let _e239 = f0_;
    let _e242 = evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_2.ndc.xyz, _e235, in_2.worldPos, _e237, v_5, _e238, metallic_13, a_3, _e239, false, true);
    color = (_e232 + _e242);
    let _e244 = color;
    return vec4<f32>(_e244, alpha);
}
