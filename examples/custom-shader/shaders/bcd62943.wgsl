...
@group(1) @binding(0) 
var<uniform> pulse: PulseUniforms;
@group(1) @binding(1) 
...
fn fs_main(in_1: VsOut) -> @location(0) vec4<f32> {
    ...
    let sampled = textureSample(baseColorTexture, baseColorTexture_sampler, in_1.uv);
    let _e45 = pulse.baseColor.w;
    return vec4<f32>(((modulated * sampled.xyz) * (vec3(1f) - (_e29 * 0.1f))), (_e45 * sampled.w));
}
