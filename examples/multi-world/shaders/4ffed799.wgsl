...
fn sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(normal_3: vec3<f32>, view_2: vec3<f32>, roughness_3: f32, F0_3: vec3<f32>, worldPosition_1: vec3<f32>, boxCenter_1: vec3<f32>, boxExtents_1: vec3<f32>, rotation_3: vec4<f32>, probeMap: texture_cube<f32>, probeSampler: sampler, brdfLut_2: texture_2d<f32>, brdfLutSampler_2: sampler, skylightMap: texture_cube<f32>, skylightRotation: vec4<f32>, skylightScale: vec3<f32>, probeIntensity: f32, boxProjection: bool) -> vec3<f32> {
    ...
    let local_55 = (_e55 * probeIntensity);
    let _e58 = sky;
    return mix(_e58, local_55, weight);
}
...
fn evaluateProbeDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX(shPreblend_1: array<vec4<f32>, 9>, localBlendFraction: f32, normal_5: vec3<f32>, e_sky: vec3<f32>, k_d: vec3<f32>, albedo: vec3<f32>, metallic: f32) -> vec3<f32> {
    ...
    let local_56 = (max(_e2, vec3(0f)) * PROBE_INV_PIX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX);
    let skyResidualFraction = (1f - localBlendFraction);
    return ((((local_56 + (skyResidualFraction * e_sky)) * k_d) * albedo) * (1f - metallic));
}
...
fn evaluate_cluster_lightX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(light: DirectLightSlotX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX, world_pos_1: vec3<f32>, normal_11: vec3<f32>, view_dir: vec3<f32>, base_color: vec3<f32>, metallic_9: f32, alpha_sq: f32, f0_2: vec3<f32>, flat_2d: bool, receive_shadows: bool) -> vec3<f32> {
    ...
    if (kind == KIND_SPOTX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX) {
        ...
        tile = encoded_tile;
        if (encoded_tile >= 0i) {
            local_27 = ((encoded_tile & PROJECTOR_FLAGX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX) != 0i);
        } else {
            local_27 = false;
        }
        let _e70 = local_27;
...
fn _samplePcssForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_14: vec3<f32>, layer_5: u32, count_4: u32, normal_16: vec3<f32>, l_1: vec3<f32>, profile: u32) -> f32 {
    var local_34: bool;
    var local_35: bool;
    var local_36: bool;
    var local_37: bool;
    ...
    if (tileUv.x >= 0f) {
        local_35 = (tileUv.x <= 1f);
    } else {
        local_35 = false;
    }
    let _e46 = local_35;
    if _e46 {
        local_36 = (tileUv.y >= 0f);
    } else {
        local_36 = false;
    }
    let _e53 = local_36;
    if _e53 {
        local_37 = (tileUv.y <= 1f);
    } else {
        local_37 = false;
    }
    let _e60 = local_37;
    if _e60 {
        local_38 = (projCoords.z >= 0f);
    } else {
        local_38 = false;
    }
    let _e67 = local_38;
    if _e67 {
        local_39 = (projCoords.z <= 1f);
    } else {
        local_39 = false;
    }
    let _e74 = local_39;
    ...
    if (lightDepthWorldSpan > 0f) {
        local_43 = (worldUnitsPerTexel > 0f);
    } else {
        local_43 = false;
    }
    let _e213 = local_43;
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_15: vec3<f32>, layer_6: u32, count_5: u32, normal_17: vec3<f32>, l_2: vec3<f32>) -> f32 {
    var local_46: bool;
    var local_47: bool;
    var local_48: bool;
    var local_49: bool;
    ...
    if (tileUv_1.x >= 0f) {
        local_46 = (tileUv_1.x <= 1f);
    } else {
        local_46 = false;
    }
    let _e40 = local_46;
    if _e40 {
        local_47 = (tileUv_1.y >= 0f);
    } else {
        local_47 = false;
    }
    let _e47 = local_47;
    if _e47 {
        local_48 = (tileUv_1.y <= 1f);
    } else {
        local_48 = false;
    }
    let _e54 = local_48;
    if _e54 {
        local_49 = (currentDepth <= 1f);
    } else {
        local_49 = false;
    }
    let _e60 = local_49;
    ...
    if (filterProfile >= 4u) {
        let _e90 = _samplePcssForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_15, layer_6, count_5, normal_17, l_2, filterProfile);
        return _e90;
    }
    ...
    if (kernel == 3u) {
        if all((uv_7 >= (tileLo + texel_5))) {
            local_50 = all((uv_7 <= (tileHi - texel_5)));
        } else {
            local_50 = false;
        }
        let interior = local_50;
        if interior {
...
fn encodeStandardGBufferX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(normal_20: vec3<f32>, roughness_9: f32, albedo_3: vec3<f32>, metallic_13: f32, f0_6: vec3<f32>, occlusion: f32, emissive: vec3<f32>, opacity: f32, reflection: u32, probeRow_1: u32) -> GBufferOutputX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX {
...
fn projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX(baseColorAlpha_1: f32, alphaCutoff: f32, alphaHash: f32, positionOS: vec3<f32>, baseColorTextureEnabled: bool, baseColorTexture: texture_2d<f32>, baseColorSampler: sampler, transform_1: vec4<f32>, metadata_3: vec4<f32>, alphaTextureEnabled: bool, alphaTexture: texture_2d<f32>, alphaSampler: sampler, alphaTransform: vec4<f32>, alphaMetadata: vec4<f32>, alphaChannel: f32, currentClip_1: vec4<f32>, previousClip_1: vec4<f32>, temporalProjection_3: vec4<f32>, reactive_3: f32, motionValid_2: bool, uv0_1: vec2<f32>, uv1_1: vec2<f32>, uv2_1: vec2<f32>, uv3_1: vec2<f32>, uv4_1: vec2<f32>, uv5_1: vec2<f32>, uv6_1: vec2<f32>, uv7_1: vec2<f32>) -> vec4<f32> {
    ...
    if (alphaCutoff > 0f) {
        local_52 = (coverage_2 <= alphaCutoff);
    } else {
        local_52 = false;
    }
    let _e49 = local_52;
...
fn alphaTestSurface(surface: SurfaceData) {
    ...
    if (surface.alphaClipThreshold > 0f) {
        local_54 = (surface.opacity <= surface.alphaClipThreshold);
    } else {
        local_54 = false;
    }
    let _e10 = local_54;
...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let a_5 = (iblRoughness * iblRoughness);
    ...
    let _e221 = evaluateStandardEnvironmentX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_2.worldPos, _e208, v_5, _e209, metallic_16, iblRoughness, _e210, _e212, irradianceMap_2, irradianceSampler_2, prefilterMap_2, prefilterSampler_2, brdfLut_4, skylightPrefilterMap_1, _e213, _e214);
    ...
    let _e460 = f0_;
    let _e461 = evaluateStandardDirectX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_2.worldPos, in_2.ndc.xyz, _e457, _e458, v_5, _e459, metallic_16, a_5, _e460, _e452);
    color = (_e453 + _e461);
    let _e463 = color;
