...
fn cloud_direct_solar_factorX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX(worldPosition_2: vec3<f32>, shadowOrigin: vec3<f32>, shadowRight: vec3<f32>, shadowUp: vec3<f32>, shadowProjection: vec4<f32>) -> f32 {
    ...
    let uv_3 = ((vec2<f32>(dot(offset_1, shadowRight), dot(offset_1, shadowUp)) / vec2(max(shadowProjection.x, 0.000001f))) + vec2(0.5f));
    if all((uv_3 >= vec2(0f))) {
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_1: vec3<f32>, layer_6: u32, count_5: u32, normal_11: vec3<f32>, l_2: vec3<f32>) -> f32 {
    ...
    if (kernel == 3u) {
        if all((uv_5 >= (tileLo + texel_3))) {
            local_35 = all((uv_5 <= (tileHi - texel_3)));
        } else {
            local_35 = false;
        }
        let interior = local_35;
        if interior {
            let pcfFraction = fract(((uv_5 / texel_3) - vec2(0.5f)));
            ...
            let litLoLo = textureSampleCompareLevel(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, (uv_5 + (vec2<f32>(loOffset.x, loOffset.y) * texel_3)), adjustedDepth);
            let litHiLo = textureSampleCompareLevel(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, (uv_5 + (vec2<f32>(hiOffset.x, loOffset.y) * texel_3)), adjustedDepth);
            let litLoHi = textureSampleCompareLevel(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, (uv_5 + (vec2<f32>(loOffset.x, hiOffset.y) * texel_3)), adjustedDepth);
            let litHiHi = textureSampleCompareLevel(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, (uv_5 + (vec2<f32>(hiOffset.x, hiOffset.y) * texel_3)), adjustedDepth);
            ...
            }
            {
                ...
                loop {
                    ...
                    }
                    {
                        let _e210 = x;
                        let _e212 = y;
                        let offsetUv = clamp((uv_5 + (vec2<f32>(f32(_e210), f32(_e212)) * texel_3)), tileLo, tileHi);
        ...
        }
        {
            ...
            loop {
                ...
                }
                {
                    let _e246 = x_1;
                    let _e248 = y_1;
                    let offsetUv_1 = clamp((uv_5 + (vec2<f32>(f32(_e246), f32(_e248)) * texel_3)), tileLo, tileHi);
...
fn evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_12: vec3<f32>, worldPos_2: vec3<f32>, viewZ_1: f32) -> f32 {
    ...
    let _e36 = _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(receiverPosition, _e28, count_7, normal_12, l_4);
    shadow_3 = _e36;
    let _e40 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cascadeBlend;
    if (_e40 > 0f) {
        local_36 = ((_e28 + 1u) < count_7);
    } else {
        local_36 = false;
    }
    let _e49 = local_36;
    if _e49 {
        ...
        if (blendWidth > 0f) {
            ...
            if (t_1 > 0f) {
                let _e72 = _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(receiverPosition, (_e28 + 1u), count_7, normal_12, l_4);
...
fn encodeStandardNormalRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(normal_13: vec3<f32>, roughness_7: f32) -> u32 {
    var oct: vec2<f32>;

    let n = normalize(normal_13);
...
fn encodeStandardGBufferX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(normal_14: vec3<f32>, roughness_8: f32, albedo_3: vec3<f32>, metallic_4: f32, f0_4: vec3<f32>, occlusion: f32, emissive: vec3<f32>, opacity: f32, reflection: u32, probeRow_1: u32) -> GBufferOutputX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX {
    let _e5 = encodeStandardNormalRoughnessX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(normal_14, roughness_8);
...
fn evaluate_standard_surface(input_1: SurfaceInput, materialValue: MaterialParameters) -> SurfaceData {
    ...
    normal_15 = (normalize(input_1.geometricNormalWS) * faceDirection_1);
    ...
    if _e93 {
        ...
        normal_15 = (_e109 * faceDirection_1);
        usesNormal = true;
    }
