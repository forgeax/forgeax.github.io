...
fn fs_main(in_1: VsOut) -> @location(0) vec4<f32> {
    ...
    let _e156 = material.clearcoatRoughness;
    let coatRoughness = max(_e156, 0.04f);
    let coatAlpha = (coatRoughness * coatRoughness);
    let _e165 = f_schlickX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(max(dot(_e110, v), 0f), vec3(0.04f));
    let _e168 = material.clearcoat;
    let coatF = (_e165 * _e168);
    let _e175 = f_schlickX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(max(dot(_e110, v), 0f), f0_3);
    let kD = ((vec3(1f) - _e175) * (1f - metallic_7));
    let _e182 = material.roughness;
    let _e184 = finiteScalar(_e182, 0.5f);
    let iblRoughness = clamp((max(_e184, 0.04f) * roughnessTex), 0.04f, 1f);
    let _e193 = skylight.rotation;
    let _e196 = sampleIblDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e110, _e193, irradianceMap_1, irradianceSampler_1);
    let _e199 = skylight.rotation;
    let _e204 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e110, v, iblRoughness, f0_3, _e199, prefilterMap_1, prefilterSampler_1, brdfLut_1, brdfLutSampler_1);
    let _e207 = material.occlusionTextureCoordinatesTransform;
    let _e210 = material.occlusionTextureCoordinatesMetadata;
    let _e211 = transformedMaterialUv(_e207, _e210, in_1);
    let _e214 = material.occlusionTextureCoordinatesMetadata;
    let _e218 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(occlusionTexture, occlusionSampler, _e211, _e214.zw);
    let _e222 = material.occlusionStrength;
    let ao = mix(1f, _e218.x, _e222);
    let _e227 = skylight.colorR;
    let _e230 = skylight.colorG;
    let _e233 = skylight.colorB;
    let skyColor = vec3<f32>(_e227, _e230, _e233);
    let _e236 = diffuseAlbedo;
    ambient = ((((kD * _e196) * _e236) + _e204) * (vec3(1f) - coatF));
    let _e246 = material.clearcoat;
    if (_e246 != 0f) {
        let _e253 = skylight.rotation;
        let _e258 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e110, v, coatRoughness, vec3(0.04f), _e253, prefilterMap_1, prefilterSampler_1, brdfLut_1, brdfLutSampler_1);
        let _e259 = ambient;
        let _e262 = material.clearcoat;
        ambient = (_e259 + (_e258 * _e262));
    }
    let _e265 = ambient;
    let _e269 = skylight.intensity;
    ambient = (((_e265 * skyColor) * _e269) * ao);
    let ssaoUv = ((in_1.ndc.xy * vec2<f32>(0.5f, -0.5f)) + vec2<f32>(0.5f, 0.5f));
    let _e284 = textureSample(ssaoBlurredTexture, ssaoBlurredSampler, ssaoUv);
    let ssaoFactor = _e284.x;
    let _e286 = get_ssao_intensityX_naga_oil_mod_XMZXXEZ3FMF4F62DEOJYDUOTDNR2XG5DFOJPWM33SO5QXEZAX();
    let _e287 = ambient;
    ambient = (_e287 * mix(1f, (ssaoFactor * ao), _e286));
    let _e292 = ambient;
    color = _e292;
    let _e296 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e110, in_1.worldPos, in_1.viewZ);
    let _e297 = diffuseAlbedo;
    let _e298 = a;
    let _e299 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e110, v, _e297, metallic_7, _e298, f0_3);
    let _e300 = color;
    color = (_e300 + (_e296 * _e299));
    let _e305 = material.clearcoat;
    if (_e305 != 0f) {
        let _e313 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e110, v, vec3(0f), 1f, coatAlpha, vec3(0.04f));
        let _e314 = color;
        let _e317 = material.clearcoat;
        color = (_e314 + ((_e296 * _e317) * _e313));
    }
    let _e321 = color;
    let _e325 = diffuseAlbedo;
    let _e326 = a;
    let _e327 = evaluate_cluster_lightsX_naga_oil_mod_XMZXXEZ3FMF4F62DEOJYDUOTDNR2XG5DFOJPWM33SO5QXEZAX(in_1.ndc, in_1.viewZ, in_1.worldPos, _e110, v, _e325, metallic_7, _e326, f0_3);
    color = (_e321 + _e327);
    let _e331 = material.clearcoat;
    if (_e331 != 0f) {
        let _e334 = color;
        let _e337 = material.clearcoat;
        let _e346 = evaluate_cluster_lightsX_naga_oil_mod_XMZXXEZ3FMF4F62DEOJYDUOTDNR2XG5DFOJPWM33SO5QXEZAX(in_1.ndc, in_1.viewZ, in_1.worldPos, _e110, v, vec3(0f), 1f, coatAlpha, vec3(0.04f));
        color = (_e334 + (_e337 * _e346));
    }
    let _e351 = material.emissiveTextureCoordinatesTransform;
    let _e354 = material.emissiveTextureCoordinatesMetadata;
    let _e355 = transformedMaterialUv(_e351, _e354, in_1);
    let _e358 = material.emissiveTextureCoordinatesMetadata;
    let _e362 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(emissiveTexture, emissiveSampler, _e355, _e358.zw);
    let emissiveSample = _e362.xyz;
    let _e364 = color;
    let _e367 = material.emissive;
    let _e370 = material.emissiveIntensity;
    color = (_e364 + ((_e367 * _e370) * emissiveSample));
    let _e375 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX;
    let _e376 = color;
    let _e378 = applySceneFog(_e375, _e376, alpha_3, in_1.worldPos);
    return _e378;
}
