...
fn evalSpotShadowedX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(lightPos_3: vec3<f32>, lightDir_1: vec3<f32>, colorTimesIntensity_3: vec3<f32>, cosInner_1: f32, cosOuter_1: f32, invRangeSquared_3: f32, worldPos_5: vec3<f32>, normal_8: vec3<f32>, viewDir_4: vec3<f32>, baseColor_4: vec3<f32>, metallic_4: f32, alphaSq_4: f32, F0_7: vec3<f32>, lightViewProj: mat4x4<f32>, shadowAtlasTile: i32, depthBias_1: f32, normalBias_1: f32) -> vec3<f32> {
    ...
    let _e51 = local_9;
    if _e51 {
        local_10 = (clipUv.y >= 0f);
    } else {
        local_10 = false;
    }
    let _e58 = local_10;
    if _e58 {
        local_11 = (clipUv.y <= 1f);
    } else {
        local_11 = false;
    }
    let _e65 = local_11;
...
fn fs_main(in_1: VsOut) -> @location(0) vec4<f32> {
    ...
    let _e157 = material.transmissionTextureCoordinatesTransform;
    let _e160 = material.transmissionTextureCoordinatesMetadata;
    let _e161 = transformedMaterialUv(_e157, _e160, in_1);
    let _e164 = material.thicknessTextureCoordinatesTransform;
    let _e167 = material.thicknessTextureCoordinatesMetadata;
    let _e168 = transformedMaterialUv(_e164, _e167, in_1);
    let _e171 = material.transmissionTextureCoordinatesMetadata;
    let _e175 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(transmissionTexture, transmissionSampler, _e161, _e171.zw);
    let transmissionSample = _e175.x;
    let _e179 = material.thicknessTextureCoordinatesMetadata;
    let _e183 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(thicknessTexture, thicknessSampler, _e168, _e179.zw);
    let thicknessSample = _e183.y;
    let _e187 = material.transmission;
    let _e189 = finiteScalar(_e187, 0f);
    let _e191 = finiteScalar(transmissionSample, 1f);
    let transmissionFactor = clamp((_e189 * _e191), 0f, 1f);
    let _e198 = finiteScalar(dot(_e111, v), 0f);
    let refractionFromInside = (_e198 < 0f);
    let refractionNormal = select(_e111, -(_e111), refractionFromInside);
    ...
    let viewCos = clamp(abs(_e198), 0f, 1f);
    let _e214 = f_schlickX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(viewCos, vec3(dielectricF0_));
    let fresnel_1 = clamp(_e214.x, 0f, 1f);
    let transmittedEnergy = select(0f, ((transmissionFactor * (1f - metallic_5)) * (1f - fresnel_1)), (refractedLengthSquared > 0.000001f));
    diffuseAlbedo = (albedo * (1f - transmittedEnergy));
    let _e234 = material.clearcoatRoughness;
    let coatRoughness = max(_e234, 0.04f);
    let coatAlpha = (coatRoughness * coatRoughness);
    let _e243 = f_schlickX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(max(dot(_e111, v), 0f), vec3(0.04f));
    let _e246 = material.clearcoat;
    let coatF = (_e243 * _e246);
    let _e253 = f_schlickX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(max(dot(_e111, v), 0f), f0_1);
    let kD = ((vec3(1f) - _e253) * (1f - metallic_5));
    let _e260 = material.roughness;
    let _e262 = finiteScalar(_e260, 0.5f);
    let iblRoughness = clamp((max(_e262, 0.04f) * roughnessTex), 0.04f, 1f);
    let _e271 = skylight.rotation;
    let _e274 = sampleIblDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e111, _e271, irradianceMap_1, irradianceSampler_1);
    let _e277 = skylight.rotation;
    let _e282 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e111, v, iblRoughness, f0_1, _e277, prefilterMap_1, prefilterSampler_1, brdfLut_1, brdfLutSampler_1);
    let _e285 = material.occlusionTextureCoordinatesTransform;
    let _e288 = material.occlusionTextureCoordinatesMetadata;
    let _e289 = transformedMaterialUv(_e285, _e288, in_1);
    let _e292 = material.occlusionTextureCoordinatesMetadata;
    let _e296 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(occlusionTexture, occlusionSampler, _e289, _e292.zw);
    let _e300 = material.occlusionStrength;
    let ao = mix(1f, _e296.x, _e300);
    let _e305 = skylight.colorR;
    let _e308 = skylight.colorG;
    let _e311 = skylight.colorB;
    let skyColor = vec3<f32>(_e305, _e308, _e311);
    let _e314 = diffuseAlbedo;
    ambient = ((((kD * _e274) * _e314) + _e282) * (vec3(1f) - coatF));
    let _e324 = material.clearcoat;
    if (_e324 != 0f) {
        let _e331 = skylight.rotation;
        let _e336 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e111, v, coatRoughness, vec3(0.04f), _e331, prefilterMap_1, prefilterSampler_1, brdfLut_1, brdfLutSampler_1);
        let _e337 = ambient;
        let _e340 = material.clearcoat;
        ambient = (_e337 + (_e336 * _e340));
    }
    let _e343 = ambient;
    let _e347 = skylight.intensity;
    ambient = (((_e343 * skyColor) * _e347) * ao);
    let _e350 = ambient;
    color = _e350;
    let screenUv = ((in_1.ndc.xy * vec2<f32>(0.5f, -0.5f)) + vec2(0.5f));
    let _e364 = meshesX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX[0].worldFromLocal;
    let _e369 = instancesX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX[in_1.instanceIdx].localFromInstance;
    let localToWorld_1 = (_e364 * _e369);
    ...
    let _e423 = material.thickness;
    let _e425 = finiteScalar(_e423, 0f);
    let _e429 = finiteScalar(thicknessSample, 1f);
    let worldThickness = max(((_e425 * length(worldRefractedDirection)) * _e429), 0f);
    ...
    let _e471 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(screenUv, vec2(0f), vec2(1f)), backdropLod);
    let unrefractedBackdrop = _e471.xyz;
    ...
    let _e479 = local_1;
    if _e479 {
        let _e487 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(refractedUv, vec2(0f), vec2(1f)), backdropLod);
        transmittedBackdrop = _e487.xyz;
    }
    let _e491 = material.attenuationColor;
    let _e494 = finiteColor(_e491, vec3(1f));
    let safeAttenuationColor = clamp(_e494, vec3(0f), vec3(1f));
    let _e502 = material.attenuationDistance;
    let _e504 = finiteScalar(_e502, 0f);
    let safeAttenuationDistance = max(_e504, 0f);
    ...
    let _e521 = local_2;
    let beerAttenuation = select(vec3(1f), pow(safeAttenuationColor, vec3(attenuationExponent)), _e521);
    let _e523 = color;
    let _e524 = transmittedBackdrop;
    let _e527 = finiteColor(_e524, vec3(0f));
    color = (_e523 + ((_e527 * transmittedEnergy) * beerAttenuation));
    let _e533 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e111, in_1.worldPos, in_1.viewZ);
    let _e534 = diffuseAlbedo;
    let _e535 = a;
    let _e536 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e111, v, _e534, metallic_5, _e535, f0_1);
    let _e537 = color;
    color = (_e537 + (_e533 * _e536));
    let _e542 = material.clearcoat;
    if (_e542 != 0f) {
        let _e550 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e111, v, vec3(0f), 1f, coatAlpha, vec3(0.04f));
        let _e551 = color;
        let _e554 = material.clearcoat;
        color = (_e551 + ((_e533 * _e554) * _e550));
    }
    ...
    loop {
        ...
        {
            let _e566 = i;
            let p = pointLightsBufferX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.slots[_e566];
            let _e569 = color;
            let _e574 = diffuseAlbedo;
            let _e575 = a;
            let _e576 = evalPointX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(p.position, p.colorTimesIntensity, p.invRangeSquared, in_1.worldPos, _e111, v, _e574, metallic_5, _e575, f0_1);
            color = (_e569 + _e576);
            let _e580 = material.clearcoat;
            if (_e580 != 0f) {
                let _e583 = color;
                let _e586 = material.clearcoat;
                let _e596 = evalPointX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(p.position, p.colorTimesIntensity, p.invRangeSquared, in_1.worldPos, _e111, v, vec3(0f), 1f, coatAlpha, vec3(0.04f));
                color = (_e583 + (_e586 * _e596));
            }
        }
        continuing {
            let _e599 = i;
    ...
    loop {
        ...
        {
            let _e610 = i_1;
            let s_2 = spotLightsBufferX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.slots[_e610];
            if (s_2.shadowAtlasTile >= 0i) {
                let _e616 = color;
                let _e624 = diffuseAlbedo;
                let _e625 = a;
                let _e630 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.spotLightViewProj[s_2.shadowAtlasTile];
                let _e634 = evalSpotShadowedX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(s_2.position, s_2.direction, s_2.colorTimesIntensity, s_2.cosInner, s_2.cosOuter, s_2.invRangeSquared, in_1.worldPos, _e111, v, _e624, metallic_5, _e625, f0_1, _e630, s_2.shadowAtlasTile, 0.005f, 0.05f);
                color = (_e616 + _e634);
                let _e638 = material.clearcoat;
                if (_e638 != 0f) {
                    let _e641 = color;
                    let _e644 = material.clearcoat;
                    let _e660 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.spotLightViewProj[s_2.shadowAtlasTile];
                    let _e665 = evalSpotShadowedX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(s_2.position, s_2.direction, s_2.colorTimesIntensity, s_2.cosInner, s_2.cosOuter, s_2.invRangeSquared, in_1.worldPos, _e111, v, vec3(0f), 1f, coatAlpha, vec3(0.04f), _e660, s_2.shadowAtlasTile, 0.005f, 0.05f);
                    color = (_e641 + (_e644 * _e665));
                }
            } else {
                let _e668 = color;
                let _e676 = diffuseAlbedo;
                let _e677 = a;
                let _e678 = evalSpotX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(s_2.position, s_2.direction, s_2.colorTimesIntensity, s_2.cosInner, s_2.cosOuter, s_2.invRangeSquared, in_1.worldPos, _e111, v, _e676, metallic_5, _e677, f0_1);
                color = (_e668 + _e678);
                let _e682 = material.clearcoat;
                if (_e682 != 0f) {
                    let _e685 = color;
                    let _e688 = material.clearcoat;
                    let _e701 = evalSpotX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(s_2.position, s_2.direction, s_2.colorTimesIntensity, s_2.cosInner, s_2.cosOuter, s_2.invRangeSquared, in_1.worldPos, _e111, v, vec3(0f), 1f, coatAlpha, vec3(0.04f));
                    color = (_e685 + (_e688 * _e701));
                }
            }
        }
        continuing {
    ...
    let _e709 = material.emissiveTextureCoordinatesTransform;
    let _e712 = material.emissiveTextureCoordinatesMetadata;
    let _e713 = transformedMaterialUv(_e709, _e712, in_1);
    let _e716 = material.emissiveTextureCoordinatesMetadata;
    let _e720 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(emissiveTexture, emissiveSampler, _e713, _e716.zw);
    let emissiveSample = _e720.xyz;
    let _e722 = color;
    let _e725 = material.emissive;
    let _e728 = material.emissiveIntensity;
    color = (_e722 + ((_e725 * _e728) * emissiveSample));
    let _e733 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX;
    let _e734 = color;
    let _e736 = applySceneFog(_e733, _e734, alpha_3, in_1.worldPos);
    return _e736;
}
