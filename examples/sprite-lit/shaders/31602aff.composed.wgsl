...
fn packSceneTemporalV1WithValidityX_naga_oil_mod_XMZXXEZ3FMF4F643DMVXGKX3UMVWXA33SMFWAX(currentClip: vec4<f32>, previousClip: vec4<f32>, temporalProjection_2: vec4<f32>, reactive: f32, motionValid_1: bool) -> vec4<f32> {
    ...
    return vec4<f32>((_e1 - _e3), _e6, (clamp(reactive, 0f, 1f) + select(0f, 2f, !(motionValid_1))));
}
...
fn evaluate_cluster_lightX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(light: DirectLightSlotX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX, world_pos: vec3<f32>, normal_4: vec3<f32>, view_dir: vec3<f32>, base_color: vec3<f32>, metallic_6: f32, alpha_sq: f32, f0_1: vec3<f32>, flat_2d: bool, receive_shadows: bool) -> vec3<f32> {
    ...
    if (kind == KIND_SPOTX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX) {
        ...
        if _e84 {
            ...
            let _e102 = tile;
            let _e110 = evalSpotShadowedX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(light.position.xyz, light.direction.xyz, light.colorTimesIntensity.xyz, light.colorTimesIntensity.w, light.direction.w, light.position.w, world_pos, normal_4, view_dir, base_color, metallic_6, alpha_sq, f0_1, _e101, _e102, 0.005f, 0.05f, 3f, select(0f, 1f, receive_shadows));
            return (_e110 * modifier);
        }
        let _e124 = evalSpotX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(light.position.xyz, light.direction.xyz, light.colorTimesIntensity.xyz, light.colorTimesIntensity.w, light.direction.w, light.position.w, world_pos, normal_4, view_dir, base_color, metallic_6, alpha_sq, f0_1);
...
fn evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(ndc_1: vec3<f32>, view_z_2: f32, world_pos_1: vec3<f32>, normal_5: vec3<f32>, view_dir_1: vec3<f32>, base_color_1: vec3<f32>, metallic_7: f32, alpha_sq_1: f32, f0_2: vec3<f32>, flat_2d_1: bool, receive_shadows_1: bool) -> vec3<f32> {
    ...
    loop {
        ...
        }
        {
            ...
            let _e70 = evaluate_cluster_lightX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(light_1, world_pos_1, normal_5, view_dir_1, base_color_1, metallic_7, alpha_sq_1, f0_2, flat_2d_1, receive_shadows_1);
...
fn spriteLitShadeAccum(albedo_1: vec3<f32>, worldPos_11: vec3<f32>, ndc_2: vec3<f32>, viewZ: f32) -> vec3<f32> {
    ...
    let _e9 = lit;
    let _e22 = evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(ndc_2, viewZ, worldPos_11, vec3<f32>(0f, 0f, 1f), viewDir_4, albedo_1, 0f, 1f, vec3(0.04f), true, true);
    lit = (_e9 + _e22);
    let _e24 = lit;
    return _e24;
}
...
fn fs_main(in_1: VsOut) -> @location(0) vec4<f32> {
    applyViewClippingX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MNFYHA2LOM45DU4DMMFXGK4YX(in_1.worldPos, false);
    let _e6 = material.baseColorTextureCoordinatesMetadata;
    let _e10 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(baseColorTexture, baseColorSampler, in_1.uv_atlas, _e6.zw);
    let _e13 = material.colorTint;
    let albedo4_ = (_e10 * _e13);
    let _e19 = spriteLitShadeAccum(albedo4_.xyz, in_1.worldPos, in_1.ndc, in_1.viewZ);
    let lit_rgba = clamp(vec4<f32>(_e19, albedo4_.w), vec4(0f), vec4(1f));
    let premult = vec4<f32>((lit_rgba.xyz * lit_rgba.w), lit_rgba.w);
    let _e33 = linear_to_srgb(premult.x);
    let _e35 = linear_to_srgb(premult.y);
    let _e37 = linear_to_srgb(premult.z);
    return vec4<f32>(_e33, _e35, _e37, premult.w);
}
...
fn fs_main_hdr(in_2: VsOut) -> @location(0) vec4<f32> {
    ...
    let _e6 = material.baseColorTextureCoordinatesMetadata;
    let _e10 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(baseColorTexture, baseColorSampler, in_2.uv_atlas, _e6.zw);
    let _e13 = material.colorTint;
    let albedo4_1 = (_e10 * _e13);
    let _e19 = spriteLitShadeAccum(albedo4_1.xyz, in_2.worldPos, in_2.ndc, in_2.viewZ);
    let alpha = clamp(albedo4_1.w, 0f, 1f);
    return vec4<f32>((_e19 * alpha), alpha);
}
...
fn fs_temporal(in_4: TemporalVsOut) -> @location(0) vec4<f32> {
    ...
    let _e7 = material.baseColorTextureCoordinatesMetadata;
    let _e11 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(baseColorTexture, baseColorSampler, in_4.uvAtlas, _e7.zw);
    let _e16 = material.colorTint.w;
    let alpha_1 = clamp((_e11.w * _e16), 0f, 1f);
    if (alpha_1 <= 0f) {
        discard;
    }
    let _e27 = meshesX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX[0].temporal.y;
    motionValid = (_e27 >= 0.5f);
    let _e35 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.temporalProjection;
    let _e36 = motionValid;
    let _e38 = packSceneTemporalV1WithValidityX_naga_oil_mod_XMZXXEZ3FMF4F643DMVXGKX3UMVWXA33SMFWAX(in_4.currentClip, in_4.previousClip, _e35, 1f, _e36);
    return _e38;
}
