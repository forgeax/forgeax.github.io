...
fn sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(normal_3: vec3<f32>, view_2: vec3<f32>, roughness_3: f32, F0_3: vec3<f32>, worldPosition_1: vec3<f32>, boxCenter_1: vec3<f32>, boxExtents_1: vec3<f32>, rotation_3: vec4<f32>, probeMap: texture_cube<f32>, probeSampler: sampler, brdfLut_2: texture_2d<f32>, brdfLutSampler_2: sampler, skylightMap: texture_cube<f32>, skylightRotation: vec4<f32>, skylightScale: vec3<f32>, probeIntensity: f32, boxProjection: bool) -> vec3<f32> {
    ...
    let local_37 = (_e55 * probeIntensity);
    let _e58 = sky;
    return mix(_e58, local_37, weight);
}
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_1: vec3<f32>, layer_6: u32, count_5: u32, normal_11: vec3<f32>, l_2: vec3<f32>) -> f32 {
    ...
    if (kernel == 3u) {
        if all((uv_5 >= (tileLo + texel_3))) {
            local_32 = all((uv_5 <= (tileHi - texel_3)));
        } else {
            local_32 = false;
        }
...
fn projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX(baseColorAlpha_1: f32, alphaCutoff: f32, alphaHash: f32, positionOS: vec3<f32>, baseColorTextureEnabled: bool, baseColorTexture: texture_2d<f32>, baseColorSampler: sampler, transform_1: vec4<f32>, metadata_1: vec4<f32>, alphaTextureEnabled: bool, alphaTexture: texture_2d<f32>, alphaSampler: sampler, alphaTransform: vec4<f32>, alphaMetadata: vec4<f32>, alphaChannel: f32, currentClip_1: vec4<f32>, previousClip_1: vec4<f32>, temporalProjection_3: vec4<f32>, reactive_3: f32, motionValid_2: bool, uv0_1: vec2<f32>, uv1_1: vec2<f32>, uv2_1: vec2<f32>, uv3_1: vec2<f32>, uv4_1: vec2<f32>, uv5_1: vec2<f32>, uv6_1: vec2<f32>, uv7_1: vec2<f32>) -> vec4<f32> {
    ...
    if (alphaCutoff > 0f) {
        local_34 = (coverage_2 <= alphaCutoff);
    } else {
        local_34 = false;
    }
    let _e49 = local_34;
...
fn alphaTestSurface(surface: SurfaceData) {
    ...
    if (surface.alphaClipThreshold > 0f) {
        local_36 = (surface.opacity <= surface.alphaClipThreshold);
    } else {
        local_36 = false;
    }
    let _e10 = local_36;
...
fn fs_main(in_1: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let _e200 = evaluateStandardEnvironmentX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_1.worldPos, _e187, v, _e188, metallic_7, iblRoughness, _e189, _e191, irradianceMap_2, irradianceSampler_2, prefilterMap_2, prefilterSampler_2, brdfLut_4, skylightPrefilterMap_1, _e192, _e193);
    ...
    let _e308 = material.thickness;
    let _e310 = finiteScalar(_e308, 0f);
    let _e313 = thicknessSample;
    let _e315 = finiteScalar(_e313, 1f);
    let worldThickness = max(((_e310 * length(worldRefractedDirection)) * _e315), 0f);
    ...
    let _e357 = textureSampleLevel(transmissionBackdropTexture, prefilterSampler_2, clamp(screenUv, vec2(0f), vec2(1f)), backdropLod);
    let unrefractedBackdrop = _e357.xyz;
    ...
    let _e365 = local_1;
    if _e365 {
        let _e373 = textureSampleLevel(transmissionBackdropTexture, prefilterSampler_2, clamp(refractedUv, vec2(0f), vec2(1f)), backdropLod);
        transmittedBackdrop = _e373.xyz;
    }
    let _e377 = material.attenuationColor;
    let _e380 = finiteColor(_e377, vec3(1f));
    let safeAttenuationColor = clamp(_e380, vec3(0f), vec3(1f));
    let _e388 = material.attenuationDistance;
    let _e390 = finiteScalar(_e388, 0f);
    let safeAttenuationDistance = max(_e390, 0f);
    ...
    let _e407 = local_2;
    let beerAttenuation = select(vec3(1f), pow(safeAttenuationColor, vec3(attenuationExponent)), _e407);
    let _e409 = color;
    let _e410 = transmittedBackdrop;
    let _e413 = finiteColor(_e410, vec3(0f));
    color = (_e409 + ((_e413 * transmittedEnergy) * beerAttenuation));
    let _e417 = physicalNormal;
    let _e419 = standardViewZ(in_1);
    let _e420 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e417, in_1.worldPos, _e419);
    let _e421 = color;
    let _e425 = standardViewZ(in_1);
    let _e426 = physicalNormal;
    let _e427 = diffuseAlbedo;
    let _e428 = f0_;
    let _e429 = evaluateStandardDirectX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_1.worldPos, in_1.ndc.xyz, _e425, _e426, v, _e427, metallic_7, a_4, _e428, _e420);
    color = (_e421 + _e429);
    let _e431 = color;
    color = (_e431 + _e6.emissive);
    let _e436 = color;
    output.color = vec4<f32>(_e436, alpha_5);
    let _e439 = reflectionFallback;
    let _e440 = standardSsrCoverage();
    output.reflectionFallback = vec4<f32>(_e439, _e440);
    let _e442 = output;
    return _e442;
}
