...
fn evaluate_point_lightX_naga_oil_mod_XMZXXEZ3FMF4F62DEOJYDUOTDNR2XG5DFOJPWM33SO5QXEZAX(light: LightSlotX_naga_oil_mod_XMZXXEZ3FMF4F62DEOJYDUOTDNR2XG5DFOJPWM33SO5QXEZAX, world_pos: vec3<f32>, N: vec3<f32>, V: vec3<f32>, base_color: vec3<f32>, metallic: f32, roughness_2: f32) -> vec3<f32> {
    ...
    let F0_3 = mix(vec3(0.04f), base_color, metallic);
    let F = (F0_3 + ((vec3(1f) - F0_3) * pow((1f - VdotH), 5f)));
...
fn evaluate_spot_lightX_naga_oil_mod_XMZXXEZ3FMF4F62DEOJYDUOTDNR2XG5DFOJPWM33SO5QXEZAX(light_1: LightSlotX_naga_oil_mod_XMZXXEZ3FMF4F62DEOJYDUOTDNR2XG5DFOJPWM33SO5QXEZAX, world_pos_1: vec3<f32>, N_1: vec3<f32>, V_1: vec3<f32>, base_color_1: vec3<f32>, metallic_1: f32, roughness_3: f32) -> vec3<f32> {
    ...
    let F0_4 = mix(vec3(0.04f), base_color_1, metallic_1);
    let F_1 = (F0_4 + ((vec3(1f) - F0_4) * pow((1f - VdotH_1), 5f)));
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos: vec3<f32>, layer_3: u32, count_2: u32, normal_2: vec3<f32>, l: vec3<f32>) -> f32 {
    ...
    let bias = max((0.05f * (1f - dot(normal_2, l))), 0.005f);
    ...
    let _e81 = textureDimensions(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, 0i);
    let texelDims = vec2<f32>(_e81);
        ...
        }
        {
            ...
            loop {
                ...
                }
                {
                    let _e103 = x;
                    let _e105 = y;
                    let offsetUv = clamp((uv + (vec2<f32>(f32(_e103), f32(_e105)) * texel)), tileLo, tileHi);
...
fn evalDirectionalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_3: vec3<f32>, viewDir: vec3<f32>, baseColor: vec3<f32>, metallic_3: f32, alphaSq: f32, F0_2: vec3<f32>, worldPos_1: vec3<f32>, viewZ: f32) -> vec3<f32> {
    var shadow: f32;
    var local_4: bool;
    ...
    let _e94 = shadow;
    return ((((diffuse_2 + specular_2) * _e91) * nDotL_1) * _e94);
}
...
fn fs_main(in_1: VsOut) -> @location(0) vec4<f32> {
    ...
    let _e38 = a;
    a = (_e37 * _e38);
    let _e43 = textureSample(normalTexture, normalSampler, in_1.uv);
    let normSampleRg = _e43.xy;
    let _e45 = decodeTangentSpaceNormalRgX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DCNYX(normSampleRg);
    let _e48 = applyTBNX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DCNYX(in_1.worldNormal, in_1.worldTangent, _e45);
    let _e51 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cameraPos;
    let v = normalize((_e51 - in_1.worldPos));
    let f0_1 = mix(vec3(0.04f), albedo, metallic_4);
    let _e63 = f_schlickX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(max(dot(_e48, v), 0f), f0_1);
    let kD_2 = ((vec3(1f) - _e63) * (1f - metallic_4));
    let _e70 = material.roughness;
    let iblRoughness = (max(_e70, 0.04f) * _e28);
    let _e76 = sampleIblDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e48, irradianceMap_1, irradianceSampler_1);
    let _e81 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e48, v, iblRoughness, f0_1, prefilterMap_1, prefilterSampler_1, brdfLut_1, brdfLutSampler_1);
    let aoSample = textureSample(occlusionTexture, occlusionSampler, in_1.uv);
    let _e89 = material.occlusionStrength;
    let ao = mix(1f, aoSample.x, _e89);
    let _e94 = skylight.colorR;
    let _e97 = skylight.colorG;
    let _e100 = skylight.colorB;
    let skyColor = vec3<f32>(_e94, _e97, _e100);
    let _e108 = skylight.intensity;
    ambient = ((((((kD_2 * _e76) * albedo) + _e81) * skyColor) * _e108) * ao);
    let ssaoUv = ((in_1.ndc.xy * vec2<f32>(0.5f, -0.5f)) + vec2<f32>(0.5f, 0.5f));
    let _e124 = textureSample(ssaoBlurredTexture, ssaoBlurredSampler, ssaoUv);
    let ssaoFactor = _e124.x;
    let _e126 = get_ssao_intensityX_naga_oil_mod_XMZXXEZ3FMF4F62DEOJYDUOTDNR2XG5DFOJPWM33SO5QXEZAX();
    let _e127 = ambient;
    ambient = (_e127 * mix(1f, (ssaoFactor * ao), _e126));
    let _e132 = ambient;
    color = _e132;
    let _e134 = color;
    let _e135 = a;
    let _e138 = evalDirectionalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e48, v, albedo, metallic_4, _e135, f0_1, in_1.worldPos, in_1.viewZ);
    color = (_e134 + _e138);
    let _e140 = color;
    let _e144 = a;
    let _e145 = evaluate_cluster_lightsX_naga_oil_mod_XMZXXEZ3FMF4F62DEOJYDUOTDNR2XG5DFOJPWM33SO5QXEZAX(in_1.ndc, in_1.viewZ, in_1.worldPos, _e48, v, albedo, metallic_4, _e144);
    color = (_e140 + _e145);
    let _e150 = textureSample(emissiveTexture, emissiveSampler, in_1.uv);
    let emissiveSample = _e150.xyz;
    let _e152 = color;
    let _e155 = material.emissive;
    let _e158 = material.emissiveIntensity;
    color = (_e152 + ((_e155 * _e158) * emissiveSample));
    let _e162 = color;
    let _e166 = material.baseColor.w;
    return vec4<f32>(_e162, (_e166 * baseSample.w));
}
