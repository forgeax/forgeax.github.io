...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos: vec3<f32>, layer_4: u32, count_4: u32, normal_10: vec3<f32>, l_1: vec3<f32>) -> f32 {
    ...
    if (kernel == 3u) {
        if all((uv_4 >= (tileLo + texel))) {
            local_17 = all((uv_4 <= (tileHi - texel)));
        } else {
            local_17 = false;
        }
...
fn projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX(baseColorAlpha_1: f32, alphaCutoff: f32, alphaHash: f32, positionOS: vec3<f32>, baseColorTextureEnabled: bool, baseColorTexture: texture_2d<f32>, baseColorSampler: sampler, transform_1: vec4<f32>, metadata_1: vec4<f32>, alphaTextureEnabled: bool, alphaTexture: texture_2d<f32>, alphaSampler: sampler, alphaTransform: vec4<f32>, alphaMetadata: vec4<f32>, alphaChannel: f32, currentClip_1: vec4<f32>, previousClip_1: vec4<f32>, temporalProjection_3: vec4<f32>, reactive_3: f32, motionValid_2: bool, uv0_1: vec2<f32>, uv1_1: vec2<f32>, uv2_1: vec2<f32>, uv3_1: vec2<f32>, uv4_1: vec2<f32>, uv5_1: vec2<f32>, uv6_1: vec2<f32>, uv7_1: vec2<f32>) -> vec4<f32> {
    ...
    if (alphaCutoff > 0f) {
        local_19 = (coverage_2 <= alphaCutoff);
    } else {
        local_19 = false;
    }
    let _e49 = local_19;
...
fn fs_main(in_1: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let _e117 = evaluateStandardEnvironmentX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_1.worldPos, _e104, v, _e105, metallic_7, iblRoughness, _e106, _e108, irradianceMap_2, irradianceSampler_2, prefilterMap_2, prefilterSampler_2, brdfLut_4, skylightPrefilterMap_1, _e109, _e110);
    ...
    let _e152 = ambient;
    color = _e152;
    let _e154 = physicalNormal;
    let _e156 = standardViewZ(in_1);
    let _e157 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e154, in_1.worldPos, _e156);
    let _e158 = color;
    let _e162 = standardViewZ(in_1);
    let _e163 = physicalNormal;
    let _e164 = diffuseAlbedo;
    let _e165 = f0_;
    let _e166 = evaluateStandardDirectX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_1.worldPos, in_1.ndc.xyz, _e162, _e163, v, _e164, metallic_7, a_4, _e165, _e157);
    color = (_e158 + _e166);
    let _e168 = color;
    color = (_e168 + _e5.emissive);
    let _e173 = color;
    output.color = vec4<f32>(_e173, alpha_5);
    let _e176 = reflectionFallback;
    let _e177 = standardSsrCoverage();
    output.reflectionFallback = vec4<f32>(_e176, _e177);
    let _e179 = output;
    return _e179;
}
