...
fn evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_18: vec3<f32>, worldPos_13: vec3<f32>, viewZ_1: f32) -> f32 {
    ...
    let _e36 = _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(receiverPosition, _e28, count_6, normal_18, l_7);
    shadow_2 = _e36;
    ...
    if _e49 {
        ...
        if (blendWidth > 0f) {
            ...
            if (t_1 > 0f) {
                let _e72 = _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(receiverPosition, (_e28 + 1u), count_6, normal_18, l_7);
...
fn encodeStandardGBufferX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(normal_19: vec3<f32>, roughness_7: f32, albedo_3: vec3<f32>, metallic_12: f32, f0_6: vec3<f32>, occlusion: f32, emissive: vec3<f32>, opacity: f32, reflection: u32, probeRow_1: u32) -> GBufferOutputX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX {
    return GBufferOutputX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(vec4<f32>(((normalize(normal_19) * 0.5f) + vec3(0.5f)), roughness_7), pack4x8unorm(vec4<f32>(albedo_3, metallic_12)), vec4<f32>(f0_6, occlusion), vec4<f32>(emissive, opacity), ((reflection << 24u) | probeRow_1));
}
...
fn evaluate_standard_surface(input_2: SurfaceInput, materialValue: MaterialParameters) -> SurfaceData {
    ...
    normal_20 = normalize(input_2.geometricNormalWS);
    ...
    let metallic_13 = clamp((materialValue.metallic * _e75), 0f, 1f);
    let _e81 = metallicRoughnessSample;
    let _e84 = surfaceChannel(_e81, u32(materialValue.roughnessChannel));
    let roughness_10 = clamp((materialValue.roughness * _e84), 0.04f, 1f);
    ...
    return SurfaceData(baseColor_9, _e106, metallic_13, roughness_10, emissive_1, occlusion_1, clamp(((materialValue.baseColor.w * _e110) * vertexColor.w), 0f, 1f), clamp(materialValue.alphaCutoff, 0f, 1f));
}
...
fn blendLinearTransparent(source_3: vec3<f32>, destination: vec3<f32>, alpha_1: f32) -> vec3<f32> {
    return ((source_3 * alpha_1) + (destination * (1f - alpha_1)));
}
...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let _e96 = finiteScalar(dot(n_2, v_2), 0f);
    ...
    let _e460 = f0_;
    let _e461 = evaluateStandardDirectX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_2.worldPos, in_2.ndc.xyz, _e457, _e458, v_2, _e459, metallic_15, a_3, _e460, _e452);
    color = (_e453 + _e461);
    let _e463 = color;
