...
fn _samplePcssForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_14: vec3<f32>, layer_5: u32, count_4: u32, normal_16: vec3<f32>, l_1: vec3<f32>, profile: u32) -> f32 {
    ...
    let _e60 = local_33;
    if _e60 {
        local_34 = (projCoords.z >= 0f);
    } else {
        local_34 = false;
    }
    let _e67 = local_34;
    if _e67 {
        local_35 = (projCoords.z <= 1f);
    } else {
        local_35 = false;
    }
    let _e74 = local_35;
    ...
    if (lightDepthWorldSpan > 0f) {
        local_39 = (worldUnitsPerTexel > 0f);
    } else {
        local_39 = false;
    }
    let _e213 = local_39;
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_15: vec3<f32>, layer_6: u32, count_5: u32, normal_17: vec3<f32>, l_2: vec3<f32>) -> f32 {
    ...
    if (tileUv_1.x >= 0f) {
        local_42 = (tileUv_1.x <= 1f);
    } else {
        local_42 = false;
    }
    let _e40 = local_42;
    if _e40 {
        local_43 = (tileUv_1.y >= 0f);
    } else {
        local_43 = false;
    }
    let _e47 = local_43;
    if _e47 {
        local_44 = (tileUv_1.y <= 1f);
    } else {
        local_44 = false;
    }
    let _e54 = local_44;
    if _e54 {
        local_45 = (currentDepth <= 1f);
    } else {
        local_45 = false;
    }
    let _e60 = local_45;
    ...
    if (kernel == 3u) {
        if all((uv_7 >= (tileLo + texel_5))) {
            local_46 = all((uv_7 <= (tileHi - texel_5)));
        } else {
            local_46 = false;
        }
        let interior = local_46;
        if interior {
...
fn encodeStandardGBufferX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(normal_19: vec3<f32>, roughness_8: f32, albedo_3: vec3<f32>, metallic_13: f32, f0_6: vec3<f32>, occlusion: f32, emissive: vec3<f32>, opacity: f32, reflection: u32, probeRow_1: u32) -> GBufferOutputX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX {
    return GBufferOutputX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(vec4<f32>(((normalize(normal_19) * 0.5f) + vec3(0.5f)), roughness_8), pack4x8unorm(vec4<f32>(albedo_3, metallic_13)), vec4<f32>(f0_6, occlusion), vec4<f32>(emissive, opacity), ((reflection << 24u) | probeRow_1));
}
...
fn projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX(baseColorAlpha_1: f32, alphaCutoff: f32, baseColorTextureEnabled: bool, baseColorTexture: texture_2d<f32>, baseColorSampler: sampler, transform_1: vec4<f32>, metadata_3: vec4<f32>, currentClip_1: vec4<f32>, previousClip_1: vec4<f32>, temporalProjection_3: vec4<f32>, reactive_3: f32, motionValid_2: bool, uv0_1: vec2<f32>, uv1_1: vec2<f32>, uv2_1: vec2<f32>, uv3_1: vec2<f32>, uv4_1: vec2<f32>, uv5_1: vec2<f32>, uv6_1: vec2<f32>, uv7_1: vec2<f32>) -> vec4<f32> {
    ...
    if (alphaCutoff > 0f) {
        ...
        local_48 = ((baseColorAlpha_1 * _e24) <= alphaCutoff);
    } else {
        local_48 = false;
    }
    let _e30 = local_48;
...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let _e96 = finiteScalar(dot(n_5, v_5), 0f);
    ...
    let _e460 = f0_;
    let _e461 = evaluateStandardDirectX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_2.worldPos, in_2.ndc.xyz, _e457, _e458, v_5, _e459, metallic_16, a_4, _e460, _e452);
    color = (_e453 + _e461);
    let _e463 = color;
