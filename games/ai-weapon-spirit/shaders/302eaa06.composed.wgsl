...
fn evaluateProbeDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX(shPreblend_1: array<vec4<f32>, 9>, localBlendFraction: f32, normal_1: vec3<f32>, e_sky: vec3<f32>, k_d: vec3<f32>, albedo: vec3<f32>, metallic: f32) -> vec3<f32> {
    ...
    let local_31 = (max(_e2, vec3(0f)) * PROBE_INV_PIX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX);
    let skyResidualFraction = (1f - localBlendFraction);
    return ((((local_31 + (skyResidualFraction * e_sky)) * k_d) * albedo) * (1f - metallic));
}
...
fn sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(normal_5: vec3<f32>, view_2: vec3<f32>, roughness_3: f32, F0_3: vec3<f32>, worldPosition_1: vec3<f32>, boxCenter_1: vec3<f32>, boxExtents_1: vec3<f32>, rotation_3: vec4<f32>, probeMap: texture_cube<f32>, probeSampler: sampler, brdfLut_2: texture_2d<f32>, brdfLutSampler_2: sampler, skylightMap: texture_cube<f32>, skylightRotation: vec4<f32>, skylightScale: vec3<f32>, probeIntensity: f32, boxProjection: bool) -> vec3<f32> {
    ...
    let local_32 = (_e55 * probeIntensity);
    let _e58 = sky;
    return mix(_e58, local_32, weight);
}
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_14: vec3<f32>, layer_4: u32, count_4: u32, normal_14: vec3<f32>, l_1: vec3<f32>) -> f32 {
    var local_16: bool;
    var local_17: bool;
    var local_18: bool;
    var local_19: bool;
    ...
    if (kernel == 3u) {
        if all((uv_5 >= (tileLo + texel_2))) {
            local_20 = all((uv_5 <= (tileHi - texel_2)));
        } else {
            local_20 = false;
        }
...
fn evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_15: vec3<f32>, worldPos_15: vec3<f32>, viewZ: f32) -> f32 {
    var shadow_1: f32;
    var local_21: bool;
    ...
    let receiverPosition = (worldPos_15 + (normal_15 * _e33));
...
fn cloud_direct_solar_factorX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX(worldPosition_3: vec3<f32>, shadowOrigin: vec3<f32>, shadowRight: vec3<f32>, shadowUp: vec3<f32>, shadowProjection: vec4<f32>) -> f32 {
    ...
    if (worldPosition_3.x == worldPosition_3.x) {
        local_22 = (worldPosition_3.y == worldPosition_3.y);
    } else {
        local_22 = false;
    }
    let _e10 = local_22;
    if _e10 {
        local_23 = (worldPosition_3.z == worldPosition_3.z);
    } else {
        local_23 = false;
    }
    let _e17 = local_23;
    if _e17 {
        local_24 = all((abs(worldPosition_3) < vec3(3.402823e38f)));
    } else {
        local_24 = false;
    }
    let validReceiver = local_24;
    ...
    if all((uv_6 >= vec2(0f))) {
        local_25 = all((uv_6 <= vec2(1f)));
    } else {
        local_25 = false;
    }
    let inRange = local_25;
    if (shadowProjection.y > 0.5f) {
        local_26 = (shadowProjection.z < 0.5f);
    } else {
        local_26 = false;
    }
    let _e64 = local_26;
    if _e64 {
        local_27 = (shadowProjection.x > 0f);
    } else {
        local_27 = false;
    }
    let available = local_27;
