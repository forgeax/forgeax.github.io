...
fn _samplePcssForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_14: vec3<f32>, layer_5: u32, count_4: u32, normal_16: vec3<f32>, l_1: vec3<f32>, profile: u32) -> f32 {
    ...
    let _e60 = local_27;
    if _e60 {
        local_28 = (projCoords.z >= 0f);
    } else {
        local_28 = false;
    }
    let _e67 = local_28;
    if _e67 {
        local_29 = (projCoords.z <= 1f);
    } else {
        local_29 = false;
    }
    let _e74 = local_29;
    ...
    if (lightDepthWorldSpan > 0f) {
        local_33 = (worldUnitsPerTexel > 0f);
    } else {
        local_33 = false;
    }
    let _e213 = local_33;
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_15: vec3<f32>, layer_6: u32, count_5: u32, normal_17: vec3<f32>, l_2: vec3<f32>) -> f32 {
    ...
    if (tileUv_1.x >= 0f) {
        local_36 = (tileUv_1.x <= 1f);
    } else {
        local_36 = false;
    }
    let _e40 = local_36;
    if _e40 {
        local_37 = (tileUv_1.y >= 0f);
    } else {
        local_37 = false;
    }
    let _e47 = local_37;
    if _e47 {
        local_38 = (tileUv_1.y <= 1f);
    } else {
        local_38 = false;
    }
    let _e54 = local_38;
    if _e54 {
        local_39 = (currentDepth <= 1f);
    } else {
        local_39 = false;
    }
    let _e60 = local_39;
    ...
    if (kernel == 3u) {
        if all((uv_7 >= (tileLo + texel_5))) {
            local_40 = all((uv_7 <= (tileHi - texel_5)));
        } else {
            local_40 = false;
        }
        let interior = local_40;
        if interior {
...
fn projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX(baseColorAlpha_1: f32, alphaCutoff: f32, baseColorTextureEnabled: bool, baseColorTexture: texture_2d<f32>, baseColorSampler: sampler, transform_1: vec4<f32>, metadata_3: vec4<f32>, currentClip_1: vec4<f32>, previousClip_1: vec4<f32>, temporalProjection_3: vec4<f32>, reactive_3: f32, motionValid_2: bool, uv0_1: vec2<f32>, uv1_1: vec2<f32>, uv2_1: vec2<f32>, uv3_1: vec2<f32>, uv4_1: vec2<f32>, uv5_1: vec2<f32>, uv6_1: vec2<f32>, uv7_1: vec2<f32>) -> vec4<f32> {
    ...
    if (alphaCutoff > 0f) {
        ...
        local_42 = ((baseColorAlpha_1 * _e24) <= alphaCutoff);
    } else {
        local_42 = false;
    }
    let _e30 = local_42;
...
fn fs_main(in_1: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let n_5 = normalize(_e5.normalWS);
    ...
    let _e176 = f0_;
    let _e177 = evaluateStandardDirectX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_1.worldPos, in_1.ndc.xyz, _e173, _e174, v_5, _e175, metallic_16, a_4, _e176, _e168);
    color = (_e169 + _e177);
    let _e179 = color;
