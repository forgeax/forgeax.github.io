...
fn cloud_direct_solar_factorX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX(worldPosition_2: vec3<f32>, shadowOrigin: vec3<f32>, shadowRight: vec3<f32>, shadowUp: vec3<f32>, shadowProjection: vec4<f32>) -> f32 {
    ...
    let uv_1 = ((vec2<f32>(dot(offset_1, shadowRight), dot(offset_1, shadowUp)) / vec2(max(shadowProjection.x, 0.000001f))) + vec2(0.5f));
    if all((uv_1 >= vec2(0f))) {
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_1: vec3<f32>, layer_6: u32, count_5: u32, normal_11: vec3<f32>, l_2: vec3<f32>) -> f32 {
    ...
    if (kernel == 3u) {
        if all((uv_3 >= (tileLo + texel_3))) {
            local_25 = all((uv_3 <= (tileHi - texel_3)));
        } else {
            local_25 = false;
        }
        let interior = local_25;
        if interior {
            let pcfFraction = fract(((uv_3 / texel_3) - vec2(0.5f)));
            ...
            let litLoLo = textureSampleCompareLevel(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, (uv_3 + (vec2<f32>(loOffset.x, loOffset.y) * texel_3)), adjustedDepth);
            let litHiLo = textureSampleCompareLevel(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, (uv_3 + (vec2<f32>(hiOffset.x, loOffset.y) * texel_3)), adjustedDepth);
            let litLoHi = textureSampleCompareLevel(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, (uv_3 + (vec2<f32>(loOffset.x, hiOffset.y) * texel_3)), adjustedDepth);
            let litHiHi = textureSampleCompareLevel(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, (uv_3 + (vec2<f32>(hiOffset.x, hiOffset.y) * texel_3)), adjustedDepth);
            ...
            }
            {
                ...
                loop {
                    ...
                    }
                    {
                        let _e210 = x_1;
                        let _e212 = y_1;
                        let offsetUv = clamp((uv_3 + (vec2<f32>(f32(_e210), f32(_e212)) * texel_3)), tileLo, tileHi);
        ...
        }
        {
            ...
            loop {
                ...
                }
                {
                    let _e246 = x_2;
                    let _e248 = y_2;
                    let offsetUv_1 = clamp((uv_3 + (vec2<f32>(f32(_e246), f32(_e248)) * texel_3)), tileLo, tileHi);
...
fn resolveStandardDeferred(in_2: FullscreenOutputX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX) -> DeferredOutput {
    ...
    let ndc_1 = vec3<f32>(((uv_4 * vec2<f32>(2f, -2f)) + vec2<f32>(-1f, 1f)), depth);
    ...
    let normal_13 = normalize(((normalSample.xyz * 2f) - vec3(1f)));
    ...
    let _e131 = evaluateStandardEnvironmentX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(position, normal_13, direction_4, albedo_3.xyz, albedo_3.w, roughness_8, response.xyz, _e122, irradianceMap_2, linearSampler, prefilterMap_2, linearSampler, brdfLut_4, skylightPrefilterMap_1, _e123, _e124);
    let _e134 = params.x;
    if (_e134 > 0f) {
    ...
    let _e163 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_13, position, _e162);
    let _e168 = evaluateStandardDirectX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(position, ndc_1, _e162, normal_13, direction_4, albedo_3.xyz, albedo_3.w, (roughness_8 * roughness_8), response.xyz, _e163);
