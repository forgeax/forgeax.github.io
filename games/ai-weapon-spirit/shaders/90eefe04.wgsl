...
fn sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(normal_3: vec3<f32>, view_2: vec3<f32>, roughness_3: f32, F0_3: vec3<f32>, worldPosition_1: vec3<f32>, boxCenter_1: vec3<f32>, boxExtents_1: vec3<f32>, rotation_3: vec4<f32>, probeMap: texture_cube<f32>, probeSampler: sampler, brdfLut_2: texture_2d<f32>, brdfLutSampler_2: sampler, skylightMap: texture_cube<f32>, skylightRotation: vec4<f32>, skylightScale: vec3<f32>, probeIntensity: f32, boxProjection: bool) -> vec3<f32> {
    ...
    let local_42 = (_e55 * probeIntensity);
    let _e58 = sky;
    return mix(_e58, local_42, weight);
}
...
fn _samplePcssForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_11: vec3<f32>, layer_5: u32, count_4: u32, normal_16: vec3<f32>, l_1: vec3<f32>, profile: u32) -> f32 {
    ...
    if (tileUv.x >= 0f) {
        local_23 = (tileUv.x <= 1f);
    } else {
        local_23 = false;
    }
    let _e46 = local_23;
    if _e46 {
        local_24 = (tileUv.y >= 0f);
    } else {
        local_24 = false;
    }
    let _e53 = local_24;
    if _e53 {
        local_25 = (tileUv.y <= 1f);
    } else {
        local_25 = false;
    }
    let _e60 = local_25;
    if _e60 {
        local_26 = (projCoords.z >= 0f);
    } else {
        local_26 = false;
    }
    let _e67 = local_26;
    if _e67 {
        local_27 = (projCoords.z <= 1f);
    } else {
        local_27 = false;
    }
    let _e74 = local_27;
    ...
    if (lightDepthWorldSpan > 0f) {
        local_31 = (worldUnitsPerTexel > 0f);
    } else {
        local_31 = false;
    }
    let _e213 = local_31;
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_12: vec3<f32>, layer_6: u32, count_5: u32, normal_17: vec3<f32>, l_2: vec3<f32>) -> f32 {
    ...
    if (tileUv_1.x >= 0f) {
        local_34 = (tileUv_1.x <= 1f);
    } else {
        local_34 = false;
    }
    let _e40 = local_34;
    if _e40 {
        local_35 = (tileUv_1.y >= 0f);
    } else {
        local_35 = false;
    }
    let _e47 = local_35;
    if _e47 {
        local_36 = (tileUv_1.y <= 1f);
    } else {
        local_36 = false;
    }
    let _e54 = local_36;
    if _e54 {
        local_37 = (currentDepth <= 1f);
    } else {
        local_37 = false;
    }
    let _e60 = local_37;
    ...
    if (kernel == 3u) {
        if all((uv_7 >= (tileLo + texel_5))) {
            local_38 = all((uv_7 <= (tileHi - texel_5)));
        } else {
            local_38 = false;
        }
        let interior = local_38;
        if interior {
...
fn projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX(baseColorAlpha_1: f32, alphaCutoff: f32, baseColorTextureEnabled: bool, baseColorTexture: texture_2d<f32>, baseColorSampler: sampler, transform_1: vec4<f32>, metadata_3: vec4<f32>, currentClip_1: vec4<f32>, previousClip_1: vec4<f32>, temporalProjection_3: vec4<f32>, reactive_3: f32, motionValid_2: bool, uv0_1: vec2<f32>, uv1_1: vec2<f32>, uv2_1: vec2<f32>, uv3_1: vec2<f32>, uv4_1: vec2<f32>, uv5_1: vec2<f32>, uv6_1: vec2<f32>, uv7_1: vec2<f32>) -> vec4<f32> {
    ...
    if (alphaCutoff > 0f) {
        ...
        local_40 = ((baseColorAlpha_1 * _e24) <= alphaCutoff);
    } else {
        local_40 = false;
    }
    let _e30 = local_40;
...
fn alphaTestSurface(surface: SurfaceData) {
    ...
    if (surface.alphaClipThreshold > 0f) {
        local_41 = (surface.opacity <= surface.alphaClipThreshold);
    } else {
        local_41 = false;
    }
    let _e10 = local_41;
...
fn fs_main(in_1: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let _e439 = f0_;
    let _e440 = evaluateStandardDirectX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_1.worldPos, in_1.ndc.xyz, _e436, _e437, v_2, _e438, metallic_15, a_3, _e439, _e431);
    color = (_e432 + _e440);
    let _e442 = color;
