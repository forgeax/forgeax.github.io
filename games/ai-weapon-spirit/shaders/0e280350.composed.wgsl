...
fn sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(normal_3: vec3<f32>, view_2: vec3<f32>, roughness_3: f32, F0_3: vec3<f32>, worldPosition_1: vec3<f32>, boxCenter_1: vec3<f32>, boxExtents_1: vec3<f32>, rotation_3: vec4<f32>, probeMap: texture_cube<f32>, probeSampler: sampler, brdfLut_2: texture_2d<f32>, brdfLutSampler_2: sampler, skylightMap: texture_cube<f32>, skylightRotation: vec4<f32>, skylightScale: vec3<f32>, probeIntensity: f32, boxProjection: bool) -> vec3<f32> {
    ...
    let local_22 = (_e55 * probeIntensity);
    let _e58 = sky;
    return mix(_e58, local_22, weight);
}
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_11: vec3<f32>, layer_4: u32, count_4: u32, normal_16: vec3<f32>, l_1: vec3<f32>) -> f32 {
    ...
    if (tileUv.x >= 0f) {
        local_14 = (tileUv.x <= 1f);
    } else {
        local_14 = false;
    }
    let _e40 = local_14;
    if _e40 {
        local_15 = (tileUv.y >= 0f);
    } else {
        local_15 = false;
    }
    let _e47 = local_15;
    if _e47 {
        local_16 = (tileUv.y <= 1f);
    } else {
        local_16 = false;
    }
    let _e54 = local_16;
    if _e54 {
        local_17 = (currentDepth <= 1f);
    } else {
        local_17 = false;
    }
    let _e60 = local_17;
    ...
    if (kernel == 3u) {
        if all((uv_5 >= (tileLo + texel_2))) {
            local_18 = all((uv_5 <= (tileHi - texel_2)));
        } else {
            local_18 = false;
        }
        let interior = local_18;
        if interior {
...
fn encodeStandardGBufferX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(normal_18: vec3<f32>, roughness_7: f32, albedo_3: vec3<f32>, metallic_12: f32, f0_6: vec3<f32>, occlusion: f32, emissive: vec3<f32>, opacity: f32, reflection: u32, probeRow_1: u32) -> GBufferOutputX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX {
    return GBufferOutputX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUZ3COVTGMZLSX(vec4<f32>(((normalize(normal_18) * 0.5f) + vec3(0.5f)), roughness_7), pack4x8unorm(vec4<f32>(albedo_3, metallic_12)), vec4<f32>(f0_6, occlusion), vec4<f32>(emissive, opacity), ((reflection << 24u) | probeRow_1));
}
...
fn projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX(baseColorAlpha_1: f32, alphaCutoff: f32, baseColorTextureEnabled: bool, baseColorTexture: texture_2d<f32>, baseColorSampler: sampler, transform_1: vec4<f32>, metadata_2: vec4<f32>, currentClip_1: vec4<f32>, previousClip_1: vec4<f32>, temporalProjection_3: vec4<f32>, reactive_3: f32, motionValid_2: bool, uv0_1: vec2<f32>, uv1_1: vec2<f32>, uv2_1: vec2<f32>, uv3_1: vec2<f32>, uv4_1: vec2<f32>, uv5_1: vec2<f32>, uv6_1: vec2<f32>, uv7_1: vec2<f32>) -> vec4<f32> {
    ...
    if (alphaCutoff > 0f) {
        ...
        local_20 = ((baseColorAlpha_1 * _e24) <= alphaCutoff);
    } else {
        local_20 = false;
    }
    let _e30 = local_20;
...
fn evaluate_standard_surface(input_2: SurfaceInput, materialValue: MaterialParameters) -> SurfaceData {
    ...
    normal_19 = normalize(input_2.geometricNormalWS);
...
fn alphaTestSurface(surface: SurfaceData) {
    ...
    if (surface.alphaClipThreshold > 0f) {
        local_21 = (surface.opacity <= surface.alphaClipThreshold);
    } else {
        local_21 = false;
    }
    let _e10 = local_21;
