...
fn _samplePcssForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_11: vec3<f32>, layer_5: u32, count_4: u32, normal_16: vec3<f32>, l_1: vec3<f32>, profile: u32) -> f32 {
    ...
    if (tileUv.x >= 0f) {
        local_20 = (tileUv.x <= 1f);
    } else {
        local_20 = false;
    }
    let _e46 = local_20;
    if _e46 {
        local_21 = (tileUv.y >= 0f);
    } else {
        local_21 = false;
    }
    let _e53 = local_21;
    if _e53 {
        local_22 = (tileUv.y <= 1f);
    } else {
        local_22 = false;
    }
    let _e60 = local_22;
    if _e60 {
        local_23 = (projCoords.z >= 0f);
    } else {
        local_23 = false;
    }
    let _e67 = local_23;
    if _e67 {
        local_24 = (projCoords.z <= 1f);
    } else {
        local_24 = false;
    }
    let _e74 = local_24;
    ...
    if (lightDepthWorldSpan > 0f) {
        local_28 = (worldUnitsPerTexel > 0f);
    } else {
        local_28 = false;
    }
    let _e213 = local_28;
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_12: vec3<f32>, layer_6: u32, count_5: u32, normal_17: vec3<f32>, l_2: vec3<f32>) -> f32 {
    ...
    if (tileUv_1.x >= 0f) {
        local_31 = (tileUv_1.x <= 1f);
    } else {
        local_31 = false;
    }
    let _e40 = local_31;
    if _e40 {
        local_32 = (tileUv_1.y >= 0f);
    } else {
        local_32 = false;
    }
    let _e47 = local_32;
    if _e47 {
        local_33 = (tileUv_1.y <= 1f);
    } else {
        local_33 = false;
    }
    let _e54 = local_33;
    ...
    if (kernel == 3u) {
        if all((uv_7 >= (tileLo + texel_5))) {
            local_35 = all((uv_7 <= (tileHi - texel_5)));
        } else {
            local_35 = false;
        }
...
fn fs_main(in_1: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let n_2 = normalize(_e5.normalWS);
    ...
    let _e176 = f0_;
    let _e177 = evaluateStandardDirectX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(in_1.worldPos, in_1.ndc.xyz, _e173, _e174, v_2, _e175, metallic_15, a_3, _e176, _e168);
    color = (_e169 + _e177);
    let _e179 = color;
