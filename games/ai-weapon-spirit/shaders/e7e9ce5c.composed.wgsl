...
fn cloud_direct_solar_factorX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX(worldPosition_2: vec3<f32>, shadowOrigin: vec3<f32>, shadowRight: vec3<f32>, shadowUp: vec3<f32>, shadowProjection: vec4<f32>) -> f32 {
    ...
    let _e80 = textureSampleLevel(cloudShadowMapX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX, cloudShadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F6Y3MN52WIOR2NRQXSZLSX, clamp(uv, vec2(0f), vec2(1f)), 0f);
    let sampled = _e80.x;
    if validReceiver {
        local_6 = inRange;
    } else {
...
fn evaluateStandardDirectX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(worldPosition_5: vec3<f32>, ndc: vec3<f32>, viewZ: f32, normal_8: vec3<f32>, direction_3: vec3<f32>, albedo_2: vec3<f32>, metallic_3: f32, alpha: f32, f0_2: vec3<f32>, shadow: f32) -> vec3<f32> {
    var direct: vec3<f32>;
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos: vec3<f32>, layer_4: u32, count_4: u32, normal_10: vec3<f32>, l_1: vec3<f32>) -> f32 {
    ...
    if (kernel == 3u) {
        if all((uv_2 >= (tileLo + texel))) {
            local_13 = all((uv_2 <= (tileHi - texel)));
        } else {
            local_13 = false;
        }
...
fn resolveStandardDeferred(in_2: FullscreenOutputX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX) -> DeferredOutput {
    ...
    let normal_12 = normalize(((normalSample.xyz * 2f) - vec3(1f)));
    let roughness_8 = normalSample.w;
    ...
    let direction_4 = normalize((_e73 - position));
    ...
    let _e131 = evaluateStandardEnvironmentX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(position, normal_12, direction_4, albedo_3.xyz, albedo_3.w, roughness_8, response.xyz, _e122, irradianceMap_2, linearSampler, prefilterMap_2, linearSampler, brdfLut_4, skylightPrefilterMap_1, _e123, _e124);
    let _e134 = params.x;
    if (_e134 > 0f) {
    ...
    let _e163 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_12, position, _e162);
    let _e168 = evaluateStandardDirectX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43UMFXGIYLSMRPWY2LHNB2GS3THX(position, ndc_1, _e162, normal_12, direction_4, albedo_3.xyz, albedo_3.w, (roughness_8 * roughness_8), response.xyz, _e163);
