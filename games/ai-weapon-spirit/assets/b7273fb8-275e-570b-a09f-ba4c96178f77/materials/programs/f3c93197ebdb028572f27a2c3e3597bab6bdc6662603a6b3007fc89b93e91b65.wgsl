struct MaterialParametersX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX {
    ...
    floraPadA: f32,
    floraPadB: f32,
    _gpuDrivenPadding: array<vec4<f32>, 24>,
}
...
struct InputX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DUZTMN5ZGCX3DN5WW233OX {
...
struct OutputX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DUZTMN5ZGCX3DN5WW233OX {
    @builtin(position) clip: vec4<f32>,
    @location(0) world: vec3<f32>,
    @location(1) color: vec4<f32>,
    @location(2) height: f32,
    @location(3) viewZ: f32,
    @location(4) @interpolate(flat) shade: vec4<f32>,
    @location(5) @interpolate(flat) state: vec4<f32>,
    @location(6) @interpolate(flat) shadow: vec3<f32>,
    @location(7) @interpolate(flat) rim: vec3<f32>,
}
...
@group(1) @binding(46) 
var<storage> sceneMaterialsX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX: array<MaterialParametersX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX>;
@group(3) @binding(2) 
var<storage> visibleItemsX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX: array<vec4<u32>>;
...
fn plantPositionX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DUZTMN5ZGCX3DN5WW233OX(v_3: InputX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DUZTMN5ZGCX3DN5WW233OX, model: mat4x4<f32>, material: MaterialParametersX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX) -> vec4<f32> {
    var p: vec3<f32>;

    let encodedRole = v_3.pigmentBRole.y;
    let role = select(encodedRole, (-(encodedRole) - 1f), (encodedRole < 0f));
    let pivot = vec3<f32>(v_3.pivotXY, v_3.pivotZFlex.x);
    let root = model[3].xyz;
    let phase = ((root.x * 0.37f) + (root.z * 0.29f));
    let t_1 = material.floraTimeSeconds;
    let wind = (material.floraWindStrength * (1f - material.floraFreeze));
    let flutter = (((sin((((t_1 * 2.4f) + phase) + (pivot.y * 1.7f))) * 0.065f) * wind) * v_3.pivotZFlex.y);
    let leafScale = select(1f, (mix(0.12f, 1f, smoothstep(0.12f, 0.9f, material.floraGrowth)) * (1f - (material.floraWither * 0.18f))), (role > 0.5f));
    let delta = ((v_3.position - pivot) * leafScale);
    p = (pivot + vec3<f32>(delta.x, ((delta.y * cos(flutter)) - (delta.z * sin(flutter))), ((delta.y * sin(flutter)) + (delta.z * cos(flutter)))));
    let _e83 = p.y;
    let h = (max(0f, _e83) / max(v_3.wind.y, 0.01f));
    let bend = (((h * h) * v_3.wind.x) * v_3.wind.y);
    let wave = ((sin(((t_1 * 0.85f) + phase)) * 0.045f) + (sin(((t_1 * 1.47f) + (phase * 0.61f))) * 0.017f));
    let _e113 = p;
    p = (_e113 + ((vec3<f32>(0.84f, 0f, 0.54f) * bend) * ((wave * wind) + (material.floraImpact * 0.12f))));
    let growth = clamp(material.floraGrowth, 0.001f, 1f);
    let _e131 = p.x;
    let _e134 = p.y;
    let _e137 = p.z;
    p = vec3<f32>((_e131 * growth), (_e134 * growth), (_e137 * growth));
    if (material.floraStump > 0.5f) {
        let _e145 = p.y;
        p.y = min(_e145, max(0.08f, (v_3.wind.y * 0.12f)));
    }
    let _e153 = p;
    return (model * vec4<f32>(_e153, 1f));
}

fn plantVertexX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DUZTMN5ZGCX3DN5WW233OX(v_4: InputX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DUZTMN5ZGCX3DN5WW233OX, model_1: mat4x4<f32>, material_1: MaterialParametersX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX, clipMatrix: mat4x4<f32>) -> OutputX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DUZTMN5ZGCX3DN5WW233OX {
    var o: OutputX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DUZTMN5ZGCX3DN5WW233OX;

    let _e3 = plantPositionX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DUZTMN5ZGCX3DN5WW233OX(v_4, model_1, material_1);
    o.clip = (clipMatrix * _e3);
    o.world = _e3.xyz;
    o.color = vec4<f32>(v_4.pigmentRG, v_4.pigmentBRole);
    o.height = (v_4.position.y / max(v_4.wind.y, 0.01f));
    let _e25 = o.clip.w;
    o.viewZ = -(_e25);
    o.shade = vec4<f32>(material_1.sideShade, material_1.specularStrength, material_1.rimStrength, 0f);
    o.state = vec4<f32>(material_1.floraBurn, material_1.floraWither, material_1.floraFreeze, material_1.floraStump);
    o.shadow = material_1.shadowColor.xyz;
    o.rim = material_1.rimColor.xyz;
    let _e45 = o;
    return _e45;
}

fn pruneSurfaceX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DUZTMN5ZGCX3DN5WW233OX(v_5: OutputX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DUZTMN5ZGCX3DN5WW233OX) {
    var local_7: bool;
    var local_8: bool;

    if (v_5.state.w > 0.5f) {
        local_7 = (v_5.color.w >= 0f);
    } else {
        local_7 = false;
    }
    let _e12 = local_7;
    ...
    if (v_5.color.w > 0.5f) {
        local_8 = (v_5.state.x > (0.68f + (v_5.height * 0.3f)));
    } else {
        local_8 = false;
    }
    let _e28 = local_8;
...
fn plantColorX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DUZTMN5ZGCX3DN5WW233OX(v_6: OutputX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DUZTMN5ZGCX3DN5WW233OX, front_1: bool) -> vec4<f32> {
    var n: vec3<f32>;
    var local_9: bool;
    var local_10: bool;
    var color: vec3<f32>;

    pruneSurfaceX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DUZTMN5ZGCX3DN5WW233OX(v_6);
    let _e2 = dpdy(v_6.world);
    let _e4 = dpdx(v_6.world);
    n = normalize(cross(_e2, _e4));
    if !(front_1) {
        let _e10 = n;
        n = -(_e10);
    }
    let role_1 = select(v_6.color.w, (-(v_6.color.w) - 1f), (v_6.color.w < 0f));
    if (role_1 > 0.5f) {
        local_9 = (role_1 < 1.5f);
    } else {
        local_9 = false;
    ...
    let leaf = local_9;
    let burn = clamp((v_6.state.x * 1.6f), 0f, 1f);
    let dry = mix(v_6.color.xyz, vec3<f32>(0.39f, 0.28f, 0.085f), v_6.state.y);
    let pigment = mix(dry, vec3<f32>(0.018f, 0.034f, 0.027f), burn);
    let base_2 = mix(pigment, vec3<f32>(0.29f, 0.48f, 0.49f), (v_6.state.z * 0.7f));
    let _e62 = n;
    let _e65 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e62, v_6.world, v_6.viewZ);
    let roughness_4 = select(0.9f, 0.72f, leaf);
    let _e71 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.lightDir;
    let light_1 = normalize(-(_e71));
    let _e76 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cameraPos;
    let camera_2 = normalize((_e76 - v_6.world));
    if (role_1 > 1.5f) {
        local_10 = (role_1 < 2.5f);
    } else {
        local_10 = false;
    }
    let _e88 = local_10;
    let shadowPigment_1 = mix(v_6.shadow, base_2, select(0f, 0.48f, _e88));
    let _e93 = n;
    let _e97 = illustratedDiffuseX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DU2LMNR2XG5DSMF2GKZC7NRUWO2DUNFXGOX(base_2, shadowPigment_1, _e93, _e65, v_6.shade.x, false);
    color = _e97;
    let _e99 = color;
    let _e100 = n;
    let _e104 = paintedSpecularX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DU4DBNFXHIZLSNR4V643VOJTGCY3FX(base_2, _e100, light_1, camera_2, roughness_4, 0f, v_6.shade.y, _e65);
    color = (_e99 + _e104);
    let _e106 = color;
    let _e108 = n;
    let _e111 = paintedRimX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DU4DBNFXHIZLSNR4V643VOJTGCY3FX(_e108, camera_2, roughness_4, v_6.shade.z, _e65);
    color = (_e106 + (v_6.rim * _e111));
    let _e114 = color;
    return vec4<f32>(_e114, 1f);
}

@vertex 
fn vs_main(v: InputX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DUZTMN5ZGCX3DN5WW233OX, @builtin(instance_index) idx: u32) -> OutputX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DUZTMN5ZGCX3DN5WW233OX {
    ...
    let _e11 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX;
    let _e14 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.worldViewProj;
    let _e16 = plantVertexX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DUZTMN5ZGCX3DN5WW233OX(v, (_e3 * _e8), _e11, _e14);
    return _e16;
}

@vertex 
fn vs_scene_index(v_1: InputX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DUZTMN5ZGCX3DN5WW233OX, @builtin(instance_index) idx_1: u32) -> OutputX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DUZTMN5ZGCX3DN5WW233OX {
    let visible = visibleItemsX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX[idx_1];
    let _e8 = meshesX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX[visible.x].worldFromLocal;
    let _e12 = instancesX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX[0].localFromInstance;
    let _e17 = sceneMaterialsX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX[visible.y];
    let _e20 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.worldViewProj;
    let _e22 = plantVertexX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DUZTMN5ZGCX3DN5WW233OX(v_1, (_e8 * _e12), _e17, _e20);
    return _e22;
}

@fragment 
fn fs_main(v_2: OutputX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DUZTMN5ZGCX3DN5WW233OX, @builtin(front_facing) front: bool) -> @location(0) vec4<f32> {
    let _e2 = plantColorX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DUZTMN5ZGCX3DN5WW233OX(v_2, front);
    return _e2;
}
