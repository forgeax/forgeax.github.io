...
struct Material {
    ...
    pigmentStrength: f32,
    pigmentScale: f32,
    surfaceAccentA: vec4<f32>,
    surfaceAccentB: vec4<f32>,
    surfaceDetailMode: f32,
    surfaceDetailStrength: f32,
    surfacePatchSize: f32,
}
...
struct VsOut {
    ...
    @location(3) @interpolate(flat) sediment: u32,
}
...
fn _directionalReceiverDepthBiasX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(layer_3: u32, normal_2: vec3<f32>, l: vec3<f32>, radius: f32) -> f32 {
    var local_3: bool;

    let nDotL_2 = dot(normal_2, l);
    ...
    let slope = ((abs(dot(normal_2, right)) + abs(dot(normal_2, up))) / nDotL_2);
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos: vec3<f32>, layer_4: u32, count_4: u32, normal_3: vec3<f32>, l_1: vec3<f32>) -> f32 {
    var local_4: bool;
    var local_5: bool;
    var local_6: bool;
    var local_7: bool;
    ...
    let _e102 = _directionalReceiverDepthBiasX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(layer_4, normal_3, l_1, f32(((kernel / 2u) + 1u)));
...
fn evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_4: vec3<f32>, worldPos_1: vec3<f32>, viewZ: f32) -> f32 {
    ...
    let receiverPosition = (worldPos_1 + (normal_4 * _e33));
    let _e36 = _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(receiverPosition, _e28, count_5, normal_4, l_2);
    shadow = _e36;
    ...
    if _e49 {
        ...
        if (blendWidth > 0f) {
            ...
            if (t > 0f) {
                let _e72 = _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(receiverPosition, (_e28 + 1u), count_5, normal_4, l_2);
...
fn illustratedDiffuseX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DU2LMNR2XG5DSMF2GKZC7NRUWO2DUNFXGOX(surface: vec3<f32>, shadowPigment: vec3<f32>, normal_5: vec3<f32>, visibility: f32, sideShade: f32, continuous: bool) -> vec3<f32> {
    var diffuse: f32;

    let _e3 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.lightDir;
    let nDotL_3 = dot(normal_5, normalize(-(_e3)));
    let up_1 = clamp(normal_5.y, 0f, 1f);
...
fn paintedSpecularX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DU4DBNFXHIZLSNR4V643VOJTGCY3FX(base: vec3<f32>, normal_6: vec3<f32>, light: vec3<f32>, camera: vec3<f32>, roughness_2: f32, metallic: f32, strength: f32, visibility_1: f32) -> vec3<f32> {
    let halfVector = normalize((light + camera));
    let smoothness = (1f - clamp(roughness_2, 0.04f, 1f));
    let lobe = pow(max(dot(normal_6, halfVector), 0f), mix(12f, 112f, smoothness));
    let reflection = (((((strength * smoothness) * smoothness) * lobe) * max(dot(normal_6, light), 0f)) * visibility_1);
    return (mix(vec3<f32>(0.88f, 0.86f, 0.81f), base, metallic) * reflection);
}

fn paintedRimX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DU4DBNFXHIZLSNR4V643VOJTGCY3FX(normal_7: vec3<f32>, camera_1: vec3<f32>, roughness_3: f32, strength_1: f32, visibility_2: f32) -> f32 {
    let fresnel_1 = pow((1f - clamp(dot(normal_7, camera_1), 0f, 1f)), 4f);
    return (((fresnel_1 * strength_1) * mix(1f, 0.18f, roughness_3)) * mix(0.35f, 1f, visibility_2));
}

fn surfaceHash21_(cell: vec2<f32>) -> f32 {
    var p: vec3<f32>;

    p = fract((vec3<f32>(cell.x, cell.y, cell.x) * 0.1031f));
    let _e9 = p;
    let _e10 = p;
    let _e11 = p;
...
fn surfaceValueNoise(point: vec2<f32>) -> f32 {
    let cell_1 = floor(point);
    let local_10 = fract(point);
    let blend = (((local_10 * local_10) * local_10) * ((local_10 * ((local_10 * 6f) - vec2(15f))) + vec2(10f)));
    let _e15 = surfaceHash21_(cell_1);
    let _e20 = surfaceHash21_((cell_1 + vec2<f32>(1f, 0f)));
    let _e27 = surfaceHash21_((cell_1 + vec2<f32>(0f, 1f)));
    let _e32 = surfaceHash21_((cell_1 + vec2<f32>(1f, 1f)));
    return mix(mix(_e15, _e20, blend.x), mix(_e27, _e32, blend.x), blend.y);
}

fn surfaceFilteredNoise(point_1: vec2<f32>) -> f32 {
    let _e1 = dpdx(point_1);
    let _e3 = dpdy(point_1);
    let footprint_1 = max(length(_e1), length(_e3));
    let resolved = (1f - smoothstep(0.18f, 0.65f, footprint_1));
    let _e11 = surfaceValueNoise(point_1);
    return (0.5f + ((_e11 - 0.5f) * resolved));
}

fn applyTerrainSurfaceDetail(worldPos_2: vec3<f32>, baseColor: vec3<f32>) -> vec3<f32> {
    let p_1 = (vec2<f32>(((worldPos_2.x * 0.8f) + (worldPos_2.z * 0.6f)), ((worldPos_2.z * 0.8f) - (worldPos_2.x * 0.6f))) + (worldPos_2.y * vec2<f32>(0.31f, 0.73f)));
    let _e24 = material.surfacePatchSize;
    let _e31 = surfaceFilteredNoise((p_1 / vec2(max(3f, (_e24 * 3f)))));
    let _e34 = material.surfaceAccentA;
    let _e38 = material.surfaceAccentB;
    let pigment = mix(_e34.xyz, _e38.xyz, _e31);
    let _e43 = material.surfaceDetailStrength;
    return mix(baseColor, pigment, (_e43 * 0.5f));
}
...
fn vs_main(input: VsIn, @builtin(instance_index) idx: u32) -> VsOut {
    ...
    let _e33 = output.clip.w;
    output.viewZ = -(_e33);
    output.sediment = select(0u, 1u, (input.uv.y > 1.5f));
    let _e43 = output;
    return _e43;
}
...
fn fs_main(input_1: VsOut, @builtin(front_facing) frontFacing: bool) -> @location(0) vec4<f32> {
    ...
    let isSediment = (input_1.sediment == 1u);
    if isSediment {
        normal = (normalize(input_1.worldNormal) * select(-1f, 1f, frontFacing));
    }
    let _e24 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.lightDir;
    let toLight = normalize(-(_e24));
    let _e29 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cameraPos;
    let toCamera = normalize((_e29 - input_1.worldPos));
    let _e33 = normal;
    let lightingNDotL = dot(_e33, toLight);
    let _e35 = normal;
    let lightingUpFacing = dot(_e35, vec3<f32>(0f, 1f, 0f));
    let _e44 = material.baseColor;
    let _e46 = applyTerrainSurfaceDetail(input_1.worldPos, _e44.xyz);
    let _e49 = material.surfaceDetailMode;
    if (_e49 > 0.5f) {
        let _e54 = material.surfaceDetailMode;
        local = (_e54 < 1.5f);
    } else {
        local = false;
    }
    let _e60 = local;
    if _e60 {
        local_1 = (lightingUpFacing > 0.5f);
    } else {
        local_1 = false;
    }
    let isGrassCanopy = local_1;
    if isGrassCanopy {
        let _e67 = normal;
        normal = normalize(mix(_e67, vec3<f32>(0f, 1f, 0f), 0.72f));
    }
    let lightingNormal = normal;
    let _e78 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(geometricNormal, input_1.worldPos, input_1.viewZ);
    let _e81 = material.shadowColor;
    let _e83 = normal;
    let _e86 = material.sideShade;
    if !(isGrassCanopy) {
        local_2 = isSediment;
    } else {
        local_2 = true;
    ...
    let _e91 = local_2;
    let _e92 = illustratedDiffuseX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DU2LMNR2XG5DSMF2GKZC7NRUWO2DUNFXGOX(_e46, _e81.xyz, _e83, _e78, _e86, _e91);
    color = _e92;
    let _e94 = color;
    let _e97 = material.surfaceRoughness;
    let _e100 = material.surfaceMetallic;
    let _e103 = material.specularStrength;
    let _e104 = paintedSpecularX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DU4DBNFXHIZLSNR4V643VOJTGCY3FX(_e46, lightingNormal, toLight, toCamera, _e97, _e100, _e103, _e78);
    color = (_e94 + _e104);
    let _e108 = material.surfaceRoughness;
    let _e111 = material.rimStrength;
    let _e112 = paintedRimX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DU4DBNFXHIZLSNR4V643VOJTGCY3FX(lightingNormal, toCamera, _e108, _e111, _e78);
    let _e113 = color;
    let _e116 = material.rimColor;
    color = (_e113 + (_e116.xyz * _e112));
    let _e120 = color;
    let _e123 = material.rimColor;
    let _e127 = material.emissionStrength;
    color = (_e120 + (_e123.xyz * _e127));
    let _e130 = color;
    let _e134 = material.baseColor.w;
    return vec4<f32>(_e130, _e134);
}
...
fn fs_readability_candidate(input_2: VsOut, @builtin(front_facing) frontFacing_1: bool) -> @location(0) vec4<f32> {
    ...
    let _e24 = normal_1;
    let nDotL_4 = dot(_e24, toLight_1);
    let _e26 = normal_1;
    let upFacing = dot(_e26, vec3<f32>(0f, 1f, 0f));
    let _e35 = material.baseColor;
    let _e37 = applyTerrainSurfaceDetail(input_2.worldPos, _e35.xyz);
    if (nDotL_4 > -0.22f) {
        lightBand = 0.43f;
    }
    if (nDotL_4 > 0.08f) {
    ...
    let _e50 = material.shadowColor;
    let _e52 = lightBand;
    color_1 = mix(_e50.xyz, _e37, _e52);
    let _e55 = normal_1;
    let _e58 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e55, input_2.worldPos, input_2.viewZ);
    if (nDotL_4 > 0.08f) {
        let retainedLight = mix(0.48f, 1f, _e58);
        let _e66 = material.shadowColor;
        let _e68 = color_1;
        color_1 = mix(_e66.xyz, _e68, retainedLight);
    }
    let _e72 = material.sideShade;
    planeShade = _e72;
    if (upFacing > 0.72f) {
        planeShade = 1f;
    }
    if (upFacing < -0.72f) {
        let _e81 = material.sideShade;
        planeShade = (_e81 * 0.86f);
    }
    let _e84 = color_1;
    let _e85 = planeShade;
    color_1 = (_e84 * _e85);
    let _e87 = color_1;
    let _e88 = normal_1;
    let _e91 = material.surfaceRoughness;
    let _e94 = material.surfaceMetallic;
    let _e97 = material.specularStrength;
    let _e98 = paintedSpecularX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DU4DBNFXHIZLSNR4V643VOJTGCY3FX(_e37, _e88, toLight_1, toCamera_1, _e91, _e94, _e97, _e58);
    color_1 = (_e87 + _e98);
    let _e100 = normal_1;
    let _e103 = material.surfaceRoughness;
    let _e106 = material.rimStrength;
    let _e107 = paintedRimX_naga_oil_mod_XMFUV653FMFYG63S7ONYGS4TJOQ5DU4DBNFXHIZLSNR4V643VOJTGCY3FX(_e100, toCamera_1, _e103, _e106, _e58);
    let _e108 = color_1;
    let _e111 = material.rimColor;
    color_1 = (_e108 + (_e111.xyz * _e107));
    let _e115 = color_1;
    let _e118 = material.rimColor;
    let _e122 = material.emissionStrength;
    color_1 = (_e115 + (_e118.xyz * _e122));
    let _e125 = color_1;
    let _e129 = material.baseColor.w;
    return vec4<f32>(_e125, _e129);
}
