...
fn shapeCorner(vertex_1: u32) -> vec2<f32> {
    ...
    let _e26 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.shapeA.x;
    if (_e26 == 0f) {
        return (corner - vec2(0.5f));
    }
...
fn gradient_3_(t_3: f32) -> vec4<f32> {
    ...
    if (t_3 <= 0.09117266f) {
        ...
        if (t_3 <= 0.28824294f) {
            rgb_2 = mix(vec3<f32>(1f, 0f, 0.01978827f), vec3<f32>(0.5566038f, 0.03393925f, 0f), ((t_3 - 0.09117266f) / 0.19707027f));
        } else {
            if (t_3 <= 0.66764325f) {
                rgb_2 = mix(vec3<f32>(0.5566038f, 0.03393925f, 0f), vec3<f32>(0.3962264f, 0.01219158f, 0f), ((t_3 - 0.28824294f) / 0.3794003f));
            }
        }
    }
    if (t_3 <= 0f) {
...
fn gradient_4_(t_4: f32) -> vec4<f32> {
    ...
    if (t_4 <= 0.1764706f) {
        ...
        if (t_4 <= 0.30882734f) {
            rgb_3 = mix(vec3<f32>(1f, 1f, 1f), vec3<f32>(1f, 1f, 1f), ((t_4 - 0.1764706f) / 0.13235676f));
        } else {
            if (t_4 <= 0.4617685f) {
                rgb_3 = mix(vec3<f32>(1f, 1f, 1f), vec3<f32>(1f, 1f, 1f), ((t_4 - 0.30882734f) / 0.15294118f));
            } else {
                if (t_4 <= 0.8352941f) {
                    rgb_3 = mix(vec3<f32>(1f, 1f, 1f), vec3<f32>(1f, 1f, 1f), ((t_4 - 0.4617685f) / 0.3735256f));
                }
            }
        }
    }
...
fn gradient_5_(t_5: f32) -> vec4<f32> {
    ...
    if (t_5 <= 0.1764706f) {
        ...
        if (t_5 <= 0.30882734f) {
            rgb_4 = mix(vec3<f32>(1f, 1f, 1f), vec3<f32>(1f, 1f, 1f), ((t_5 - 0.1764706f) / 0.13235676f));
        } else {
            if (t_5 <= 0.4617685f) {
                rgb_4 = mix(vec3<f32>(1f, 1f, 1f), vec3<f32>(1f, 1f, 1f), ((t_5 - 0.30882734f) / 0.15294118f));
            } else {
                if (t_5 <= 0.8352941f) {
                    rgb_4 = mix(vec3<f32>(1f, 1f, 1f), vec3<f32>(1f, 1f, 1f), ((t_5 - 0.4617685f) / 0.3735256f));
                }
            }
        }
    }
...
fn vs_main(native: NativeInput, @builtin(vertex_index) vertex: u32) -> Varying {
    ...
    let _e186 = dm_streams((((uvCorner + vec2(0.5f)) + cell_1) / _e177.xy), _e180, bitcast<u32>(_e184));
...
fn fs_main(i: Varying, @builtin(front_facing) front: bool) -> @location(0) vec4<f32> {
    ...
    let _e93 = rgb;
    let _e97 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.shapeD.x;
    rgb = (_e93 * _e97);
    let _e99 = alpha;
    let _e103 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.shapeD.x;
    alpha = (_e99 * min(1f, _e103));
    let _e108 = rgb.x;
    let _e110 = rgb.y;
    let _e113 = rgb.z;
