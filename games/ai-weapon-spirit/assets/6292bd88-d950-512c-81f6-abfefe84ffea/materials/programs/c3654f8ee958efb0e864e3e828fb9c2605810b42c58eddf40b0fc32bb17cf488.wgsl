...
fn curve_2_(t_2: f32) -> f32 {
    ...
    if (t_2 < 1f) {
        let u_5 = ((t_2 - 0.0480008f) / 0.9519992f);
...
fn dm_appearance(seed_1: u32, id_1: u32, t_3: f32, particle: ptr<function, Particle>) {
    ...
    (*particle).size_rotation.w = max(0f, ((mix(0.1f, 0.3f, _e37) * 1f) * (_e43 * 1.2f)));
...
fn fs_main(i: Varying, @builtin(front_facing) front: bool) -> @location(0) vec4<f32> {
    ...
    if (_e93 > 0f) {
        ...
        rgb = _e106;
        let _e107 = rgb;
        let _e111 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.detailB.x;
        let _e115 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.detailA.y;
        rgb = (_e107 * mix(1f, _e111, _e115));
    }
    let _e120 = rgb.x;
    let _e122 = rgb.y;
    let _e125 = rgb.z;
    let peak_1 = max(max(_e120, _e122), _e125);
    let eclipse = (mix(vec3<f32>(0.32f, 0.045f, 0.82f), vec3<f32>(0.88f, 0.78f, 1f), smoothstep(0.6f, 4f, peak_1)) * peak_1);
    let _e143 = frame.design.z;
    if (_e143 > 0.5f) {
        let _e146 = rgb;
        let _e150 = frame.design.y;
        let _e151 = enchantmentParticleColor(_e146, _e150);
        rgb = _e151;
    } else {
        let _e152 = rgb;
        let _e156 = frame.design.x;
        rgb = mix(_e152, eclipse, _e156);
    }
    let _e158 = alpha;
    let _e162 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.tint.w;
    alpha = (_e158 * _e162);
    let _e164 = rgb;
    let _e167 = materialX_naga_oil_mod_XMZXXEZ3FMF4F63LBORSXE2LBNQ5DU4DBOJQW2ZLUMVZHGX.tint;
    let _e170 = alpha;
    let _e172 = alpha;
    return vec4<f32>(((_e164 * _e167.xyz) * _e170), _e172);
}
