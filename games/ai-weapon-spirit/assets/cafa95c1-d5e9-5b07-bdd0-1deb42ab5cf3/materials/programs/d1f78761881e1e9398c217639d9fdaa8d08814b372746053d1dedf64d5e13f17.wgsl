...
fn curve_0_(t: f32) -> f32 {
    ...
    if (t < 0.1561905f) {
        let u = ((t - 0f) / 0.1561905f);
        return (((((((((2f * u) * u) * u) - ((3f * u) * u)) + 1f) * 0f) + (((((u * u) * u) - ((2f * u) * u)) + u) * 0f)) + (((((-2f * u) * u) * u) + ((3f * u) * u)) * 1.01194f)) + ((((u * u) * u) - (u * u)) * 0f));
    }
    return 1.01194f;
}
...
fn curve_1_(t_1: f32) -> f32 {
    ...
    if (t_1 < 0.1561905f) {
        let u_1 = ((t_1 - 0f) / 0.1561905f);
        return (((((((((2f * u_1) * u_1) * u_1) - ((3f * u_1) * u_1)) + 1f) * 0f) + (((((u_1 * u_1) * u_1) - ((2f * u_1) * u_1)) + u_1) * 0f)) + (((((-2f * u_1) * u_1) * u_1) + ((3f * u_1) * u_1)) * 1.01194f)) + ((((u_1 * u_1) * u_1) - (u_1 * u_1)) * 0f));
    }
    return 1.01194f;
}
...
fn curve_2_(t_2: f32) -> f32 {
    ...
    if (t_2 < 0.1561905f) {
        let u_2 = ((t_2 - 0f) / 0.1561905f);
        return (((((((((2f * u_2) * u_2) * u_2) - ((3f * u_2) * u_2)) + 1f) * 0f) + (((((u_2 * u_2) * u_2) - ((2f * u_2) * u_2)) + u_2) * 0f)) + (((((-2f * u_2) * u_2) * u_2) + ((3f * u_2) * u_2)) * 1.01194f)) + ((((u_2 * u_2) * u_2) - (u_2 * u_2)) * 0f));
    }
    return 1.01194f;
}
...
fn gradient_3_(t_3: f32) -> vec4<f32> {
    var rgb_2: vec3<f32> = vec3<f32>(0.05882353f, 0.003921569f, 0f);
    var alpha_1: f32 = 1f;

    if (t_3 <= 0f) {
        rgb_2 = vec3<f32>(1f, 0.9257448f, 0.5235849f);
    } else {
        if (t_3 <= 0.008819715f) {
            rgb_2 = mix(vec3<f32>(1f, 0.9257448f, 0.5235849f), vec3<f32>(0.9386859f, 0.5520116f, 0.042311423f), ((t_3 - 0f) / 0.008819715f));
        } else {
            if (t_3 <= 0.07353323f) {
                rgb_2 = mix(vec3<f32>(0.9386859f, 0.5520116f, 0.042311423f), vec3<f32>(0.8879232f, 0.15292618f, 0.012983031f), ((t_3 - 0.008819715f) / 0.064713515f));
            } else {
                if (t_3 <= 0.21176471f) {
                    rgb_2 = mix(vec3<f32>(0.8879232f, 0.15292618f, 0.012983031f), vec3<f32>(0.822786f, 0.038204376f, 0.005181517f), ((t_3 - 0.07353323f) / 0.13823149f));
                } else {
                    if (t_3 <= 0.55883116f) {
                        rgb_2 = mix(vec3<f32>(0.822786f, 0.038204376f, 0.005181517f), vec3<f32>(0.042311423f, 0.020288562f, 0.016807375f), ((t_3 - 0.21176471f) / 0.34706646f));
                    } else {
                        if (t_3 <= 1f) {
                            rgb_2 = mix(vec3<f32>(0.042311423f, 0.020288562f, 0.016807375f), vec3<f32>(0.05882353f, 0.003921569f, 0f), ((t_3 - 0.55883116f) / 0.44116884f));
                        }
                    }
                }
            }
    ...
    let _e101 = rgb_2;
    let _e102 = alpha_1;
    return vec4<f32>(_e101, _e102);
}

fn gradient_4_(t_4: f32) -> vec4<f32> {
    var rgb_3: vec3<f32> = vec3<f32>(0.745283f, 0.745283f, 0.745283f);
    var alpha_2: f32 = 0f;
    ...
    if (t_4 <= 0f) {
        ...
        if (t_4 <= 0.8f) {
            rgb_3 = mix(vec3<f32>(1f, 1f, 1f), vec3<f32>(0.745283f, 0.745283f, 0.745283f), ((t_4 - 0f) / 0.8f));
        }
    }
    if (t_4 <= 0.29117265f) {
        alpha_2 = 1f;
        ...
        if (t_4 <= 0.88235295f) {
            alpha_2 = mix(1f, 0f, ((t_4 - 0.29117265f) / 0.59118026f));
        }
    }
    let _e39 = rgb_3;
    let _e40 = alpha_2;
...
fn dm_appearance(seed_1: u32, id_1: u32, t_5: f32, particle: ptr<function, Particle>) {
    ...
    let _e45 = gradient_3_(t_5);
    let _e46 = gradient_4_(t_5);
    let _e48 = dm_random(seed_1, id_1, 7u);
    (*particle).color = (mix(vec4<f32>(1f, 0f, 0f, 0.6666667f), vec4<f32>(1f, 0f, 0f, 0.6392157f), _e43) * mix(_e45, _e46, _e48));
...
fn dm_streams(uv: vec2<f32>, p_1: Particle, seed_3: u32) -> DmStreams {
    ...
    let custom1_ = vec4<f32>(mix(1f, 0f, _e10), 0f, 0f, 0f);
