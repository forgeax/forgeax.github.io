...
fn curve_4_(t_4: f32) -> f32 {
    ...
    if (t_4 < 1f) {
        let u_7 = ((t_4 - 0.127191f) / 0.872809f);
...
fn curve_5_(t_5: f32) -> f32 {
    ...
    if (t_5 < 1f) {
        let u_8 = ((t_5 - 0.06939553f) / 0.93060446f);
...
fn gradient_6_(t_6: f32) -> vec4<f32> {
    ...
    if (t_6 <= 0f) {
        ...
        if (t_6 <= 0.49542993f) {
            alpha_1 = mix(1f, 0f, ((t_6 - 0f) / 0.49542993f));
        }
    }
    let _e39 = rgb_2;
    let _e40 = alpha_1;
...
fn dm_appearance(seed_1: u32, id_1: u32, t_7: f32, particle: ptr<function, Particle>) {
    ...
    let _e51 = curve_4_(t_7);
    let _e52 = curve_5_(t_7);
    let _e54 = dm_random(seed_1, id_1, 6u);
    (*particle).size_rotation.w = max(0f, ((mix(2f, 1f, _e45) * 1f) * (mix(_e51, _e52, _e54) * 0.9f)));
    let _e67 = gradient_6_(t_7);
    (*particle).color = (vec4<f32>(0.2f, 0f, 0.0102033615f, 1f) * _e67);
    let _e72 = dm_random(seed_1, id_1, 61u);
    (*particle).velocity.w = floor((fract((mix(0f, 0.9999f, _e72) + 0f)) * 4f));
    return;
}
...
fn dm_streams(uv: vec2<f32>, p_1: Particle, seed_3: u32) -> DmStreams {
    ...
    o_1.uv1_.z = t_10;
    let _e47 = stream_dm_random(seed_3, id_3, 345u);
    o_1.uv1_.w = _e47;
    let _e48 = o_1;
    return _e48;
}
