...
fn fs_main(in_1: VsOut) -> @location(0) vec4<f32> {
    ...
    let _e73 = material.clearcoatRoughness;
    let coatRoughness = max(_e73, 0.04f);
    let coatAlpha = (coatRoughness * coatRoughness);
    let _e82 = f_schlickX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(max(dot(_e61, v), 0f), vec3(0.04f));
    let _e85 = material.clearcoat;
    let coatF = (_e82 * _e85);
    let _e92 = f_schlickX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(max(dot(_e61, v), 0f), f0_1);
    let kD_2 = ((vec3(1f) - _e92) * (1f - metallic_5));
    let _e99 = material.roughness;
    let iblRoughness = (max(_e99, 0.04f) * _e39);
    let _e105 = skylight.rotation;
    let _e108 = sampleIblDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e61, _e105, irradianceMap_1, irradianceSampler_1);
    let _e111 = skylight.rotation;
    let _e116 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e61, v, iblRoughness, f0_1, _e111, prefilterMap_1, prefilterSampler_1, brdfLut_1, brdfLutSampler_1);
    let _e121 = skylight.rotation;
    let _e126 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e61, v, coatRoughness, vec3(0.04f), _e121, prefilterMap_1, prefilterSampler_1, brdfLut_1, brdfLutSampler_1);
    let _e129 = material.occlusionUvScale;
    let _e132 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(occlusionTexture, occlusionSampler, _e1, _e129);
    let _e136 = material.occlusionStrength;
    let ao = mix(1f, _e132.x, _e136);
    let _e141 = skylight.colorR;
    let _e144 = skylight.colorG;
    let _e147 = skylight.colorB;
    let skyColor = vec3<f32>(_e141, _e144, _e147);
    let _e158 = material.clearcoat;
    let _e164 = skylight.intensity;
    ambient = ((((((((kD_2 * _e108) * albedo) + _e116) * (vec3(1f) - coatF)) + (_e126 * _e158)) * skyColor) * _e164) * ao);
    let ssaoUv = ((in_1.ndc.xy * vec2<f32>(0.5f, -0.5f)) + vec2<f32>(0.5f, 0.5f));
    let _e180 = textureSample(ssaoBlurredTexture, ssaoBlurredSampler, ssaoUv);
    let ssaoFactor = _e180.x;
    let _e182 = get_ssao_intensityX_naga_oil_mod_XMZXXEZ3FMF4F62DEOJYDUOTDNR2XG5DFOJPWM33SO5QXEZAX();
    let _e183 = ambient;
    ambient = (_e183 * mix(1f, (ssaoFactor * ao), _e182));
    let _e188 = ambient;
    color = _e188;
    let _e190 = color;
    let _e191 = a;
    let _e194 = evalDirectionalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e61, v, albedo, metallic_5, _e191, f0_1, in_1.worldPos, in_1.viewZ);
    color = (_e190 + _e194);
    let _e196 = color;
    let _e199 = material.clearcoat;
    let _e207 = evalDirectionalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e61, v, vec3(0f), 1f, coatAlpha, vec3(0.04f), in_1.worldPos, in_1.viewZ);
    color = (_e196 + (_e199 * _e207));
    let _e210 = color;
    let _e214 = a;
    let _e215 = evaluate_cluster_lightsX_naga_oil_mod_XMZXXEZ3FMF4F62DEOJYDUOTDNR2XG5DFOJPWM33SO5QXEZAX(in_1.ndc, in_1.viewZ, in_1.worldPos, _e61, v, albedo, metallic_5, _e214);
    color = (_e210 + _e215);
    let _e217 = color;
    let _e220 = material.clearcoat;
    let _e227 = evaluate_cluster_lightsX_naga_oil_mod_XMZXXEZ3FMF4F62DEOJYDUOTDNR2XG5DFOJPWM33SO5QXEZAX(in_1.ndc, in_1.viewZ, in_1.worldPos, _e61, v, vec3(0f), 1f, coatRoughness);
    color = (_e217 + (_e220 * _e227));
    let _e232 = material.emissiveUvScale;
    let _e235 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(emissiveTexture, emissiveSampler, _e1, _e232);
    let emissiveSample = _e235.xyz;
    let _e237 = color;
    let _e240 = material.emissive;
    let _e243 = material.emissiveIntensity;
    color = (_e237 + ((_e240 * _e243) * emissiveSample));
    let _e247 = color;
    let _e251 = material.baseColor.w;
    return vec4<f32>(_e247, (_e251 * _e7.w));
}
