...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let _e289 = composeProbeDiffuse(probeShPreblend, probeLocalBlendFraction, _e285, (_e286 * skyFactor), probeDiffuseK, _e288, metallic_4);
    ...
    let _e373 = material.thickness;
    let _e375 = finiteScalar(_e373, 0f);
    let _e379 = finiteScalar(thicknessSample, 1f);
    let worldThickness = max(((_e375 * length(worldRefractedDirection)) * _e379), 0f);
    ...
    let _e421 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(screenUv, vec2(0f), vec2(1f)), backdropLod);
    let unrefractedBackdrop = _e421.xyz;
    ...
    let _e429 = local_1;
    if _e429 {
        let _e437 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(refractedUv, vec2(0f), vec2(1f)), backdropLod);
        transmittedBackdrop = _e437.xyz;
    }
    let _e441 = material.attenuationColor;
    let _e444 = finiteColor(_e441, vec3(1f));
    let safeAttenuationColor = clamp(_e444, vec3(0f), vec3(1f));
    let _e452 = material.attenuationDistance;
    let _e454 = finiteScalar(_e452, 0f);
    let safeAttenuationDistance = max(_e454, 0f);
    ...
    let _e471 = local_2;
    let beerAttenuation = select(vec3(1f), pow(safeAttenuationColor, vec3(attenuationExponent)), _e471);
    let _e473 = color;
    let _e474 = transmittedBackdrop;
    let _e477 = finiteColor(_e474, vec3(0f));
    color = (_e473 + ((_e477 * transmittedEnergy) * beerAttenuation));
    let _e481 = physicalNormal;
    let _e483 = standardViewZ(in_2);
    let _e484 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e481, in_2.worldPos, _e483);
    let _e485 = physicalNormal;
    let _e486 = diffuseAlbedo;
    let _e487 = f0_;
    let _e488 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e485, v, _e486, metallic_4, a_3, _e487);
    let _e489 = color;
    color = (_e489 + (_e484 * _e488));
    let _e492 = color;
    color = (_e492 + _e6.emissive);
    let _e497 = color;
    output.color = vec4<f32>(_e497, alpha_1);
    let _e500 = reflectionFallback;
    output.reflectionFallback = vec4<f32>(_e500, 1f);
    let _e503 = output;
    return _e503;
}
