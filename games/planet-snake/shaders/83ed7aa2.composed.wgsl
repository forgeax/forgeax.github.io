            ...
            loop {
                ...
                }
                {
                    ...
                    if _e41 {
                        let _e44 = x_2;
                        let _e46 = y_2;
                        let offsetUv_2 = (uv + (vec2<f32>(f32(_e44), f32(_e46)) * texel));
...
fn spotModifierFactorsX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXG4DPORPW233ENFTGSZLSOMX(lightPos_7: vec3<f32>, lightDir_4: vec3<f32>, cosOuter_4: f32, worldPos_10: vec3<f32>, rollDeg: f32, metadata: vec4<u32>) -> vec4<f32> {
    ...
    if (metadata.w != 4294967295u) {
        if !((metadata.y == 4294967295u)) {
            local_12 = ((metadata.y & PROJECTOR_FLAGX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXG4DPORPW233ENFTGSZLSOMX) == 0u);
        } else {
            local_12 = true;
        }
        let _e90 = local_12;
    ...
    if _e94 {
        if (depth <= 0f) {
            ...
            if !((cookieUv.x < 0f)) {
                local_13 = (cookieUv.x > 1f);
            } else {
                local_13 = true;
            }
            let _e137 = local_13;
            if !(_e137) {
                local_14 = (cookieUv.y < 0f);
            } else {
                local_14 = true;
            }
            let _e145 = local_14;
            if !(_e145) {
                local_15 = (cookieUv.y > 1f);
            } else {
                local_15 = true;
            }
            let _e153 = local_15;
...
fn projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX(baseColorAlpha_1: f32, alphaCutoff: f32, baseColorTexture: texture_2d<f32>, baseColorSampler: sampler, transform_1: vec4<f32>, metadata_2: vec4<f32>, currentClip_1: vec4<f32>, previousClip_1: vec4<f32>, temporalProjection_3: vec4<f32>, reactive_2: f32, uv0_1: vec2<f32>, uv1_1: vec2<f32>, uv2_1: vec2<f32>, uv3_1: vec2<f32>, uv4_1: vec2<f32>, uv5_1: vec2<f32>, uv6_1: vec2<f32>, uv7_1: vec2<f32>) -> vec4<f32> {
    ...
    if (alphaCutoff > 0f) {
        local_18 = ((baseColorAlpha_1 * _e14.w) <= alphaCutoff);
    } else {
        local_18 = false;
    }
    let _e25 = local_18;
