...
fn fs_main(in_1: VsOut) -> @location(0) vec4<f32> {
    ...
    let _e72 = a;
    a = (_e71 * _e72);
    let _e76 = material.normalTextureCoordinatesTransform;
    let _e79 = material.normalTextureCoordinatesMetadata;
    let _e80 = transformedMaterialUv(_e76, _e79, in_1);
    let _e83 = material.normalTextureCoordinatesMetadata;
    let _e87 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(normalTexture, normalSampler, _e80, _e83.zw);
    let normSampleRg = _e87.xy;
    let _e89 = decodeTangentSpaceNormalRgX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DCNYX(normSampleRg);
    let _e92 = material.normalScale;
    let _e93 = scaleTangentSpaceNormalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DCNYX(_e89, _e92);
    let _e96 = applyTBNX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DCNYX(in_1.worldNormal, in_1.worldTangent, _e93);
    let _e99 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cameraPos;
    let v = normalize((_e99 - in_1.worldPos));
    let _e105 = material.specularTintTextureCoordinatesTransform;
    let _e108 = material.specularTintTextureCoordinatesMetadata;
    let _e109 = transformedMaterialUv(_e105, _e108, in_1);
    let _e112 = material.specularTint;
    let _e115 = material.specularTintTextureCoordinatesMetadata;
    let _e119 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(specularTintTexture, specularTintSampler, _e109, _e115.zw);
    let specularTint = (_e112 * _e119.xyz);
    let f0_3 = mix((vec3(0.04f) * specularTint), albedo, metallic_7);
    let _e128 = material.clearcoatRoughness;
    let coatRoughness = max(_e128, 0.04f);
    let coatAlpha = (coatRoughness * coatRoughness);
    let _e137 = f_schlickX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(max(dot(_e96, v), 0f), vec3(0.04f));
    let _e140 = material.clearcoat;
    let coatF = (_e137 * _e140);
    let _e147 = f_schlickX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(max(dot(_e96, v), 0f), f0_3);
    let kD = ((vec3(1f) - _e147) * (1f - metallic_7));
    let _e154 = material.roughness;
    let iblRoughness = (max(_e154, 0.04f) * _e62);
    let _e160 = skylight.rotation;
    let _e163 = sampleIblDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e96, _e160, irradianceMap_1, irradianceSampler_1);
    let _e166 = skylight.rotation;
    let _e171 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e96, v, iblRoughness, f0_3, _e166, prefilterMap_1, prefilterSampler_1, brdfLut_1, brdfLutSampler_1);
    let _e174 = material.occlusionTextureCoordinatesTransform;
    let _e177 = material.occlusionTextureCoordinatesMetadata;
    let _e178 = transformedMaterialUv(_e174, _e177, in_1);
    let _e181 = material.occlusionTextureCoordinatesMetadata;
    let _e185 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(occlusionTexture, occlusionSampler, _e178, _e181.zw);
    let _e189 = material.occlusionStrength;
    let ao = mix(1f, _e185.x, _e189);
    let _e194 = skylight.colorR;
    let _e197 = skylight.colorG;
    let _e200 = skylight.colorB;
    let skyColor = vec3<f32>(_e194, _e197, _e200);
    ambient = ((((kD * _e163) * albedo) + _e171) * (vec3(1f) - coatF));
    let _e212 = material.clearcoat;
    if (_e212 != 0f) {
        let _e219 = skylight.rotation;
        let _e224 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e96, v, coatRoughness, vec3(0.04f), _e219, prefilterMap_1, prefilterSampler_1, brdfLut_1, brdfLutSampler_1);
        let _e225 = ambient;
        let _e228 = material.clearcoat;
        ambient = (_e225 + (_e224 * _e228));
    }
    let _e231 = ambient;
    let _e235 = skylight.intensity;
    ambient = (((_e231 * skyColor) * _e235) * ao);
    let ssaoUv = ((in_1.ndc.xy * vec2<f32>(0.5f, -0.5f)) + vec2<f32>(0.5f, 0.5f));
    let _e250 = textureSample(ssaoBlurredTexture, ssaoBlurredSampler, ssaoUv);
    let ssaoFactor = _e250.x;
    let _e252 = get_ssao_intensityX_naga_oil_mod_XMZXXEZ3FMF4F62DEOJYDUOTDNR2XG5DFOJPWM33SO5QXEZAX();
    let _e253 = ambient;
    ambient = (_e253 * mix(1f, (ssaoFactor * ao), _e252));
    let _e258 = ambient;
    color = _e258;
    let _e262 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e96, in_1.worldPos, in_1.viewZ);
    let _e263 = a;
    let _e264 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e96, v, albedo, metallic_7, _e263, f0_3);
    let _e265 = color;
    color = (_e265 + (_e262 * _e264));
    let _e270 = material.clearcoat;
    if (_e270 != 0f) {
        let _e278 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e96, v, vec3(0f), 1f, coatAlpha, vec3(0.04f));
        let _e279 = color;
        let _e282 = material.clearcoat;
        color = (_e279 + ((_e262 * _e282) * _e278));
    }
    let _e286 = color;
    let _e290 = a;
    let _e291 = evaluate_cluster_lightsX_naga_oil_mod_XMZXXEZ3FMF4F62DEOJYDUOTDNR2XG5DFOJPWM33SO5QXEZAX(in_1.ndc, in_1.viewZ, in_1.worldPos, _e96, v, albedo, metallic_7, _e290, f0_3);
    color = (_e286 + _e291);
    let _e295 = material.clearcoat;
    if (_e295 != 0f) {
        let _e298 = color;
        let _e301 = material.clearcoat;
        let _e310 = evaluate_cluster_lightsX_naga_oil_mod_XMZXXEZ3FMF4F62DEOJYDUOTDNR2XG5DFOJPWM33SO5QXEZAX(in_1.ndc, in_1.viewZ, in_1.worldPos, _e96, v, vec3(0f), 1f, coatAlpha, vec3(0.04f));
        color = (_e298 + (_e301 * _e310));
    }
    let _e315 = material.emissiveTextureCoordinatesTransform;
    let _e318 = material.emissiveTextureCoordinatesMetadata;
    let _e319 = transformedMaterialUv(_e315, _e318, in_1);
    let _e322 = material.emissiveTextureCoordinatesMetadata;
    let _e326 = sampleMaterialTextureX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX(emissiveTexture, emissiveSampler, _e319, _e322.zw);
    let emissiveSample = _e326.xyz;
    let _e328 = color;
    let _e331 = material.emissive;
    let _e334 = material.emissiveIntensity;
    color = (_e328 + ((_e331 * _e334) * emissiveSample));
    let _e339 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX;
    let _e340 = color;
    let _e342 = applySceneFog(_e339, _e340, alpha_3, in_1.worldPos);
    return _e342;
}
