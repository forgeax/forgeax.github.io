...
fn evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_5: vec3<f32>, viewDir: vec3<f32>, baseColor: vec3<f32>, metallic_1: f32, alphaSq: f32, F0_4: vec3<f32>) -> vec3<f32> {
    ...
    let nDotL_3 = max(dot(normal_5, l_2), 0f);
...
fn surfaceUv(input: SurfaceInput, transform_3: vec4<f32>, metadata_3: vec4<f32>) -> vec2<f32> {
    let source_2 = select(input.uv0_, input.uv1_, (metadata_3.x >= 1f));
...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> @location(0) vec4<f32> {
    ...
    let _e282 = composeProbeDiffuse(probeShPreblend, probeLocalBlendFraction, _e278, (_e279 * skyFactor), probeDiffuseK, _e281, metallic_4);
    ...
    let _e483 = f0_;
    let _e484 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e481, v, _e482, metallic_4, a_2, _e483);
    let _e485 = color;
    color = (_e485 + (_e480 * _e484));
    let _e488 = color;
    return vec4<f32>(_e488, alpha);
}
