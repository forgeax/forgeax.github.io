...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let _e171 = skylight.intensity;
    if (_e171 < 0f) {
        let _e176 = skylight.intensity;
        let environmentScale = (max((-(_e176) - 1f), 0f) * ao);
        let _e183 = ambient;
        ambient = (_e183 * environmentScale);
        let _e185 = reflectionFallback;
        reflectionFallback = (_e185 * environmentScale);
    } else {
        let _e189 = skylight.intensity;
        let environmentScale_1 = ((skyColor * _e189) * ao);
        let _e192 = ambient;
        ambient = (_e192 * environmentScale_1);
        let _e194 = reflectionFallback;
        reflectionFallback = (_e194 * environmentScale_1);
    }
    let ssaoUv = ((in_2.ndc.xy * vec2<f32>(0.5f, -0.5f)) + vec2<f32>(0.5f, 0.5f));
    let _e208 = textureSample(ssaoBlurredTexture, ssaoBlurredSampler, ssaoUv);
    let ssaoFactor = _e208.x;
    let _e210 = get_ssao_intensityX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX();
    let _e211 = ambient;
    ambient = (_e211 * mix(1f, (ssaoFactor * ao), _e210));
    let ssaoScale = mix(1f, (ssaoFactor * ao), _e210);
    let _e219 = reflectionFallback;
    reflectionFallback = (_e219 * ssaoScale);
    let _e221 = ambient;
    color = _e221;
    let _e223 = physicalNormal;
    let _e225 = standardViewZ(in_2);
    let _e226 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e223, in_2.worldPos, _e225);
    let _e227 = physicalNormal;
    let _e228 = diffuseAlbedo;
    let _e229 = f0_;
    let _e230 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e227, v_5, _e228, metallic_13, a_4, _e229);
    let _e231 = color;
    color = (_e231 + (_e226 * _e230));
    let _e234 = color;
    let _e237 = standardViewZ(in_2);
    let _e239 = physicalNormal;
    let _e240 = diffuseAlbedo;
    let _e241 = f0_;
    let _e243 = evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_2.ndc.xyz, _e237, in_2.worldPos, _e239, v_5, _e240, metallic_13, a_4, _e241, false);
    color = (_e234 + _e243);
    let _e245 = color;
    color = (_e245 + _e6.emissive);
    let _e250 = color;
    output.color = vec4<f32>(_e250, alpha_1);
    let _e253 = reflectionFallback;
    output.reflectionFallback = vec4<f32>(_e253, 1f);
    let _e256 = output;
    return _e256;
}
