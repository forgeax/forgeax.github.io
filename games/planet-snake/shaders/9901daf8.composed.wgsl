...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> @location(0) vec4<f32> {
    ...
    let kD = ((vec3(1f) - _e73) * (1f - metallic_11));
    ...
    let _e158 = coatF;
    ambient = ((((kD * _e149) * _e151) + _e153) * (vec3(1f) - _e158));
    let _e164 = skylight.intensity;
    if (_e164 < 0f) {
        let _e169 = skylight.intensity;
        let environmentScale = (max((-(_e169) - 1f), 0f) * ao);
        let _e176 = ambient;
        ambient = (_e176 * environmentScale);
    } else {
        let _e180 = skylight.intensity;
        let environmentScale_1 = ((skyColor * _e180) * ao);
        let _e183 = ambient;
        ambient = (_e183 * environmentScale_1);
    }
    let _e185 = ambient;
    color = _e185;
    let _e187 = color;
    color = (_e187 + _e6.emissive);
    ...
    let _e195 = diffuseAlbedo;
    let _e196 = f0_;
    let _e197 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e194, v_5, _e195, metallic_11, a_3, _e196);
    let _e198 = color;
    color = (_e198 + (_e193 * _e197));
    let _e201 = color;
    let _e204 = standardViewZ(in_2);
    let _e206 = physicalNormal;
    let _e207 = diffuseAlbedo;
    let _e208 = f0_;
    let _e210 = evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_2.ndc.xyz, _e204, in_2.worldPos, _e206, v_5, _e207, metallic_11, a_3, _e208, false);
    color = (_e201 + _e210);
    let _e212 = color;
    return vec4<f32>(_e212, alpha);
}
