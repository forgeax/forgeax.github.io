...
fn vs_main(in: VsInput, @builtin(instance_index) idx: u32) -> VsOut {
    ...
    let worldNormal = normalize((skinMatrix * vec4<f32>(in.normal, 0f)).xyz);
    let worldTangent = normalize((skinMatrix * vec4<f32>(in.tangent.xyz, 0f)).xyz);
    let _e55 = shadowCasterCascadeX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.isSpot;
    if (_e55 == 1u) {
        let _e62 = shadowCasterCascadeX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.spotLightViewProj;
        out.clip = (_e62 * worldPos);
    ...
    let _e81 = shadowCasterCascadeX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.index;
    let _e82 = _cascadeLightViewProj(_e81);
    out_1.clip = (_e82 * worldPos);
