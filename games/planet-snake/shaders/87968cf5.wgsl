...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> StandardPbrOutput {
    ...
    let _e149 = specularIbl;
    let _e153 = coatF;
    reflectionFallback = (_e149 * (vec3(1f) - _e153));
    let _e157 = irradiance;
    let _e159 = diffuseAlbedo;
    let _e161 = specularIbl;
    let _e165 = coatF;
    ambient = ((((kD_1 * _e157) * _e159) + _e161) * (vec3(1f) - _e165));
    let _e171 = probeBlendRecords[1];
    let _e174 = probeBlendRecords[2];
    let _e177 = probeBlendRecords[3];
    let _e180 = probeBlendRecords[4];
    let _e183 = probeBlendRecords[5];
    let _e186 = probeBlendRecords[6];
    let _e189 = probeBlendRecords[7];
    let _e192 = probeBlendRecords[8];
    let _e195 = probeBlendRecords[9];
    let probeShPreblend = array<vec4<f32>, 9>(_e171, _e174, _e177, _e180, _e183, _e186, _e189, _e192, _e195);
    ...
    let _e209 = irradiance;
    let _e211 = diffuseAlbedo;
    let _e212 = composeProbeDiffuse(probeShPreblend, probeLocalBlendFraction, _e208, (_e209 * skyFactor), probeDiffuseK, _e211, metallic_13);
    let _e213 = specularIbl;
    let _e218 = coatF;
    ambient = ((_e212 + (_e213 * skyFactor)) * (vec3(1f) - _e218));
    let _e221 = ambient;
    ambient = (_e221 * ao);
    let ssaoUv = ((in_2.ndc.xy * vec2<f32>(0.5f, -0.5f)) + vec2<f32>(0.5f, 0.5f));
    let _e235 = textureSample(ssaoBlurredTexture, ssaoBlurredSampler, ssaoUv);
    let ssaoFactor = _e235.x;
    let _e237 = get_ssao_intensityX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX();
    let _e238 = ambient;
    ambient = (_e238 * mix(1f, (ssaoFactor * ao), _e237));
    let ssaoScale = mix(1f, (ssaoFactor * ao), _e237);
    let _e246 = reflectionFallback;
    reflectionFallback = (_e246 * ssaoScale);
    let _e248 = ambient;
    color = _e248;
    let _e250 = physicalNormal;
    let _e252 = standardViewZ(in_2);
    let _e253 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e250, in_2.worldPos, _e252);
    let _e254 = physicalNormal;
    let _e255 = diffuseAlbedo;
    let _e256 = f0_;
    let _e257 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e254, v_5, _e255, metallic_13, a_4, _e256);
    let _e258 = color;
    color = (_e258 + (_e253 * _e257));
    let _e261 = color;
    let _e264 = standardViewZ(in_2);
    let _e266 = physicalNormal;
    let _e267 = diffuseAlbedo;
    let _e268 = f0_;
    let _e270 = evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_2.ndc.xyz, _e264, in_2.worldPos, _e266, v_5, _e267, metallic_13, a_4, _e268, false);
    color = (_e261 + _e270);
    let _e272 = color;
    color = (_e272 + _e6.emissive);
    let _e277 = color;
    output.color = vec4<f32>(_e277, alpha_1);
    let _e280 = reflectionFallback;
    output.reflectionFallback = vec4<f32>(_e280, 1f);
    let _e283 = output;
    return _e283;
}
