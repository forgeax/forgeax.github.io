...
fn evaluateProbeDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX(shPreblend_1: array<vec4<f32>, 9>, localBlendFraction: f32, normal_1: vec3<f32>, e_sky: vec3<f32>, k_d: vec3<f32>, albedo: vec3<f32>, metallic: f32) -> vec3<f32> {
    ...
    let local_35 = (max(_e2, vec3(0f)) * PROBE_INV_PIX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX);
    let skyResidualFraction = (1f - localBlendFraction);
    return ((((local_35 + (skyResidualFraction * e_sky)) * k_d) * albedo) * (1f - metallic));
}
...
fn sampleThreeR184DfgLutX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(roughness_3: f32, dotNV: f32) -> vec2<f32> {
    let uv_4 = clamp(vec2<f32>(roughness_3, dotNV), vec2(0f), vec2(1f));
...
fn threeR184DirectMultiScatterX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(roughness_4: f32, nDotV_1: f32, nDotL_2: f32, F0_3: vec3<f32>) -> vec3<f32> {
    let _e2 = sampleThreeR184DfgLutX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(roughness_4, nDotV_1);
    let _e4 = sampleThreeR184DfgLutX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(roughness_4, nDotL_2);
...
fn evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_5: vec3<f32>, viewDir: vec3<f32>, baseColor: vec3<f32>, metallic_1: f32, alphaSq: f32, F0_4: vec3<f32>) -> vec3<f32> {
    ...
    let nDotL_4 = max(dot(normal_5, l_2), 0f);
    ...
    let _e31 = threeR184DirectMultiScatterX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(roughness_6, nDotV_2, nDotL_4, F0_4);
    ...
    return (((diffuse + specular) * _e48) * nDotL_4);
}
...
fn _samplePcssForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos: vec3<f32>, layer_4: u32, count_4: u32, normal_6: vec3<f32>, l: vec3<f32>, profile: u32) -> f32 {
    var local_3: bool;
    var local_4: bool;
    var local_5: bool;
    var local_6: bool;
    ...
    let _e60 = local_6;
    if _e60 {
        local_7 = (projCoords.z >= 0f);
    } else {
        local_7 = false;
    }
    let _e67 = local_7;
    if _e67 {
        local_8 = (projCoords.z <= 1f);
    } else {
        local_8 = false;
    }
    let _e74 = local_8;
    ...
    let _e116 = shadow_biased_receiver_depthX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43IMFSG6527OBRWMX(projCoords.z, _e112, _e115, nDotL_5);
    ...
    if (lightDepthWorldSpan > 0f) {
        local_12 = (worldUnitsPerTexel > 0f);
    } else {
        local_12 = false;
    }
    let _e216 = local_12;
...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_1: vec3<f32>, layer_5: u32, count_5: u32, normal_7: vec3<f32>, l_1: vec3<f32>, useNormalBias: bool) -> f32 {
    var local_15: bool;
    var local_16: bool;
    var local_17: bool;
    var local_18: bool;
    ...
    let bias = select(_e33, max((_e36 * (1f - dot(normal_7, l_1))), _e46), useNormalBias);
    let adjustedDepth = (currentDepth - bias);
    if (tileUv_1.x >= 0f) {
        local_15 = (tileUv_1.x <= 1f);
    } else {
        local_15 = false;
    }
    let _e59 = local_15;
    if _e59 {
        local_16 = (tileUv_1.y >= 0f);
    } else {
        local_16 = false;
    }
    let _e66 = local_16;
    if _e66 {
        local_17 = (tileUv_1.y <= 1f);
    } else {
        local_17 = false;
    }
    let _e73 = local_17;
    if _e73 {
        local_18 = (currentDepth <= 1f);
    } else {
        local_18 = false;
    }
    let _e79 = local_18;
    ...
    let texel_4 = vec2<f32>((1f / texelDims.x), (1f / texelDims.y));
    let tileLo = (_e15 + texel_4);
    let tileHi = ((_e15 + _e14) - texel_4);
    ...
    if (filterProfile >= 4u) {
        let _e107 = _samplePcssForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_1, layer_5, count_5, normal_7, l_1, filterProfile);
        return _e107;
    }
    let kernel = select(select(3u, 5u, (filterProfile == 3u)), 1u, (filterProfile == 1u));
    if (kernel == 1u) {
        let lit = textureSampleCompareLevel(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, clamp(uv_5, tileLo, tileHi), adjustedDepth);
        return lit;
    }
    if (kernel == 3u) {
        if all((uv_5 >= (tileLo + texel_4))) {
            local_19 = all((uv_5 <= (tileHi - texel_4)));
        } else {
            local_19 = false;
        }
        let interior = local_19;
        if interior {
            let pcfFraction = fract(((uv_5 / texel_4) - vec2(0.5f)));
            ...
            let litLoLo = textureSampleCompareLevel(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, (uv_5 + (vec2<f32>(loOffset.x, loOffset.y) * texel_4)), adjustedDepth);
            let litHiLo = textureSampleCompareLevel(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, (uv_5 + (vec2<f32>(hiOffset.x, loOffset.y) * texel_4)), adjustedDepth);
            let litLoHi = textureSampleCompareLevel(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, (uv_5 + (vec2<f32>(loOffset.x, hiOffset.y) * texel_4)), adjustedDepth);
            let litHiHi = textureSampleCompareLevel(shadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, (uv_5 + (vec2<f32>(hiOffset.x, hiOffset.y) * texel_4)), adjustedDepth);
            ...
            }
            {
                ...
                loop {
                    ...
                    }
                    {
                        let _e220 = x;
                        let _e222 = y;
                        let offsetUv = clamp((uv_5 + (vec2<f32>(f32(_e220), f32(_e222)) * texel_4)), tileLo, tileHi);
        ...
        }
        {
            ...
            loop {
                ...
                }
                {
                    let _e256 = x_1;
                    let _e258 = y_1;
                    let offsetUv_1 = clamp((uv_5 + (vec2<f32>(f32(_e256), f32(_e258)) * texel_4)), tileLo, tileHi);
...
fn evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(normal_8: vec3<f32>, worldPos_2: vec3<f32>, viewZ: f32) -> f32 {
    var shadow: f32;
    var local_20: bool;
    ...
    let _e32 = _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_2, _e28, count_6, normal_8, l_3, true);
    shadow = _e32;
    let _e36 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.cascadeBlend;
    if (_e36 > 0f) {
        local_20 = ((_e28 + 1u) < count_6);
    } else {
        local_20 = false;
    }
    let _e45 = local_20;
    if _e45 {
        ...
        if (blendWidth > 0f) {
            ...
            if (t_1 > 0f) {
                let _e69 = _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_2, (_e28 + 1u), count_6, normal_8, l_3, true);
...
fn evalSpotAttenuationX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWC5DUMVXHKYLUNFXW4X(lightPos: vec3<f32>, lightDir: vec3<f32>, worldPos_3: vec3<f32>, cosInner: f32, cosOuter: f32, invRangeSquared_1: f32) -> f32 {
    let toLight = (lightPos - worldPos_3);
...
fn projectSpotUvX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWC5DUMVXHKYLUNFXW4X(lightViewProj: mat4x4<f32>, worldPos_4: vec3<f32>) -> vec2<f32> {
    let clip_3 = (lightViewProj * vec4<f32>(worldPos_4, 1f));
    let invW = select((1f / clip_3.w), 0f, (abs(clip_3.w) < 0.000001f));
    return vec2<f32>((((clip_3.x * invW) * 0.5f) + 0.5f), (((clip_3.y * invW) * -0.5f) + 0.5f));
}

fn spotModifierProductX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXG4DPORPW233ENFTGSZLSOMX(brdf: f32, range: f32, cone: f32, ies: f32, cookie: f32, shadow_1: f32) -> f32 {
    return (((((brdf * range) * cone) * ies) * cookie) * shadow_1);
}

fn sample_shadow_2d_kernelX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43IMFSG6527OBRWMX(shadowMap_2: texture_depth_2d, shadowSampler_1: sampler_comparison, uv_3: vec2<f32>, texel_3: vec2<f32>, depthRef_1: f32, normalBias_1: f32, depthBias_1: f32, nDotL_3: f32, kernelSize: f32) -> f32 {
    ...
    let bias_1 = max((normalBias_1 * (1f - nDotL_3)), (depthBias_1 / 1000f));
    let adjustedDepth_1 = (depthRef_1 - bias_1);
            ...
            loop {
                ...
                }
                {
                    ...
                    if _e41 {
                        let _e44 = x_2;
                        let _e46 = y_2;
                        let offsetUv_2 = (uv_3 + (vec2<f32>(f32(_e44), f32(_e46)) * texel_3));
                        let lit_3 = textureSampleCompareLevel(shadowMap_2, shadowSampler_1, offsetUv_2, adjustedDepth_1);
                        let _e55 = blocked_1;
                        blocked_1 = (_e55 + (1f - lit_3));
                        let _e60 = samples;
                        samples = (_e60 + 1f);
                    }
                }
...
fn evalPunctualBodyX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(lightPos_1: vec3<f32>, colorTimesIntensity: vec3<f32>, invRangeSquared_2: f32, worldPos_5: vec3<f32>, normal_9: vec3<f32>, viewDir_1: vec3<f32>, baseColor_1: vec3<f32>, metallic_2: f32, alphaSq_1: f32, F0_5: vec3<f32>) -> vec3<f32> {
    let toLight_1 = (lightPos_1 - worldPos_5);
    let dSquared_3 = max(dot(toLight_1, toLight_1), 0.0001f);
    let l_4 = (toLight_1 / vec3(sqrt(dSquared_3)));
    let h_1 = normalize((viewDir_1 + l_4));
    let nDotL_6 = max(dot(normal_9, l_4), 0f);
    let nDotV_3 = max(dot(normal_9, viewDir_1), 0.00001f);
    let nDotH_2 = max(dot(normal_9, h_1), 0f);
    let vDotH_2 = max(dot(viewDir_1, h_1), 0f);
    let _e26 = f_schlickX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(vDotH_2, F0_5);
    let roughness_7 = sqrt(max(alphaSq_1, 0f));
    let _e31 = threeR184DirectMultiScatterX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(roughness_7, nDotV_3, nDotL_6, F0_5);
    ...
    let diffuse_1 = (((1f - metallic_2) * baseColor_1) / vec3(3.1415927f));
    ...
    return ((((diffuse_1 + specular_1) * colorTimesIntensity) * nDotL_6) * _e46);
}

fn evalPointX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(lightPos_2: vec3<f32>, colorTimesIntensity_1: vec3<f32>, invRangeSquared_3: f32, worldPos_6: vec3<f32>, normal_10: vec3<f32>, viewDir_2: vec3<f32>, baseColor_2: vec3<f32>, metallic_3: f32, alphaSq_2: f32, F0_6: vec3<f32>) -> vec3<f32> {
    let _e10 = evalPunctualBodyX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(lightPos_2, colorTimesIntensity_1, invRangeSquared_3, worldPos_6, normal_10, viewDir_2, baseColor_2, metallic_3, alphaSq_2, F0_6);
    return _e10;
}
...
fn evalPointFlatX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(lightPos_3: vec3<f32>, colorTimesIntensity_2: vec3<f32>, invRangeSquared_5: f32, worldPos_7: vec3<f32>, baseColor_3: vec3<f32>) -> vec3<f32> {
    let toLight_2 = (lightPos_3 - worldPos_7);
...
fn evalSpotFlatX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(lightPos_4: vec3<f32>, lightDir_1: vec3<f32>, colorTimesIntensity_3: vec3<f32>, cosInner_1: f32, cosOuter_1: f32, invRangeSquared_6: f32, worldPos_8: vec3<f32>, baseColor_4: vec3<f32>) -> vec3<f32> {
    let toLight_3 = (lightPos_4 - worldPos_8);
    ...
    let _e18 = evalPointFlatX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(lightPos_4, colorTimesIntensity_3, invRangeSquared_6, worldPos_8, baseColor_4);
    return (_e18 * cone_2);
}

fn evalSpotX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(lightPos_5: vec3<f32>, lightDir_2: vec3<f32>, colorTimesIntensity_4: vec3<f32>, cosInner_2: f32, cosOuter_2: f32, invRangeSquared_7: f32, worldPos_9: vec3<f32>, normal_11: vec3<f32>, viewDir_3: vec3<f32>, baseColor_5: vec3<f32>, metallic_4: f32, alphaSq_3: f32, F0_7: vec3<f32>) -> vec3<f32> {
    let _e10 = evalPunctualBodyX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(lightPos_5, colorTimesIntensity_4, invRangeSquared_7, worldPos_9, normal_11, viewDir_3, baseColor_5, metallic_4, alphaSq_3, F0_7);
    let toLight_4 = (lightPos_5 - worldPos_9);
...
fn evalSpotShadowedX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(lightPos_6: vec3<f32>, lightDir_3: vec3<f32>, colorTimesIntensity_5: vec3<f32>, cosInner_3: f32, cosOuter_3: f32, invRangeSquared_8: f32, worldPos_10: vec3<f32>, normal_12: vec3<f32>, viewDir_4: vec3<f32>, baseColor_6: vec3<f32>, metallic_5: f32, alphaSq_4: f32, F0_8: vec3<f32>, lightViewProj_1: mat4x4<f32>, shadowAtlasTile: i32, depthBias_2: f32, normalBias_2: f32, pcfKernelSize: f32, shadowIntensity: f32) -> vec3<f32> {
    var local_22: bool;
    var local_23: bool;
    var local_24: bool;
    var local_25: bool;

    let _e13 = evalSpotX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(lightPos_6, lightDir_3, colorTimesIntensity_5, cosInner_3, cosOuter_3, invRangeSquared_8, worldPos_10, normal_12, viewDir_4, baseColor_6, metallic_5, alphaSq_4, F0_8);
    let splane = (lightViewProj_1 * vec4<f32>(worldPos_10, 1f));
    let invW_1 = select((1f / splane.w), 0f, (abs(splane.w) < 0.000001f));
    let _e27 = projectSpotUvX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWC5DUMVXHKYLUNFXW4X(lightViewProj_1, worldPos_10);
    ...
    let tileOrigin_3 = (vec2<f32>(col, row) * 0.5f);
    let atlasUv = ((_e27 * 0.5f) + tileOrigin_3);
    let _e76 = textureDimensions(spotShadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, 0i);
    let atlasDims = vec2<f32>(_e76);
    let texel_5 = (vec2<f32>(1f, 1f) / atlasDims);
    let nDotL_7 = max(dot(normal_12, normalize((lightPos_6 - worldPos_10))), 0f);
    let _e92 = sample_shadow_2d_kernelX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43IMFSG6527OBRWMX(spotShadowMapX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, shadowSamplerX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX, atlasUv, texel_5, depthRef_2, normalBias_2, depthBias_2, nDotL_7, pcfKernelSize);
    return (_e13 * mix(1f, _e92, clamp(shadowIntensity, 0f, 1f)));
}

fn spotModifierFactorsX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXG4DPORPW233ENFTGSZLSOMX(lightPos_7: vec3<f32>, lightDir_4: vec3<f32>, cosOuter_4: f32, worldPos_11: vec3<f32>, rollDeg: f32, metadata: vec4<u32>) -> vec4<f32> {
    var ies_1: f32 = 1f;
    ...
    let toSurface = (worldPos_11 - lightPos_7);
    ...
    if (metadata.w != 4294967295u) {
        if !((metadata.y == 4294967295u)) {
            local_27 = ((metadata.y & PROJECTOR_FLAGX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXG4DPORPW233ENFTGSZLSOMX) == 0u);
        } else {
            local_27 = true;
        }
        let _e90 = local_27;
    ...
    if _e94 {
        if (depth <= 0f) {
            ...
            if !((cookieUv.x < 0f)) {
                local_28 = (cookieUv.x > 1f);
            } else {
                local_28 = true;
            }
            let _e137 = local_28;
            if !(_e137) {
                local_29 = (cookieUv.y < 0f);
            } else {
                local_29 = true;
            }
            let _e145 = local_29;
            if !(_e145) {
                local_30 = (cookieUv.y > 1f);
            } else {
                local_30 = true;
            }
            let _e153 = local_30;
...
fn ltcUvX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXEZLDORPWC4TFMEX(n: vec3<f32>, v: vec3<f32>, roughness_5: f32) -> vec2<f32> {
    let nDotV_4 = clamp(dot(n, v), 0f, 1f);
    return ((vec2<f32>(clamp(roughness_5, 0f, 1f), sqrt((1f - nDotV_4))) * 0.984375f) + vec2(0.0078125f));
}
...
fn ltcEvaluateX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXEZLDORPWC4TFMEX(n_1: vec3<f32>, v_1: vec3<f32>, worldPos_12: vec3<f32>, mInv: mat3x3<f32>, lightPos_8: vec3<f32>, axisX: vec3<f32>, axisY: vec3<f32>, halfWidth: f32, halfHeight: f32) -> f32 {
    ...
    if (dot(lightNormal, (worldPos_12 - rect0_)) <= 0f) {
        return 0f;
    }
    ...
    let c0_ = normalize((transform_4 * (rect0_ - worldPos_12)));
    let c1_ = normalize((transform_4 * (rect1_ - worldPos_12)));
    let c2_ = normalize((transform_4 * (rect2_ - worldPos_12)));
    let c3_ = normalize((transform_4 * (rect3_ - worldPos_12)));
...
fn rectAreaRangeFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXEZLDORPWC4TFMEX(lightPos_9: vec3<f32>, worldPos_13: vec3<f32>, invRangeSquared_9: f32) -> f32 {
    let toLight_5 = (lightPos_9 - worldPos_13);
...
fn evalRectAreaLtcDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXEZLDORPWC4TFMEX(lightPos_10: vec3<f32>, lightColor: vec3<f32>, axisX_1: vec3<f32>, axisY_1: vec3<f32>, halfWidth_1: f32, halfHeight_1: f32, invRangeSquared_10: f32, worldPos_14: vec3<f32>, n_2: vec3<f32>, v_2: vec3<f32>, baseColor_7: vec3<f32>, metallic_6: f32, alphaSq_5: f32, F0_9: vec3<f32>) -> vec3<f32> {
    let identity_1 = mat3x3<f32>(vec3<f32>(1f, 0f, 0f), vec3<f32>(0f, 1f, 0f), vec3<f32>(0f, 0f, 1f));
    let _e21 = ltcEvaluateX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXEZLDORPWC4TFMEX(n_2, v_2, worldPos_14, identity_1, lightPos_10, axisX_1, axisY_1, halfWidth_1, halfHeight_1);
    let diffuse_2 = (baseColor_7 * (1f - metallic_6));
    let _e31 = rectAreaRangeFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXEZLDORPWC4TFMEX(lightPos_10, worldPos_14, invRangeSquared_10);
    return (((lightColor * diffuse_2) * _e21) * _e31);
}

fn evalRectAreaLtcStandardX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXEZLDORPWC4TFMEX(lightPos_11: vec3<f32>, lightColor_1: vec3<f32>, axisX_2: vec3<f32>, axisY_2: vec3<f32>, halfWidth_2: f32, halfHeight_2: f32, invRangeSquared_11: f32, worldPos_15: vec3<f32>, n_3: vec3<f32>, v_3: vec3<f32>, baseColor_8: vec3<f32>, metallic_7: f32, alphaSq_6: f32, F0_10: vec3<f32>) -> vec3<f32> {
    ...
    let _e43 = ltcEvaluateX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXEZLDORPWC4TFMEX(n_3, v_3, worldPos_15, mInv_1, lightPos_11, axisX_2, axisY_2, halfWidth_2, halfHeight_2);
    let _e48 = evalRectAreaLtcDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXEZLDORPWC4TFMEX(lightPos_11, lightColor_1, axisX_2, axisY_2, halfWidth_2, halfHeight_2, invRangeSquared_11, worldPos_15, n_3, v_3, baseColor_8, metallic_7, alphaSq_6, F0_10);
    let _e51 = rectAreaRangeFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXEZLDORPWC4TFMEX(lightPos_11, worldPos_15, invRangeSquared_11);
    return (_e48 + (((lightColor_1 * fresnel_1) * _e43) * _e51));
}

fn evalRectAreaLtcGgxX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXEZLDORPWC4TFMEX(lightPos_12: vec3<f32>, lightColor_2: vec3<f32>, axisX_3: vec3<f32>, axisY_3: vec3<f32>, halfWidth_3: f32, halfHeight_3: f32, invRangeSquared_12: f32, worldPos_16: vec3<f32>, n_4: vec3<f32>, v_4: vec3<f32>, baseColor_9: vec3<f32>, metallic_8: f32, alphaSq_7: f32, F0_11: vec3<f32>) -> vec3<f32> {
    let _e14 = evalRectAreaLtcStandardX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXEZLDORPWC4TFMEX(lightPos_12, lightColor_2, axisX_3, axisY_3, halfWidth_3, halfHeight_3, invRangeSquared_12, worldPos_16, n_4, v_4, baseColor_9, metallic_8, alphaSq_7, F0_11);
    return _e14;
}
...
fn evaluate_cluster_lightX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(light: DirectLightSlotX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX, world_pos: vec3<f32>, normal_13: vec3<f32>, view_dir: vec3<f32>, base_color: vec3<f32>, metallic_9: f32, alpha_sq: f32, f0_2: vec3<f32>, flat_2d: bool) -> vec3<f32> {
    ...
    if (kind == KIND_POINTX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX) {
        ...
        let _e27 = evalPointX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(light.position.xyz, light.colorTimesIntensity.xyz, light.position.w, world_pos, normal_13, view_dir, base_color, metallic_9, alpha_sq, f0_2);
        return _e27;
    }
    if (kind == KIND_SPOTX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX) {
        ...
        tile = encoded_tile;
        if (encoded_tile >= 0i) {
            local_31 = ((encoded_tile & PROJECTOR_FLAGX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX) != 0i);
        } else {
            local_31 = false;
        }
        let _e70 = local_31;
        ...
        if _e84 {
            ...
            let _e102 = tile;
            let _e107 = evalSpotShadowedX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(light.position.xyz, light.direction.xyz, light.colorTimesIntensity.xyz, light.colorTimesIntensity.w, light.direction.w, light.position.w, world_pos, normal_13, view_dir, base_color, metallic_9, alpha_sq, f0_2, _e101, _e102, 0.005f, 0.05f, 3f, 1f);
            return (_e107 * modifier);
        }
        let _e121 = evalSpotX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA5LOMN2HKYLMX(light.position.xyz, light.direction.xyz, light.colorTimesIntensity.xyz, light.colorTimesIntensity.w, light.direction.w, light.position.w, world_pos, normal_13, view_dir, base_color, metallic_9, alpha_sq, f0_2);
        return (_e121 * modifier);
    }
    if (kind == KIND_RECT_AREAX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX) {
        let _e139 = evalRectAreaLtcGgxX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXEZLDORPWC4TFMEX(light.position.xyz, light.colorTimesIntensity.xyz, light.auxiliary.xyz, light.direction.xyz, light.colorTimesIntensity.w, light.direction.w, light.position.w, world_pos, normal_13, view_dir, base_color, metallic_9, alpha_sq, f0_2);
        return _e139;
    }
    return vec3(0f);
}

fn evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(ndc_1: vec3<f32>, view_z_2: f32, world_pos_1: vec3<f32>, normal_14: vec3<f32>, view_dir_1: vec3<f32>, base_color_1: vec3<f32>, metallic_10: f32, alpha_sq_1: f32, f0_3: vec3<f32>, flat_2d_1: bool) -> vec3<f32> {
    ...
    loop {
        ...
        }
        {
            ...
            let _e69 = evaluate_cluster_lightX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(light_1, world_pos_1, normal_14, view_dir_1, base_color_1, metallic_10, alpha_sq_1, f0_3, flat_2d_1);
...
fn projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX(baseColorAlpha_1: f32, alphaCutoff: f32, baseColorTexture: texture_2d<f32>, baseColorSampler: sampler, transform_1: vec4<f32>, metadata_2: vec4<f32>, currentClip_1: vec4<f32>, previousClip_1: vec4<f32>, temporalProjection_3: vec4<f32>, reactive_2: f32, uv0_1: vec2<f32>, uv1_1: vec2<f32>, uv2_1: vec2<f32>, uv3_1: vec2<f32>, uv4_1: vec2<f32>, uv5_1: vec2<f32>, uv6_1: vec2<f32>, uv7_1: vec2<f32>) -> vec4<f32> {
    ...
    if (alphaCutoff > 0f) {
        local_33 = ((baseColorAlpha_1 * _e14.w) <= alphaCutoff);
    } else {
        local_33 = false;
    }
    let _e25 = local_33;
...
fn composeProbeDiffuse(shPreblend_2: array<vec4<f32>, 9>, localBlendFraction_1: f32, normal_15: vec3<f32>, skyIrradiance: vec3<f32>, kD: vec3<f32>, albedo_1: vec3<f32>, metallic_11: f32) -> vec3<f32> {
    let _e7 = evaluateProbeDiffuseX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXA4TPMJSQX(shPreblend_2, localBlendFraction_1, normal_15, skyIrradiance, kD, albedo_1, metallic_11);
    return _e7;
}
...
fn vs_main_impl(in_6: VsIn, meshIndex: u32, instanceIndex: u32) -> VsOut {
    ...
    let _e5 = palette[in_6.skinIndex.x];
    let _e13 = palette[in_6.skinIndex.y];
    let _e22 = palette[in_6.skinIndex.z];
    let _e31 = palette[in_6.skinIndex.w];
    let skinMatrix = ((((_e5 * in_6.skinWeight.x) + (_e13 * in_6.skinWeight.y)) + (_e22 * in_6.skinWeight.z)) + (_e31 * in_6.skinWeight.w));
    ...
    out_2.worldNormal = normalize((skinNormal3x3_ * in_6.normal));
    ...
    out_2.transmissionBasis0_ = vec4<f32>(skinMatrix[0].xyz, skinMatrix[1].x);
    out_2.transmissionBasis1_ = vec4<f32>(skinMatrix[1].y, skinMatrix[1].z, skinMatrix[2].x, skinMatrix[2].y);
    let clipPos = out_2.clip;
    out_2.ndc = vec4<f32>((clipPos.xy / vec2(clipPos.w)), (clipPos.z / clipPos.w), skinMatrix[2].z);
    let _e127 = viewX_naga_oil_mod_XMZXXEZ3FMF4F65TJMV3TUOTDN5WW233OX.temporalProjection;
    let _e128 = sceneViewZX_naga_oil_mod_XMZXXEZ3FMF4F643DMVXGKX3UMVWXA33SMFWAX(clipPos, _e127);
    out_2.viewZ = _e128;
    let _e129 = out_2;
    return _e129;
}
...
fn surfaceUv(input: SurfaceInput, transform_3: vec4<f32>, metadata_4: vec4<f32>) -> vec2<f32> {
    let source_2 = select(input.uv0_, input.uv1_, (metadata_4.x >= 1f));
    let scaled_2 = (source_2 * transform_3.zw);
    let c_2 = cos(metadata_4.y);
...
fn evaluate_surface(input_2: SurfaceInput) -> SurfaceData {
    ...
    return SurfaceData(baseColor_10, _e133, metallic_12, roughness_8, emissive, occlusion, clamp(((_e137 * baseSample.w) * vertexColor.w), 0f, 1f), clamp(_e147, 0f, 1f));
}
...
fn alphaTestSurface(surface: SurfaceData) {
    ...
    if (surface.alphaClipThreshold > 0f) {
        local_34 = (surface.opacity <= surface.alphaClipThreshold);
    } else {
        local_34 = false;
    }
    let _e10 = local_34;
...
@fragment 
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> @location(0) vec4<f32> {
    ...
    f0_ = mix(((vec3(dielectricF0_) * _e58) * _e60), albedo_2, metallic_13);
    physicalNormal = n_5;
    ...
    let _e109 = finiteScalar(dot(n_5, v_5), 0f);
    ...
    let fresnel_2 = clamp(_e125.x, 0f, 1f);
    let transmittedEnergy = select(0f, ((transmissionFactor * (1f - metallic_13)) * (1f - fresnel_2)), (refractedLengthSquared > 0.000001f));
    diffuseAlbedo = (albedo_2 * (1f - transmittedEnergy));
    let _e145 = physicalNormal;
    let _e149 = f0_;
    let _e150 = f_schlickX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(max(dot(_e145, v_5), 0f), _e149);
    let kD_1 = ((vec3(1f) - _e150) * (1f - metallic_13));
    ...
    if (_e162 < 0f) {
        ...
        let _e191 = sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e165, v_5, iblRoughness, _e166, in_2.worldPos, vec3<f32>(_e170, _e173, _e176), _e180.xyz, vec4<f32>(0f, 0f, 0f, 1f), prefilterMap_1, prefilterSampler_1, brdfLut_2, brdfLutSampler_2);
        specularIbl = _e191;
    } else {
        ...
        let _e210 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e201, v_5, iblRoughness, _e202, _e205, prefilterMap_1, prefilterSampler_1, brdfLut_2, brdfLutSampler_2);
        specularIbl = _e210;
    }
    ...
    let _e235 = coatF;
    ambient = ((((kD_1 * _e226) * _e228) + _e230) * (vec3(1f) - _e235));
    let _e241 = probeBlendRecords[1];
    let _e244 = probeBlendRecords[2];
    let _e247 = probeBlendRecords[3];
    let _e250 = probeBlendRecords[4];
    let _e253 = probeBlendRecords[5];
    let _e256 = probeBlendRecords[6];
    let _e259 = probeBlendRecords[7];
    let _e262 = probeBlendRecords[8];
    let _e265 = probeBlendRecords[9];
    let probeShPreblend = array<vec4<f32>, 9>(_e241, _e244, _e247, _e250, _e253, _e256, _e259, _e262, _e265);
    let probeLocalBlendFraction = probeBlendRecords[0].z;
    let probeDiffuseK = (kD_1 / max(vec3((1f - metallic_13)), vec3(0.0001f)));
    let _e278 = physicalNormal;
    let _e279 = irradiance;
    let _e281 = diffuseAlbedo;
    let _e282 = composeProbeDiffuse(probeShPreblend, probeLocalBlendFraction, _e278, (_e279 * skyFactor), probeDiffuseK, _e281, metallic_13);
    let _e283 = specularIbl;
    let _e288 = coatF;
    ambient = ((_e282 + (_e283 * skyFactor)) * (vec3(1f) - _e288));
    let _e291 = ambient;
    ambient = (_e291 * ao);
    let _e293 = ambient;
    color = _e293;
    ...
    let _e366 = material.thickness;
    let _e368 = finiteScalar(_e366, 0f);
    let _e372 = finiteScalar(thicknessSample, 1f);
    let worldThickness = max(((_e368 * length(worldRefractedDirection)) * _e372), 0f);
    ...
    let _e414 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(screenUv, vec2(0f), vec2(1f)), backdropLod);
    let unrefractedBackdrop = _e414.xyz;
    ...
    let _e422 = local_1;
    if _e422 {
        let _e430 = textureSampleLevel(transmissionBackdropTexture, transmissionBackdropSampler, clamp(refractedUv, vec2(0f), vec2(1f)), backdropLod);
        transmittedBackdrop = _e430.xyz;
    }
    let _e434 = material.attenuationColor;
    let _e437 = finiteColor(_e434, vec3(1f));
    let safeAttenuationColor = clamp(_e437, vec3(0f), vec3(1f));
    let _e445 = material.attenuationDistance;
    let _e447 = finiteScalar(_e445, 0f);
    let safeAttenuationDistance = max(_e447, 0f);
    ...
    let _e464 = local_2;
    let beerAttenuation = select(vec3(1f), pow(safeAttenuationColor, vec3(attenuationExponent)), _e464);
    let _e466 = color;
    let _e467 = transmittedBackdrop;
    let _e470 = finiteColor(_e467, vec3(0f));
    color = (_e466 + ((_e470 * transmittedEnergy) * beerAttenuation));
    let _e474 = color;
    color = (_e474 + _e6.emissive);
    let _e477 = physicalNormal;
    let _e479 = standardViewZ(in_2);
    let _e480 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e477, in_2.worldPos, _e479);
    let _e481 = physicalNormal;
    let _e482 = diffuseAlbedo;
    let _e483 = f0_;
    let _e484 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e481, v_5, _e482, metallic_13, a_3, _e483);
    let _e485 = color;
    color = (_e485 + (_e480 * _e484));
    let _e488 = color;
    let _e491 = standardViewZ(in_2);
    let _e493 = physicalNormal;
    let _e494 = diffuseAlbedo;
    let _e495 = f0_;
    let _e497 = evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_2.ndc.xyz, _e491, in_2.worldPos, _e493, v_5, _e494, metallic_13, a_3, _e495, false);
    color = (_e488 + _e497);
    let _e499 = color;
    return vec4<f32>(_e499, alpha);
}
...
fn fs_gbuffer(in_3: VsOut, @builtin(front_facing) frontFacing_1: bool) -> GBufferOutput {
    ...
    out.normal_roughness = vec4<f32>(((n_6 * 0.5f) + vec3(0.5f)), roughness_9);
    out.albedo_metallic = vec4<f32>(albedo_3, metallic_14);
...
fn vs_temporal(in_4: VsIn, @builtin(instance_index) idx_2: u32) -> TemporalVsOut {
    ...
    let _e5 = palette[in_4.skinIndex.x];
    let _e13 = palette[in_4.skinIndex.y];
    let _e22 = palette[in_4.skinIndex.z];
    let _e31 = palette[in_4.skinIndex.w];
    let currentSkin = ((((_e5 * in_4.skinWeight.x) + (_e13 * in_4.skinWeight.y)) + (_e22 * in_4.skinWeight.z)) + (_e31 * in_4.skinWeight.w));
    let _e40 = previousPalette[in_4.skinIndex.x];
    let _e48 = previousPalette[in_4.skinIndex.y];
    let _e57 = previousPalette[in_4.skinIndex.z];
    let _e66 = previousPalette[in_4.skinIndex.w];
    let previousSkin = ((((_e40 * in_4.skinWeight.x) + (_e48 * in_4.skinWeight.y)) + (_e57 * in_4.skinWeight.z)) + (_e66 * in_4.skinWeight.w));
...
fn fs_temporal(in_5: TemporalVsOut) -> @location(0) vec4<f32> {
    ...
    let _e10 = temporalVertexAlpha(in_5);
    ...
    let _e36 = projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX((_e8 * _e10), _e14, baseColorTexture_1, baseColorTexture_sampler, _e17, _e20, in_5.currentClip, in_5.previousClip, _e25, reactive_3, in_5.uv, in_5.uv1_, in_5.uv2_, in_5.uv3_, in_5.uv4_, in_5.uv5_, in_5.uv6_, in_5.uv7_);
    return _e36;
}
