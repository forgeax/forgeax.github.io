...
fn _sampleShadowForCascadeX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(worldPos_1: vec3<f32>, layer_5: u32, count_5: u32, normal_7: vec3<f32>, l_1: vec3<f32>, useNormalBias: bool) -> f32 {
    ...
    if (kernel == 3u) {
        if all((uv_5 >= (tileLo + texel_4))) {
            local_16 = all((uv_5 <= (tileHi - texel_4)));
        } else {
            local_16 = false;
        }
...
fn sample_shadow_2d_kernelX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU43IMFSG6527OBRWMX(shadowMap_2: texture_depth_2d, shadowSampler_1: sampler_comparison, uv_1: vec2<f32>, texel_3: vec2<f32>, depthRef_1: f32, normalBias_1: f32, depthBias_1: f32, nDotL_3: f32, kernelSize: f32) -> f32 {
            ...
            loop {
                ...
                }
                {
                    ...
                    if _e41 {
                        let _e44 = x_2;
                        let _e46 = y_2;
                        let offsetUv_2 = (uv_1 + (vec2<f32>(f32(_e44), f32(_e46)) * texel_3));
...
fn spotModifierFactorsX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXG4DPORPW233ENFTGSZLSOMX(lightPos_7: vec3<f32>, lightDir_4: vec3<f32>, cosOuter_4: f32, worldPos_11: vec3<f32>, rollDeg: f32, metadata: vec4<u32>) -> vec4<f32> {
    ...
    if (metadata.w != 4294967295u) {
        if !((metadata.y == 4294967295u)) {
            local_24 = ((metadata.y & PROJECTOR_FLAGX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PXG4DPORPW233ENFTGSZLSOMX) == 0u);
        } else {
            local_24 = true;
        }
        let _e90 = local_24;
    ...
    if _e94 {
        if (depth <= 0f) {
            ...
            if !((cookieUv.x < 0f)) {
                local_25 = (cookieUv.x > 1f);
            } else {
                local_25 = true;
            }
            let _e137 = local_25;
            if !(_e137) {
                local_26 = (cookieUv.y < 0f);
            } else {
                local_26 = true;
            }
            let _e145 = local_26;
            if !(_e145) {
                local_27 = (cookieUv.y > 1f);
            } else {
                local_27 = true;
            }
            let _e153 = local_27;
...
fn projectPbrSceneTemporalX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU5DFNVYG64TBNQX(baseColorAlpha_1: f32, alphaCutoff: f32, baseColorTexture: texture_2d<f32>, baseColorSampler: sampler, transform_1: vec4<f32>, metadata_2: vec4<f32>, currentClip_1: vec4<f32>, previousClip_1: vec4<f32>, temporalProjection_3: vec4<f32>, reactive_2: f32, uv0_1: vec2<f32>, uv1_1: vec2<f32>, uv2_1: vec2<f32>, uv3_1: vec2<f32>, uv4_1: vec2<f32>, uv5_1: vec2<f32>, uv6_1: vec2<f32>, uv7_1: vec2<f32>) -> vec4<f32> {
    ...
    if (alphaCutoff > 0f) {
        local_30 = ((baseColorAlpha_1 * _e14.w) <= alphaCutoff);
    } else {
        local_30 = false;
    }
    let _e25 = local_30;
...
fn vs_main_impl(in_6: VsIn, meshIndex: u32, instanceIndex: u32) -> VsOut {
    ...
    out_2.positionOSAndViewZ = vec4<f32>(in_6.pos, _e70);
...
fn evaluateStandardSurface(in_10: VsOut, frontFacing_2: bool) -> SurfaceData {
    ...
    let _e14 = materialVertexColor(in_10);
    let _e17 = evaluate_surface(SurfaceInput(positionOS, in_10.worldPos, in_10.worldNormal, in_10.worldTangent, viewDirectionWS, in_10.uv, in_10.uv1_, _e14, frontFacing_2));
    return _e17;
}
...
fn alphaTestSurface(surface: SurfaceData) {
    ...
    if (surface.alphaClipThreshold > 0f) {
        local_31 = (surface.opacity <= surface.alphaClipThreshold);
    } else {
        local_31 = false;
    }
    let _e10 = local_31;
...
fn fs_main(in_2: VsOut, @builtin(front_facing) frontFacing: bool) -> @location(0) vec4<f32> {
    ...
    let _e72 = f0_;
    let _e73 = f_schlickX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DUYTSMRTAX(max(dot(_e68, v_5), 0f), _e72);
    let kD_1 = ((vec3(1f) - _e73) * (1f - metallic_13));
    ...
    if (_e85 < 0f) {
        ...
        let _e114 = sampleReflectionProbeSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e88, v_5, iblRoughness, _e89, in_2.worldPos, vec3<f32>(_e93, _e96, _e99), _e103.xyz, vec4<f32>(0f, 0f, 0f, 1f), prefilterMap_1, prefilterSampler_1, brdfLut_2, brdfLutSampler_2);
        specularIbl = _e114;
    } else {
        ...
        let _e133 = sampleIblSpecularX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU2LCNRPXGYLNOBWGS3THX(_e124, v_5, iblRoughness, _e125, _e128, prefilterMap_1, prefilterSampler_1, brdfLut_2, brdfLutSampler_2);
        specularIbl = _e133;
    }
    ...
    let skyColor = vec3<f32>(_e137, _e140, _e143);
    let _e147 = skylight.intensity;
    let skyFactor = (skyColor * _e147);
    let _e149 = irradiance;
    let _e151 = diffuseAlbedo;
    let _e153 = specularIbl;
    let _e158 = coatF;
    ambient = ((((kD_1 * _e149) * _e151) + _e153) * (vec3(1f) - _e158));
    let _e164 = probeBlendRecords[1];
    let _e167 = probeBlendRecords[2];
    let _e170 = probeBlendRecords[3];
    let _e173 = probeBlendRecords[4];
    let _e176 = probeBlendRecords[5];
    let _e179 = probeBlendRecords[6];
    let _e182 = probeBlendRecords[7];
    let _e185 = probeBlendRecords[8];
    let _e188 = probeBlendRecords[9];
    let probeShPreblend = array<vec4<f32>, 9>(_e164, _e167, _e170, _e173, _e176, _e179, _e182, _e185, _e188);
    ...
    let _e202 = irradiance;
    let _e204 = diffuseAlbedo;
    let _e205 = composeProbeDiffuse(probeShPreblend, probeLocalBlendFraction, _e201, (_e202 * skyFactor), probeDiffuseK, _e204, metallic_13);
    let _e206 = specularIbl;
    let _e211 = coatF;
    ambient = ((_e205 + (_e206 * skyFactor)) * (vec3(1f) - _e211));
    let _e214 = ambient;
    ambient = (_e214 * ao);
    let _e216 = ambient;
    color = _e216;
    let _e218 = color;
    color = (_e218 + _e6.emissive);
    let _e221 = physicalNormal;
    let _e223 = standardViewZ(in_2);
    let _e224 = evalDirectionalShadowFactorX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e221, in_2.worldPos, _e223);
    let _e225 = physicalNormal;
    let _e226 = diffuseAlbedo;
    let _e227 = f0_;
    let _e228 = evalDirectionalNoShadowX_naga_oil_mod_XMZXXEZ3FMF4F64DCOI5DU3DJM5UHI2LOM5PWI2LSMVRXI2LPNZQWYX(_e225, v_5, _e226, metallic_13, a_3, _e227);
    let _e229 = color;
    color = (_e229 + (_e224 * _e228));
    let _e232 = color;
    let _e235 = standardViewZ(in_2);
    let _e237 = physicalNormal;
    let _e238 = diffuseAlbedo;
    let _e239 = f0_;
    let _e241 = evaluateStandardClusterLightsX_naga_oil_mod_XMZXXEZ3FMF4F643UMFXGIYLSMQ5DUY3MOVZXIZLSX(in_2.ndc.xyz, _e235, in_2.worldPos, _e237, v_5, _e238, metallic_13, a_3, _e239, false);
    color = (_e232 + _e241);
    let _e243 = color;
    return vec4<f32>(_e243, alpha);
}
