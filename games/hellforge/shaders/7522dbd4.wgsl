...
fn fs_main(in_1: VsOut) -> @location(0) vec4<f32> {
    ...
    let _e16 = hash3_(floor((n * 4f)));
    let _e21 = u.time;
    let _e26 = u.time;
    let _e33 = hash3_(floor(((n * 9f) + vec3<f32>(0f, (_e21 * 4.5f), (_e26 * 2.1f)))));
    let shimmer = ((0.88f + (0.22f * _e16)) + (0.12f * _e33));
    let _e49 = u.intensity;
    let amp = (((0.28f + ((0.72f * facing) * facing)) * shimmer) * _e49);
    let ampSafe = min(amp, 1.05f);
    let alpha = (clamp((facing * 1.55f), 0f, 1f) * clamp(ampSafe, 0f, 1f));
    let _e64 = u.baseColor;
    let rgb = (_e64.xyz * ampSafe);
