...
fn fs_main(in_1: VsOut) -> @location(0) vec4<f32> {
    ...
    let _e17 = u.time;
    let _e22 = u.time;
    let _e29 = hash3_(floor(((n * 5f) + vec3<f32>(0f, (_e17 * 7f), (_e22 * 3f)))));
    let flick = (0.82f + (0.36f * _e29));
    let _e42 = u.intensity;
    let amp = (((0.25f + ((0.85f * facing) * facing)) * flick) * _e42);
    let alpha = (clamp((facing * 1.6f), 0f, 1f) * clamp(amp, 0f, 1f));
    let _e55 = u.baseColor;
    let rgb = (_e55.xyz * amp);
